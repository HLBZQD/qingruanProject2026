# 引入国产金仓数据库 KingbaseES —— 技术方案（v15）

> **版本声明（v14 新增）**：本文件名为 `a_v10_copy_from_v9.md`，其中 `v10` 表示第 10 轮迭代的产出文件；文档标题中的 `v14` 表示该产出（自 v1 起）经历的第 14 次修订。两种编号体系补充说明：文件名序号对应迭代轮次，内容版本号对应修订次数。首次接触本方案的实现者以文档标题中的版本号（v14）为准——它反映方案当前的最新修订状态。
...
**DDL 文件（`init_kingbase_ddl.sql`）执行流程**：
  ...
  3. 过滤空语句（纯空白行、仅含注释的行）
  4. **DDL 语句在事务外逐条执行**（每条 DDL 通过 `pool.query()` 独立执行）。DDL 不使用事务包裹——原因：KingbaseES V8R6 事务内 DDL 的隐式提交行为未经充分验证，DDL 包裹在事务中风险不可控（若部分表 CREATE 后触发隐式提交，后续 DDL 失败时已创建的表无法回滚）。DDL 本身通过 `CREATE TABLE IF NOT EXISTS` 保证幂等性——重复执行不产生副作用
  5. 若某条 DDL 执行失败，记录错误日志并继续执行后续 DDL（CREATE TABLE IF NOT EXISTS 失败通常为权限或磁盘问题，不应阻塞其他表的创建）。全部 DDL 执行完毕后若存在失败，汇总输出错误列表
...
#### 3.4.8 pg 驱动 timestamp 类型自动解析控制（v11 新增）

**问题**：`pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列自动解析为 JavaScript Date 对象，JSON 序列化后变为 ISO 8601 字符串（如 `"2025-06-28T06:30:00.000Z"`），而 SQLite 路径下 timestamp 以纯文本形式返回（如 `"2025-06-28 06:30:00"`，空格分隔，无时区后缀）。同一 API 端点在 SQLite 和 KingbaseES 两个后端下返回不同的 JSON 格式，直接违背方案第 14 节"前端代码零变动"和第 6 节"所有现有 API 端点返回的 HTTP 状态码和响应结构与改造前一致"的核心承诺。影响 9 张表的 11 个 datetime 字段（`users.created_at/updated_at`、`doctor_information.created_at`、`articles.created_at`、`article_collections.created_at`、`user_risk_info.created_at`、`life_plans.created_at/updated_at`、`life_advice.created_at`、`punch_in.punch_time`、`admin_logs.operation_time`）。

**解决方案**：在 KingbaseAdapter 构造函数中，向 `pg.Pool` 注册自定义类型解析器，将 timestamp（OID 1114）和 timestamptz（OID 1184）类型的解析器设为原样返回字符串（`val => String(val)`），与 SQLite 行为保持一致：
...
pg.types.setTypeParser(1182, val => String(val));  // timestamptz（KingbaseES 可能的备用 OID，兼容注册）
```

**为什么不保留 Date 对象行为**：
1. 项目 SQLite 路径下 timestamp 以纯文本形式返回，前端可能已依赖此格式进行字符串比较或展示
2. 保留 Date 对象将破坏"前端代码零变动"的核心承诺，迫使前端对所有 datetime 字段做格式适配
...
- **OID 值说明**：1184 是 PostgreSQL 12 中 `timestamptz` 类型的标准 OID（KingbaseES V8R6 基于 PostgreSQL 12 内核对齐），1182 作为 KingbaseES 可能的备用 OID 兼容注册。同时注册两个 OID 确保无论 KingbaseES 实例使用哪个 OID 标识 timestamptz 类型，解析器都能正确拦截。`setTypeParser` 对同一类型多次注册以最后一次为准，对无效或不存在的 OID 注册不产生副作用。
- **启动验证**（推荐）：在 KingbaseAdapter 构造函数或 `init()` 方法中，启动后执行验证查询 `SELECT oid, typname FROM pg_type WHERE typname IN ('timestamp','timestamptz')` 并打印日志记录实际激活的 OID 值，便于未来版本升级后排查 OID 变化问题。
...
### 4.2.1 `server/utils/dateRange.js` 工具模块兼容性评估（v11 新增）
...
**结论：`dateRange.js` 无需改造，`T` 分隔符格式在 KingbaseES V8R6 中兼容。**
...
2. 在 KingbaseES V8R6 测试实例上执行以下验证：
   ```sql
   SELECT '2025-06-28T23:59:59'::TIMESTAMP;  -- 应成功返回 2025-06-28 23:59:59
   SELECT * FROM punch_in WHERE punch_time <= '2025-06-28T23:59:59' LIMIT 1;  -- 验证隐式转换正确性
   ```
3. SQLite 同样能正确解析含 `T` 分隔符的日期字符串——SQLite 的日期时间函数接受 ISO 8601 格式输入
...
- **前置条件（v14 新增）——`pg` 驱动与 KingbaseES V8R6 兼容性验证**：Phase 1 实现开始前，必须在目标 KingbaseES V8R6 实例上验证 `pg` v8.12 的以下核心功能，验证通过后方可进入 KingbaseAdapter 实现阶段：
  ...
  5. **数据类型映射**：验证 TIMESTAMP、JSONB、INTEGER、TEXT、BOOLEAN 列的往返读写正确（写入值 → 读取值 → 类型和内容一致）
  6. **`pg.types.setTypeParser` 对 TIMESTAMP/TIMESTAMPTZ 的拦截**：验证第 3.4.8 节的解析器配置在目标实例上生效（timestamp 列返回字符串而非 Date 对象）
  - 验证方法：编写 `scripts/verify-pg-kingbase.js` 脚本，逐项执行上述 6 个验证用例，全部通过后输出 `[PASS] pg v8.12 + KingbaseES V8R6 兼容性验证通过`。任何一项失败则输出 `[FAIL]` 及详细错误信息
...
**`query_table` 安全维度说明（v11 新增）**：`query_table` 操作将 Dify AI 生成的 WHERE/ORDER BY 子句通过字符串插值直接拼接到 SQL 语句中（`WHERE ${params.where}`），虽然 `params.where` 由 Dify AI 生成（非外部用户直接输入），但此模式仍构成 SQL 注入的间接攻击面——若 Dify AI 被恶意 prompt 操纵生成含注入代码的 WHERE 子句，注入代码将绕过参数化查询保护直接在数据库中执行。当前缓解措施仅为日志记录（事后审计），缺乏事前防护。**Phase 1 保持现状但记录到风险表**（见第 15 节"query_table WHERE 子句注入路径"风险项）。Phase 2+ 建议：(1) 在 Dify system prompt 中加入安全约束（禁止生成含子查询/DML 的 WHERE 子句）；(2) 应用层增加 WHERE 子句的安全校验（如使用 `node-sql-parser` 解析 WHERE 子句并检查是否仅包含条件表达式，拒绝含子查询或 DML 语句的 WHERE 子句）。
...
| 27 | 全部表（6 个 JSON 列） | TEXT → JSONB | `articles.tags`, `user_risk_info.result`, `user_risk_info.raw_input`, `admin_logs.operation_content`, `admin_logs.operation_result`, `life_advice.tags` 类型改为 JSONB | [ ] |
...
| **（v11 新增）pg 驱动 timestamp 类型自动解析导致 API 响应 JSON 格式不一致** | `pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列自动解析为 JavaScript Date 对象，JSON 序列化后变为 ISO 8601 字符串（如 `"2025-06-28T06:30:00.000Z"`），而 SQLite 路径下 timestamp 以纯文本形式返回（如 `"2025-06-28 06:30:00"`），同一 API 端点在两个后端下返回不同 JSON 格式，违背"前端代码零变动"的核心承诺 | 在 KingbaseAdapter 构造函数中通过 `pg.types.setTypeParser(1114, val => String(val))` 和 `pg.types.setTypeParser(1184, val => String(val))` 将 timestamp/timestamptz 类型的解析器设为原样返回字符串，与 SQLite 行为保持一致（见 3.4.8 节） |
...
| **（v11 新增）`dateRange.js` 工具模块含 T 分隔符的日期格式在 KingbaseES 中的隐式类型转换行为未经确认** | `punch.js` 依赖 `parseDateRange()` 输出 `YYYY-MM-DDTHH:MM:SS` 格式的 endDate 参数（含 T 分隔符），在 KingbaseES 中需隐式将 TEXT 转换为 TIMESTAMP 才能与 `punch_time` 列比较，此格式的转换行为方案未评估 | 已确认：含 T 分隔符的 ISO 8601 格式日期字符串是 PostgreSQL TIMESTAMP 解析的标准输入格式之一，`dateRange.js` 无需改造。Phase 1 双库对比测试验证时间范围筛选的一致性（见 4.2.1 节） |
...
| **（v11 新增）`query_table` WHERE 子句 SQL 注入路径未列入风险表** | `admin.js` 的 `query_table` tool_name 将 Dify AI 生成的 WHERE/ORDER BY 子句直接字符串插值到 SQL 中（`WHERE ${params.where}`），虽参数源于 Dify AI 而非外部用户，仍构成间接 SQL 注入攻击面。方案仅提到日志记录作为缓解，但第 15 节风险表中无对应风险项 | 在第 15 节风险表中新增此风险条目；在第 9.2 节 `query_table` 风险说明中增加安全维度讨论和 Phase 2+ 防护建议（Dify prompt 安全约束 + 应用层 WHERE 子句安全校验）（见 9.2 节"query_table 安全维度说明"） |
...
| 新建 | `server/db/adapter/KingbaseAdapter.js` | KingbaseAdapter 实现（含 init 拆分 DDL/种子文件执行（v14 修订）、`?`→`$1` 占位符转换含回退日志（v14 新增）、RETURNING id 自动追加、tableInfo、healthCheck、close（v11 新增）方法、pg.types.setTypeParser timestamp 类型解析器配置（v11 新增）、SSL/TLS 配置、pool.on('error') 事件处理） |
...
| 新建 | `scripts/verify-pg-kingbase.js` | pg v8.12 + KingbaseES V8R6 兼容性验证脚本（v14 新增，Phase 1 前置条件）。逐项验证：TCP 连接认证、参数化查询、事务、连接池、数据类型映射、timestamp 解析器拦截。全部通过后输出 `[PASS]`，任何失败输出 `[FAIL]` |
