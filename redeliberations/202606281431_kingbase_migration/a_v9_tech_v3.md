...
**v11 补充 —— timestamp 字段 JSON 响应格式一致性**：KingbaseAdapter 中通过 `pg.types.setTypeParser` 将 timestamp/timestamptz 类型的解析器设为原样返回字符串（见 3.4.8 节），确保 KingbaseES 后端下 datetime 字段的 JSON 响应格式（`"YYYY-MM-DD HH:MM:SS"`）与 SQLite 后端一致。此措施是"前端代码零变动"承诺的关键保障——若未配置此解析器，`pg` 驱动默认将 datetime 字段序列化为 ISO 8601 格式（`"YYYY-MM-DDTHH:MM:SS.000Z"`），前端可能因格式不一致而产生展示或解析错误。
...
| 新建 | `server/db/adapter/KingbaseAdapter.js` | KingbaseAdapter 实现（含 init 多语句执行、`?`→`$1` 占位符转换、RETURNING id 自动追加、tableInfo、healthCheck、close（v11 新增）方法、pg.types.setTypeParser timestamp 类型解析器配置（v11 新增）、SSL/TLS 配置、pool.on('error') 事件处理） |
...
| 重写 | `server/db/init_kingbase.sql` | 对齐 init.sql schema（使用 `CREATE TABLE IF NOT EXISTS`，补充 `result`/`diabetes_history`/`diabetes_type` 字段、全部 18 个索引含 `idx_plans_user_plan` UNIQUE 索引、doctor_information 和 punch_in 约束修复、列名/枚举值统一）；种子数据使用 `__BCRYPT_HASH_PLACEHOLDER__` 占位符，内容对齐 seed.sql |
