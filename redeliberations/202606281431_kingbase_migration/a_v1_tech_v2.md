...
- 实现 `KingbaseAdapter`（含 `init()` 的多语句 SQL 执行和密码哈希占位符替换机制）
...
| 新建 | `server/db/adapter/kingbase.js` | KingbaseAdapter 实现（含 init 多语句执行、RETURNING id 自动追加、tableInfo 方法） |
...
| 重写 | `server/db/init_kingbase.sql` | 对齐 init.sql schema；种子数据使用 `__BCRYPT_HASH_PLACEHOLDER__` 占位符 |
