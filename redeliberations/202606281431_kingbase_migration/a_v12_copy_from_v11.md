# 引入国产金仓数据库 KingbaseES —— 技术方案（v16）

> **版本声明（v14 新增，v16 修正）**：本文件名为 `a_v12_copy_from_v11.md`，其中 `v12` 表示第 12 轮迭代的产出文件；文档标题中的 `v16` 表示该产出（自 v1 起）经历的第 16 次修订。两种编号体系补充说明：文件名序号对应迭代轮次，内容版本号对应修订次数。首次接触本方案的实现者以文档标题中的版本号（v16）为准——它反映方案当前的最新修订状态。
...
- `close()`：调用 `db.close()`
- **`this.db` 属性暴露（v10 新增）**：构造函数中 `this.db = new Database(dbPath)` 创建的 better-sqlite3 Database 实例需通过 `this.db` 属性对外暴露（即 `adapter.db`）。此属性供 Phase 0 双导出过渡策略使用——`database.js` 在步骤 2 中通过 `module.exports = { db: adapter.db, getAdapter: () => adapter, initDatabase }` 保持旧路由文件的 `db` 引用可用（见 3.5.2 节）。实现者仅阅读本节可能遗漏此暴露需求，务必在 SqliteAdapter 实现中显式声明 `this.db` 属性。
...
- **参数占位符转换（重要）**：`pg`（node-postgres）仅支持 `$1, $2, ...` 格式的参数占位符，而当前项目全部使用 `?` 占位符。为保持路由层最小改动（继续使用 `?` 风格），KingbaseAdapter 的 `query()`、`queryOne()`、`execute()` 等方法必须在将 SQL 传递给 `pool.query()` 之前，将 SQL 中的 `?` 占位符转换为 `$1, $2, ...` 格式。转换策略：
  ...
  - **SQL 注释处理说明**：状态机无需处理 SQL 注释（`--`、`/* */`）。原因：路由层运行时的 SQL 语句不包含注释——注释仅出现在 DDL 初始化脚本（`init.sql`/`init_kingbase_ddl.sql`/`init_kingbase_seed.sql`）中，而这些脚本通过独立的 `init()` 多语句执行流程处理（其分号分割步骤已包含注释移除逻辑，见 3.4.5 节 init 方法）。路由层动态 SQL 中不存在注释内的 `?` 被误转换的风险。
...
- `execute()` 的 INSERT ID 获取策略：
  ...
  - 解析失败时（例如极特殊的 SQL 语法），回退到正则匹配 `/^\s*INSERT\s+/i.test(sql)` 检测，并追加 `RETURNING id`。**回退日志（v14 新增）**：当回退到正则匹配时，使用 `console.warn('[KingbaseAdapter] node-sql-parser 解析 INSERT 失败，回退到正则匹配。SQL: ' + sql.substring(0, 200))` 输出警告日志。此日志确保运维人员可观测到生产环境中是否存在因解析失败而走了回退路径的 SQL——若频繁出现此日志，表明存在 `node-sql-parser` 无法处理的 SQL 模式，需调查并扩展解析策略
...
- KingbaseAdapter.init() 采用**拆分 DDL + 种子数据两文件方案**（唯一实施路径）：

**DDL 文件（`init_kingbase_ddl.sql`）执行流程**：
  1. 使用 `fs.readFileSync` 读取 `init_kingbase_ddl.sql`（仅含 `CREATE TABLE IF NOT EXISTS` 和 `CREATE INDEX IF NOT EXISTS` 语句，不含任何种子数据 INSERT）
  2. 按 `;` 分割 SQL 文本为独立 DDL 语句。**分割注意事项**：需处理字符串字面量和注释中的分号。推荐策略：先移除单行注释（`--` 开头行）和多行注释（`/* ... */`），再用简单状态机跳过单引号字符串内的分号后分割
  3. 过滤空语句（纯空白行、仅含注释的行）
  4. **DDL 文件按 FK 依赖拓扑顺序排列（v16 新增）**：`init_kingbase_ddl.sql` 中的 CREATE TABLE 语句必须按外键依赖的拓扑顺序排列——父表在前、子表在后。推荐的 FK 拓扑顺序为：`users → doctor_information → diabetes_types → articles → article_collections → user_risk_info → life_plans → life_advice → punch_in → admin_logs`（与第 12 节迁移顺序一致）。此顺序确保任何子表的 CREATE TABLE 在引用父表时父表已存在，消除因 FK 依赖引发的级联失败
  5. **DDL 语句在事务外逐条执行**（每条 DDL 通过 `pool.query()` 独立执行）。DDL 不使用事务包裹——原因：KingbaseES V8R6 事务内 DDL 的隐式提交行为未经充分验证，DDL 包裹在事务中风险不可控（若部分表 CREATE 后触发隐式提交，后续 DDL 失败时已创建的表无法回滚）。DDL 本身通过 `CREATE TABLE IF NOT EXISTS` 保证幂等性——重复执行不产生副作用
  6. **DDL 失败处理策略（v16 修订）**：若某条 DDL 执行失败（如权限或磁盘问题），采用以下策略：
     - **根因 vs 级联失败区分**：若依赖子表因父表创建失败而级联失败，错误输出中区分"根因失败"（父表 DDL 错误）和"级联失败"（因父表不存在导致 FK 引用失败）。级联失败的子表 DDL 应在日志中标注 `[CASCADED]` 前缀，帮助运维人员快速定位根因
     - **首条失败终止策略（推荐，v16 新增）**：改为"首条 DDL 失败即终止"。理由：DDL 通过的 `CREATE TABLE IF NOT EXISTS` 保证幂等性——终止后修复问题重新运行 init() 不会产生副作用（已创建的表被 `IF NOT EXISTS` 跳过后继续，未创建的表从断点开始创建）。原"继续执行后续 DDL"策略在父表创建失败时会导致所有依赖子表级联失败，错误列表含大量噪音、运维人员难以定位根因。实现者可根据团队偏好选择"首条失败终止"或"继续执行+根因标注"策略
  7. 全部 DDL 执行完毕后若采用"继续执行"策略且存在失败，汇总输出错误列表（含根因和级联失败的区分标注）

**种子数据文件（`init_kingbase_seed.sql`）执行流程**：
  6. 使用 `fs.readFileSync` 读取 `init_kingbase_seed.sql`（仅含种子数据 INSERT 语句：管理员用户、医生信息、文章等）
  7. 使用 `bcryptjs`（项目已依赖）运行时生成 `admin123` 的 bcrypt 哈希，替换 SQL 文本中的占位符 `__BCRYPT_HASH_PLACEHOLDER__`
  8. 按 `;` 分割为独立 INSERT 语句（单引号字符串内的分号跳过处理同步骤 2）
  9. **种子数据在事务内执行**：从 pool 获取 client → `BEGIN` → 逐条 `client.query()` → `COMMIT` → `client.release()`。事务保证种子数据插入的原子性——全部插入成功或全部回滚
  10. **幂等检查顺序**：在事务开始前，先查询 `SELECT COUNT(*) FROM users` 判断是否已有数据。若 `users` 表非空（`count > 0`），则跳过整个种子数据事务——种子数据已存在，无需重复插入。若 `users` 表为空，则执行种子数据事务

- **备选方案（DEPRECATED, v14 废弃）**：原方案（v3-v13）推荐将 DDL 包裹在事务中执行，待实现前验证 KingbaseES 事务内 DDL 行为。该方案存在内部矛盾——步骤 6 要求"在一个事务内顺序执行所有语句"，同时"事务内 DDL 兼容性验证"要求"DDL 不在事务中执行"。**v14 修订**：以拆分方案为唯一实施路径，撤销单文件事务内执行方案。原备选方案降级为历史参考，不再作为可选的实施路径。

**设计理由（v14 新增）**：
- DDL 文件与种子数据文件的拆分消除了原方案步骤 6 与"事务内 DDL 兼容性验证"之间的内部矛盾
- DDL 不在事务中执行的决策基于 `CREATE TABLE IF NOT EXISTS` 已提供充分的幂等保障——DDL 重复执行安全，无需事务包裹
- 种子数据在事务中执行保证了多表种子数据插入的原子性（如管理员用户 + 医生信息 + 文章必须全部成功或全部回滚）
- 文件拆分降低单文件分号分割复杂度——DDL 文件中不含 INSERT，种子数据文件中不含 CREATE，各自的分号语义更清晰
...
#### 3.4.8 pg 驱动类型自动解析控制（timestamp + JSONB/JSON）（v11 新增，v16 扩展）

**问题**：`pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列自动解析为 JavaScript Date 对象，将 JSONB（OID 3802）和 JSON（OID 114）列自动解析为 JavaScript 对象。JSON 序列化后，timestamp 变为 ISO 8601 字符串（如 `"2025-06-28T06:30:00.000Z"`），JSONB/JSON 变为原生 JSON 对象而非 TEXT 字符串。而 SQLite 路径下 timestamp 和 JSON 均以纯文本形式返回。同一 API 端点在 SQLite 和 KingbaseES 两个后端下返回不同的 JSON 格式，直接违背方案第 14 节"前端代码零变动"和第 6 节"所有现有 API 端点返回的 HTTP 状态码和响应结构与改造前一致"的核心承诺。

影响范围：
- timestamp 类型：9 张表的 11 个 datetime 字段（`users.created_at/updated_at`、`doctor_information.created_at`、`articles.created_at`、`article_collections.created_at`、`user_risk_info.created_at`、`life_plans.created_at/updated_at`、`life_advice.created_at`、`punch_in.punch_time`、`admin_logs.operation_time`）
- JSONB/JSON 类型：4 个 JSON 存储列（`articles.tags`——articles.js 第 35/55/149/161 行使用 `JSON.parse` 解析；`life_advice.tags`——assistant.js 第 48 行 `JSON.parse`；`user_risk_info.result`——riskFormStore.ts 第 96 行 typeof 检查；`user_risk_info.raw_input`）。若 `JSON.parse` 接收到对象（pg 驱动已将 JSONB 自动解析为 JS 对象），将抛出 `TypeError` 被 catch 返回空数组/空对象——造成**静默数据丢失**。`admin_logs.operation_content` 和 `admin_logs.operation_result` 实际存储纯文本（非 JSON），不受此影响（此二列不使用 JSONB 类型，详见第 10.1/10.2 节）。

**解决方案**：在 KingbaseAdapter 构造函数中，向 `pg.Pool` 注册自定义类型解析器，将 timestamp（OID 1114）、timestamptz（OID 1184）、JSONB（OID 3802）、JSON（OID 114）类型的解析器均设为原样返回字符串（`val => String(val)`），与 SQLite 行为保持一致：

```javascript
// KingbaseAdapter 构造函数内，pg.Pool 实例化之前
pg.types.setTypeParser(1114, val => String(val));  // timestamp → 保持字符串
pg.types.setTypeParser(1184, val => String(val));  // timestamptz（PostgreSQL 12 标准 OID）
pg.types.setTypeParser(3802, val => String(val));  // jsonb → 保持字符串（v16 新增）
pg.types.setTypeParser(114,  val => String(val));  // json → 保持字符串（v16 新增）
```

**OID 1182 说明**（v16 修正）：前序版本（v11-v15）额外注册了 OID 1182 作为 timestamptz 的"KingbaseES 可能的备用 OID"。经核实——PostgreSQL 12 中 OID 1182 并非任何标准日期时间类型的 OID（OID 1182 在 PostgreSQL 12 中对应 `date` 类型的数组类型 `_date`，与 timestamptz 无关）。将 `pg.types.setTypeParser(1182, val => String(val))` 注册到 `_date` 数组类型上，虽然不会产生副作用（该类型不在本项目中使用），但该行代码缺乏来源依据且可能误导后续维护者。**已从代码示例中删除 OID 1182 注册行**。若 KingbaseES 实例的 timestamptz 使用非标准 OID，应通过启动验证中的 `SELECT oid, typname FROM pg_type WHERE typname IN ('timestamp','timestamptz','jsonb','json')` 查询确认实际 OID 后再决定是否额外注册，而非预先注册一个无依据的"可能备用 OID"。

**为什么不保留 pg 驱动的默认自动解析行为**：
1. 项目 SQLite 路径下 timestamp 和 JSON 均以纯文本形式返回，前端可能已依赖此格式进行字符串比较或展示
2. 保留 Date 对象和 JSON 对象的自动解析将破坏"前端代码零变动"的核心承诺，迫使前端对所有 datetime 和 JSON 字段做格式适配
3. `.toISOString()` 在 JSON 序列化时自动调用，输出的 ISO 8601 格式（含 `T` 和 `Z`）与 SQLite 的 `YYYY-MM-DD HH:MM:SS` 格式在字符串排序和比较上不兼容
4. JSON 对象的自动解析尤其危险——`articles.tags` 等字段在代码中执行 `JSON.parse()` 二次解析，若 pg 驱动已将值解析为对象，`JSON.parse(object)` 抛 TypeError 被 catch 静默返回空数组，导致**标签数据静默丢失**（问题严重程度高于 timestamp 的格式不一致）

**注意事项**：
- `pg.types.setTypeParser()` 是模块级全局函数，设置后影响所有 `pg.Pool` 实例。由于本项目同一进程内仅使用一个 `pg.Pool`，此副作用不影响其他模块
- 若将来需要 Date 对象进行时间运算，可在应用层显式 `new Date(str)` 转换
- 若将来需要 JSON 对象进行操作，可在应用层显式 `JSON.parse(str)` 转换
- **OID 值说明**：1114（timestamp）、1184（timestamptz）、3802（jsonb）、114（json）均为 PostgreSQL 12 标准 OID（KingbaseES V8R6 基于 PostgreSQL 12 内核对齐）。`setTypeParser` 对同一类型多次注册以最后一次为准
- **启动验证**（推荐）：在 KingbaseAdapter 构造函数或 `init()` 方法中，启动后执行验证查询 `SELECT oid, typname FROM pg_type WHERE typname IN ('timestamp','timestamptz','jsonb','json')` 并打印日志记录实际激活的 OID 值，便于未来版本升级后排查 OID 变化问题
- **BOOLEAN 类型说明**（v16 新增，横切关注点）：`pg` 驱动默认将 BOOLEAN（OID 16）列自动解析为 JavaScript boolean（true/false），而 SQLite 使用 INTEGER（1/0）返回 JS number。为保持行为一致，DDL 层面应保持使用 `INTEGER`/`SMALLINT` 类型而非 `BOOLEAN`——本项目所有表的布尔语义列（如 `is_active`、`password_changed`）在 `init.sql` 中使用 `INTEGER NOT NULL DEFAULT 0`，`init_kingbase_ddl.sql` 中继续使用 `INTEGER`/`SMALLINT` 保持类型一致。无需额外注册 OID 16 的解析器。

#### 3.4.9 `close()` 方法实现（v11 新增）

适配层接口定义包含 `async close() → void` 方法，SqliteAdapter 有 `close()` 实现描述（调用 `db.close()`），但 KingbaseAdapter 的 `close()` 实现（需调用 `pool.end()`）在方案中完全缺失。

**实现**：

```javascript
// KingbaseAdapter.close()
async close() {
  await this.pool.end();
...
`pg.Pool.end()` 的行为：等待所有进行中的查询完成 → 断开所有连接 → 返回 Promise。调用后 pool 不再接受新查询。此方法应在应用优雅关闭流程中调用（见 3.5.1 节"优雅关闭"段落）。

#### 3.4.10 KingbaseES 服务器时区验证（v13 新增）

KingbaseAdapter 在初始化完成后应验证 KingbaseES 服务器的 `timezone` 参数是否为 UTC。若服务器时区非 UTC，`CURRENT_TIMESTAMP` 将返回本地时间而非 UTC（见 4.2 节"KingbaseES 服务器时区配置要求"），直接破坏方案"统一 UTC 存储"的核心承诺。

**验证时机**：在 `init()` 方法的事务提交后、返回前（即在幂等检查之后）执行。此时数据库连接已建立，DDL 已执行完毕，可安全执行查询。

**验证实现**：

```javascript
// KingbaseAdapter.init() 方法内部，事务提交后、返回前
async init() {
  // ... DDL 执行 + 种子数据初始化（已在事务内完成）...

  // 时区验证（v13 新增）
  try {
    const { rows } = await this.pool.query("SELECT current_setting('timezone') AS tz");
    const tz = rows[0]?.tz || '';
    console.log(`[KingbaseAdapter] 服务器时区: ${tz}`);
    if (tz !== 'UTC' && tz !== 'Etc/UTC') {
      console.error(
        `[KingbaseAdapter] ⚠️ 警告：KingbaseES 服务器时区为 "${tz}"，非 UTC！` +
        `CURRENT_TIMESTAMP 将返回本地时间而非 UTC，与方案"统一 UTC 存储"决策矛盾。` +
        `请将 kingbase.conf 中 timezone 参数设置为 'UTC' 并重启服务。`
      );
    }
  } catch (err) {
    // 时区验证失败不应阻塞启动——可能是权限不足或版本差异
...
**设计要点**：
- **不阻塞启动**：时区验证失败（如查询权限不足、KingbaseES 版本不支持 `current_setting('timezone')`）时仅输出警告日志，不抛出异常。时区配置错误是运维问题而非代码缺陷——应用层不应因此拒绝启动（运维人员可能在启动后再调整时区配置并重启）
- **警告级别**：非 UTC 时区使用 `console.error`（高可见性），验证失败使用 `console.warn`（低严重性）
- **UTC 等价表示**：除 `'UTC'` 外，`'Etc/UTC'` 也视为合法 UTC 表示（部分 Linux 发行版的 PostgreSQL 包默认使用此值）
- **日志输出**：无论时区是否正确，均输出当前时区值到日志（正常级别），便于运维排查时区相关问题

**与 4.2 节的关系**：4.2 节"KingbaseES 服务器时区配置要求"规定了服务器端的强制配置要求，本节的启动验证是应用层的防御性检查——即使服务器配置遗漏，也能在应用启动日志中及时发现并告警。

### 3.5 database.js 改造

现有 `server/db/database.js` 改造为：

> **注意（v15 新增）**：以下为 Phase 0 完成后的最终导出状态（仅导出 `getAdapter()`）。Phase 0 过渡期间的导出状态（同时导出 `db` 和 `getAdapter()`）见 3.5.2 节"双导出过渡"方案。
...
**`db` 导出消费变化确认（v10 新增）**：当前 `server.js`（第 2 行）解构导入了 `{ initDatabase, db }`，但 `db` 未被 `server.js` 自身使用（`server.js` 不执行数据库查询，数据库访问全部在路由文件中）。改造后的解构 `{ initDatabase, getAdapter }` 删除 `db` 不会产生未使用变量错误，改造安全。

**优雅关闭（v11 新增）**：应用需在接收到 SIGTERM/SIGINT 信号时执行优雅关闭流程——先调用 `adapter.close()` 正常释放数据库连接池，再退出进程。缺少此机制将导致连接池暴力断开、进行中事务中断。

```javascript
// server.js 改造后（在 IIFE 的 app.listen() 之后追加）
// 优雅关闭处理
...
**关闭流程时序**：`SIGTERM`/`SIGINT` → `adapter.close()` → `pool.end()`（等待进行中查询完成 → 断开所有连接）→ `process.exit(0)`。SqliteAdapter 的 `close()` 调用 `db.close()`（better-sqlite3 的同步关闭），行为等价。
...
| 4 | **逐文件改造路由层**：每改造完一个路由文件（将其中的 `db.prepare` 改为 `adapter.query/queryOne/execute`，handler 改为 async），立即重启应用并自测该文件对应的 API 端点。此时数据库访问是混合模式——部分路由走 adapter（通过 `getAdapter()`），部分路由仍走旧 `db`——两者操作同一个 SQLite 文件，不存在数据一致性风险。各路由文件之间无数据库访问的模块间依赖，逐文件改造不会产生跨文件适配不一致。 | `db` + `getAdapter()` | 逐步迁移中 | 是（每个文件改造后可立即验证） |
...
所有路由文件（13 个文件：12 个需改造 + 1 个不变，其中 11 个涉及数据库访问）的改动模式一致：将 `db.prepare(sql).run/get/all()` 替换为 `adapter.query/queryOne/execute()`。
...
`sql.js`（`server/db/sql.js`，本方案规划新建的方言辅助模块文件，其定义见第 4 节）方言辅助函数表中不包含 `sql.insertId()`——ID 获取是 adapter 层的职责，不属于方言差异范畴。
...
**不变的文件**：`server/routes/upload.js` 不涉及数据库访问，无需修改。

> **关于 `index.js` 的说明（v16 修正）**：`server/routes/index.js` 不在"不变的文件"之列——其 `/health` 端点需改造以调用 `adapter.healthCheck()`（详见第 13.2 节），已在上方 async 改造清单中列出。此处将其从"不变的文件"段落中移出，避免读者误以为 index.js 无需改造。
...
| 自增主键语义差异（v11 新增） | `AUTOINCREMENT` 保证 ID 不重复使用且严格单调递增（即使行被删除，旧 ID 永久废弃） | `SERIAL` 基于 SEQUENCE 实现，不具备 no-reuse 保证（`ROLLBACK` 后序列不回退，但 `DELETE` 后序列继续递增——单调递增行为与 AUTOINCREMENT 相似；差异在于 SERIAL 不保证已被删除/回滚的 ID 永不再出现） | 见下方详细说明 |
...
**v11 补充 —— `AUTOINCREMENT` 与 `SERIAL` 语义差异详细说明**：

SQLite 的 `INTEGER PRIMARY KEY AUTOINCREMENT` 与 PostgreSQL 的 `SERIAL` 之间除语法差异外，还存在以下语义差异：

| 维度 | SQLite AUTOINCREMENT | PostgreSQL/KingbaseES SERIAL |
|------|---------------------|------------------------------|
| ID 重用策略 | **绝不重用**：已分配过的最大 rowid 即使对应的行被 DELETE 也不会再分配。内部维护 `sqlite_sequence` 表记录各表最大 rowid | **可能重用**：`nextval()` 返回的值在 `ROLLBACK` 后不回退——序列值一旦被消费即永久跳过。`DELETE` 后序列继续递增。正常操作下与 AUTOINCREMENT 行为类似（单调递增、不重用），但 `pg_restore`/备份恢复等运维操作可能重置序列值 |
| 严格单调递增 | 是——在无显式 `INSERT ... VALUES(id, ...)` 指定较小 id 值的前提下 | 是——除非显式指定 id 值小于当前序列值，或通过 `setval()` 手动重置序列 |
| 回滚行为 | ROLLBACK 后 rowid 不回退（该 ID 永久废弃） | ROLLBACK 后 `nextval()` 已消费的值不回退（与 AUTOINCREMENT 一致） |

**关键相似点（在正常应用使用模式下）**：本项目所有 10 张表的 `id` 列均由数据库自动生成（应用代码从不显式指定 id 值），且序列仅在 `DELETE`+重新 `INSERT` 时继续递增（SERIAL 与 AUTOINCREMENT 行为一致）。`ROLLBACK` 后序列不回退的行为在两个数据库中也一致。因此：

1. **按 ID 排序推断时间顺序**：两个数据库下均成立（ID 严格单调递增）
2. **假定无间隙的分页逻辑**：两个数据库下均不成立（ROLLBACK 产生的间隙在两种数据库中均存在）。若应用代码有此隐式假设，在两个数据库中均有 bug，非迁移引入
3. **结论**：本项目应用代码对 AUTOINCREMENT 的 no-reuse 语义无隐式依赖。`AUTOINCREMENT → SERIAL` 的翻译在业务语义上是安全的

**验证建议**：在 Phase 1 双库对比测试中，对每张表执行：
```sql
-- 验证序列递增行为（SQLite 和 KingbaseES 分别执行）
INSERT INTO _test_seq (name) VALUES ('test');  -- 记录返回的 ID
DELETE FROM _test_seq WHERE id = <刚插入的ID>;
INSERT INTO _test_seq (name) VALUES ('test2');  -- 验证新 ID > 旧 ID
```
确认两个数据库下新 ID 均大于旧 ID（均在 DELETE 后单调递增）。此验证确认应用代码对 ID 排序的任何隐式依赖在两个数据库下均成立。
...
`sql.formatDateParam()` 的实现逻辑（约 8 行，v16 补充完整代码轮廓）：使用 **UTC 方法** `Date.getUTCFullYear()`、`getUTCMonth()`、`getUTCDate()`、`getUTCHours()`、`getUTCMinutes()`、`getUTCSeconds()` 拼接 `YYYY-MM-DD HH:MM:SS` 字符串，各分量不足两位时左侧补零。

```javascript
// sql.formatDateParam() 实现轮廓（sql.js 模块内）
...
**必须使用 UTC 方法而非本地时间方法**——方案决策 `CURRENT_TIMESTAMP` 统一输出 UTC 时间，若使用 `getHours()` 等本地时间方法，在 UTC+8 时区下格式化输出的字符串比数据库存储的 `CURRENT_TIMESTAMP` 值大 8 小时，导致日期范围查询（如 punch.js 近 7 天打卡）的边界比较结果错误。此方式无需方言函数，两个数据库行为完全一致，统一输出 UTC 格式字符串，消除本地时区依赖。此方案作为首选推荐，`sql.relativeDate()` 不做为必须实现的接口。
...
**KingbaseES 服务器时区配置要求（v13 新增）**：`sql.now()` 统一输出 `CURRENT_TIMESTAMP` 的决策建立在"SQLite 的 `CURRENT_TIMESTAMP` 返回 UTC、KingbaseES 的 `CURRENT_TIMESTAMP` 也返回 UTC"这一隐式假设之上。然而，PostgreSQL/KingbaseES 的 `CURRENT_TIMESTAMP` 实际返回的是**服务器时区（`timezone` 参数）下的当前事务开始时间**——若 KingbaseES 服务器时区配置为 `Asia/Shanghai`（UTC+8，国产数据库在中国部署场景下的常见默认值），`CURRENT_TIMESTAMP` 将返回 UTC+8 时间，与 SQLite 的 `CURRENT_TIMESTAMP`（始终返回 UTC）相差 8 小时。

此遗漏直接影响以下核心路径：
1. **Phase 1 KingbaseES 下新写入的 timestamp 值**：若服务器非 UTC 时区，新数据将以本地时间存储而非 UTC——与方案"统一 UTC 存储"的基本承诺直接矛盾。
2. **Phase 2 迁移后数据混合时间戳**：迁移脚本将 SQLite 本地时间减去 8 小时转换为 UTC 后写入 KingbaseES。但若 KingbaseES 新数据又通过 `CURRENT_TIMESTAMP` 写入了服务器时区的本地时间，则迁移后数据库内同时存在 UTC（迁移来的旧数据）和本地时间（`CURRENT_TIMESTAMP` 新写入的数据）——与方案试图消除的"混合时间戳"问题性质相同，且更难排查（两个来源使用相同的 `CURRENT_TIMESTAMP` 表达式但产生不同时区的时间值）。
3. **`sql.formatDateParam()` 的 UTC 方法输出**：该函数使用 `getUTC*` 方法输出 UTC 字符串，假设数据库端也是 UTC 存储。若 KingbaseES 服务器时区非 UTC，应用层传入的 UTC 参数字符串与数据库列中存储的本地时间字符串在字符串比较时产生 8 小时偏差——与方案第 4.2 节讨论的 `.toISOString()` 不兼容问题同源。

**强制要求**：KingbaseES 服务器 `timezone` 参数**必须**设置为 `UTC`。配置方式：

- **KingbaseES 配置文件（`kingbase.conf` 或 `postgresql.conf`）**：
  ```
  timezone = 'UTC'
  ```
  修改后需重启 KingbaseES 服务或执行 `SELECT pg_reload_conf()` 使配置生效。

- **Docker 容器部署**：通过环境变量设置容器时区：
  ```bash
  docker run -d -p 54321:54321 -e TZ=UTC kingbase/kingbasees:v8r6
  ```
  注意：`TZ=UTC` 设置的是容器操作系统时区，KingbaseES 的 `timezone` 参数默认继承自操作系统时区。若 Docker 镜像中 `kingbase.conf` 已硬编码了非 UTC 时区值，需通过挂载自定义配置文件覆盖。

- **连接字符串携带时区参数**（辅助手段，不替代服务器端配置）：在 `DATABASE_URL` 的 `options` 参数中指定会话时区：
  ```
  DATABASE_URL=postgresql://system:password@localhost:54321/diabetes_db?options=-c%20timezone%3DUTC
  ```
  此方式仅在当前会话生效，不能替代服务器端全局配置。若连接池中的其他连接未携带此参数，仍会使用服务器默认时区。

- **验证方法**：连接 KingbaseES 后执行 `SHOW timezone;` 或 `SELECT current_setting('timezone');`，确认返回 `'UTC'`（或 `'Etc/UTC'` 等 UTC 等价表示）。

**KingbaseAdapter 启动时区验证**：KingbaseAdapter 初始化后应执行时区验证查询，若非 UTC 则输出警告日志（见 3.4.10 节），提醒运维人员调整服务器时区配置。此验证是最后一道防线——即使服务器时区配置遗漏，也能在应用启动时及时发现并告警。
...
### 4.2.1 `server/utils/dateRange.js` 工具模块兼容性评估（v11 新增，v16 修订）

`punch.js`（第 91、102 行）依赖 `server/utils/dateRange.js` 的 `parseDateRange()` 函数生成日期查询参数。该函数输出的 `endDate` 格式为 `YYYY-MM-DDTHH:MM:SS`（含 `T` 分隔符，如 `"2025-06-28T23:59:59"`），在 KingbaseES 中需隐式将 TEXT 转换为 TIMESTAMP 才能与 TIMESTAMP 类型列（如 `punch_time`）进行字符串比较。

**结论：`dateRange.js` 无需改造，`T` 分隔符格式在 KingbaseES V8R6 中预期兼容。**

验证依据：
1. PostgreSQL 12（KingbaseES V8R6 内核）的隐式类型转换规则：当 TEXT 值与 TIMESTAMP 类型列进行比较时，数据库自动将 TEXT 尝试解析为 TIMESTAMP。ISO 8601 格式的日期时间字符串（含 `T` 分隔符）是 PostgreSQL TIMESTAMP 解析的标准输入格式之一
2. SQLite 同样能正确解析含 `T` 分隔符的日期字符串——SQLite 的日期时间函数接受 ISO 8601 格式输入

**待执行验证（v16 修订）**：以下验证 SQL 应在 Phase 1 前置验证脚本（`scripts/verify-pg-kingbase.js`）中作为额外验证项执行，实际确认 KingbaseES V8R6 实例对 `T` 分隔符日期字符串的隐式类型转换行为：

```sql
-- 待执行：在 KingbaseES V8R6 测试实例上验证
SELECT '2025-06-28T23:59:59'::TIMESTAMP;  -- 预期成功返回 2025-06-28 23:59:59
SELECT * FROM punch_in WHERE punch_time <= '2025-06-28T23:59:59' LIMIT 1;  -- 预期隐式转换正确
```

**v16 修订说明**：前序版本（v11-v15）将上述 SQL 以"已执行验证"口吻描述（如"应成功返回"、示例 SQL），但实际该验证基于"KingbaseES V8R6 = PostgreSQL 12"的理论假设，`pg` 驱动与 KingbaseES V8R6 的兼容性尚未在目标实例上实际验证（见 Phase 1 前置条件）。v16 修订为"待执行验证"口吻，将此验证加入 Phase 1 前置验证脚本，确保实现者在目标 KingbaseES 实例上实际执行并确认。

**实现者注意事项**：
- `parseDateRange()` 函数本身无需任何改造，其输出的 `YYYY-MM-DDTHH:MM:SS` 格式在两个数据库中均被正确解析
- 若将来 KingbaseES 版本升级后隐式转换行为发生变化，备选方案为在 `parseDateRange()` 中增加 `formatDateParam` 调用，将 `T` 分隔符替换为空格——但当前版本不需要
- Phase 1 双库对比测试中应包含 `GET /api/punch?range=7d` 的时间范围筛选测试，验证两个后端下返回的记录数量和日期范围边界一致

### 4.3 DDL 层面差异

DDL 通过两套独立的初始化脚本管理，不在应用 SQL 中处理：
- `server/db/init.sql` — SQLite DDL（保持现有）
- `server/db/init_kingbase_ddl.sql` + `server/db/init_kingbase_seed.sql` — KingbaseES DDL 和种子数据（v14 拆分，见第 10 节）
...
services:
  kingbase:
    ...
    env:
      ...
      TZ: UTC  # v13 新增：强制容器时区为 UTC，确保 KingbaseES 的 timezone 参数继承 UTC（见 4.2 节"KingbaseES 服务器时区配置要求"）
```
...
| T15 | `/api/admin/execute` | POST | tool_name 模式执行命名操作 | **v10 明确测试范围**：至少测试以下 6 个核心 tool_name——① `query_user_profile`（传入 `{user_id}`，验证返回 user 对象含 `username`/`role`/`avatar` 字段）；② `query_risk_history`（传入 `{user_id}`，验证返回数组含 `risk_score`/`risk_level`/`created_at`）；③ `query_punch_records`（传入 `{user_id}`，验证返回数组含 `punch_type`/`punch_time`/`completion_status`）；④ `query_life_plans`（传入 `{user_id}`，验证返回数组含 `plan_type`/`title`/`is_active`）；⑤ `query_table`（传入 `{table: "users", where: "1=1", page: 1, page_size: 5}`，验证返回 `data` 数组和 `pagination` 对象）；⑥ `get_table_schema`（传入 `{table: "users"}`，验证返回含 `cid`/`name`/`type`/`pk` 字段的数组）。每个 tool_name 验证 HTTP 200 状态码和 `success: true`。**v16 新增安全测试**：另传入 `{ tool_name: "query_table", params: { table: "users", where: "1=1; DROP TABLE users;--", page: 1, page_size: 5 } }`，验证服务端返回 HTTP 400（WHERE 子句安全校验拒绝注入尝试）而非执行恶意 SQL |
| T16 | `/api/health` | GET | 健康检查 | 返回 `{ success: true, status: "ok", database: "connected" }` |
| T17 | `/api/assistant` | POST | 智能助手对话（SSE 流式） | **v10 明确**：POST `/api/assistant/chat` 传入 `{ message: "你好" }`，验证——① HTTP 200 状态码；② Content-Type 为 `text/event-stream`；③ 流中包含 `data:` 前缀的 SSE 事件；④ 流正常结束（非连接中断）。不验证具体 AI 回复内容（依赖 Dify 工作流）。 |
| T17a | `/api/assistant` | GET | 获取助手对话历史 | **v10 新增**：GET `/api/assistant/conversations`，验证返回数组（可为空数组），每项含 `conversation_id`/`name`/`created_at` 字段。 |
...
- **关键行为变更**：时间戳存储从本地时间（`datetime('now','localtime')`）切换到 UTC（`CURRENT_TIMESTAMP`）。新写入数据的 timestamp 字段值与原行为相差 8 小时。前端需负责时区转换显示——在时间渲染层将 UTC 字符串转换为用户本地时区后展示（详见第 14 节"时间显示层时区转换"和前端文件变更清单 `src/utils/timezone.js` 新建及 Vue 组件改造）。
...
- **验收标准**：
  ...
  4. 管理员日志记录和查询正常
  5. 科普文章 CRUD 正常
  6. 前端页面中显示的时间正确（数据库存储为 UTC，前端展示应通过 `convertUTCToLocal()` 转换为本地时间）。验证要点：(a) `src/utils/timezone.js` 已新建且导出 `convertUTCToLocal` 函数；(b) 所有展示 datetime 字段的 Vue 组件已引入并调用该函数；(c) 页面渲染的时间字符串为用户本地时区（如北京时间）而非 UTC 原值
  7. 无功能回归
...
- **前置条件（v14 新增）——`pg` 驱动与 KingbaseES V8R6 兼容性验证**：Phase 1 实现开始前，必须在目标 KingbaseES V8R6 实例上验证 `pg` v8.12 的以下核心功能，验证通过后方可进入 KingbaseAdapter 实现阶段：
  1. **TCP 连接与认证**：使用 `DATABASE_URL` 连接 KingbaseES V8R6，验证 `new pg.Pool({ connectionString }).query('SELECT 1')` 成功
  2. **参数化查询**：验证 `$1, $2, ...` 占位符的查询正确返回结果
  3. **事务（BEGIN/COMMIT/ROLLBACK）**：验证 `client.query('BEGIN')` → `client.query('INSERT ...')` → `client.query('COMMIT')` 流程完整
  4. **连接池**：验证 `pg.Pool` 的 `pool.connect()` → `client.release()` 生命周期，确认连接正常归还复用
  5. **数据类型映射**：验证 TIMESTAMP、JSONB、JSON、INTEGER、TEXT、BOOLEAN 列的往返读写正确（写入值 → 读取值 → 类型和内容一致）
  6. **`pg.types.setTypeParser` 对 TIMESTAMP/TIMESTAMPTZ/JSONB/JSON 的拦截**：验证第 3.4.8 节的解析器配置在目标实例上生效——timestamp 列返回字符串而非 Date 对象，JSONB/JSON 列返回字符串而非 JS 对象
  7. **`T` 分隔符日期字符串隐式类型转换（v16 新增）**：验证 `SELECT '2025-06-28T23:59:59'::TIMESTAMP` 及 `WHERE punch_time <= '...T...'` 的隐式转换正确性（见 4.2.1 节待执行验证）
  - 验证方法：编写 `scripts/verify-pg-kingbase.js` 脚本，逐项执行上述 7 个验证用例，全部通过后输出 `[PASS] pg v8.12 + KingbaseES V8R6 兼容性验证通过`。任何一项失败则输出 `[FAIL]` 及详细错误信息
  - 此验证必须在 KingbaseAdapter 实现之前完成——方案中多处技术判断建立在"KingbaseES V8R6 行为等同于 PostgreSQL 12"的假设上，但国产化分支可能有特定的协议变更或行为差异
- 实现 `KingbaseAdapter`（含 `init()` 的拆分 DDL/种子文件执行、密码哈希占位符替换机制、`?` → `$1` 参数占位符转换、SSL/TLS 配置、`pool.on('error')` 事件处理）
- 对齐 `init_kingbase_ddl.sql` + `init_kingbase_seed.sql` 与 `init.sql` 的 schema 差异（见第 10 节），并将硬编码的 bcrypt 哈希替换为 `__BCRYPT_HASH_PLACEHOLDER__` 占位符。**两个 KingbaseES SQL 文件必须使用 `CREATE TABLE IF NOT EXISTS` 而非 `DROP TABLE IF EXISTS`**（见第 10 节）
- 本地或测试环境部署 KingbaseES 实例（推荐 Docker：`docker run -d -p 54321:54321 -e TZ=UTC kingbase/kingbasees:v8r6`，**镜像名称待验证**，见 5.1 节 KingbaseES Docker 镜像可用性说明；`-e TZ=UTC` 确保容器时区为 UTC，见 4.2 节"KingbaseES 服务器时区配置要求"）
...
- **验收标准**：
  ...
  4. 连接池正常工作，连接失败后自动恢复
  5. **（v3 新增，v14 补全）性能基准对比**：
     - **测试数据量**：SQLite 中预填充至少 1,000 条 users 记录 + 对应每用户 10 条 punch_in 记录 + 5 条 life_plans 记录（总计约 16,000 行），模拟中等负载数据量
     - **并发模型**：使用压测工具（如 autocannon）以 **10 并发连接** 持续 30 秒向每个关键端点发送请求，记录响应时间分布
     - **关键端点及绝对时间上限**：
       | 端点 | 方法 | SQLite P95 基线 | KingbaseES P95 上限 | 说明 |
       |------|------|----------------|-------------------|------|
       | `/api/plan/generate` | POST | 实测 | **< 2000ms** | 含 Dify API 调用（主导耗时），数据库部分应 < 500ms |
       | `/api/plan/adjust` | PUT | 实测 | **< 2000ms** | 同上 |
       | `/api/risk/history` | GET | 实测 | **< 200ms** | 纯数据库查询 |
       | `/api/admin/logs` | GET | 实测 | **< 200ms** | 纯数据库查询 |
       | `/api/punch` | GET | 实测 | **< 200ms** | 含日期范围筛选 |
       | `/api/health` | GET | 实测 | **< 50ms** | `SELECT 1` 健康检查 |
     - **绝对可接受上限**：任何查询端点的 P95 响应时间不得 > 2000ms（超出此值视为 KingbaseES 部署配置或查询需优化）
     - **相对基准**：KingbaseES 下各端点的 P95 响应时间建议 < 2 倍 SQLite 基准值（绝对上限优先于相对基准——若 SQLite P95 已为 1500ms，KingbaseES 不超过 2000ms 绝对上限即可接受，不强制要求 < 3000ms 的相对宽松值）
     - **性能测试脚本**：编写 `scripts/perf-benchmark.sh` 脚本自动化执行上述测试并生成性能对比报告（JSON 格式），供 Phase 1 验收审查
     - **`plan.js` 批量 INSERT**：若使用多行 VALUES（见 8.2 节），KingbaseES 下的响应时间应在绝对上限范围内（< 2000ms）
     - **GIN 索引性能基准对比（v16 新增）**：Phase 1 性能基准对比中应在同一 KingbaseES 实例上有/无 GIN 索引分别测试 `user_risk_info` 表的写入性能（INSERT `user_risk_info` + `result` JSONB 列）和查询性能（按 `risk_level` 筛选），对比写入延迟和查询延迟差异。根据实测数据决定是否创建 GIN 索引（见 10.2 节 GIN 索引写入性能说明）
...
- 生产环境部署 KingbaseES（V8R6 或更高），执行 `init_kingbase_ddl.sql` + `init_kingbase_seed.sql` 初始化
...
- **SQLite 数据库文件生命周期管理（v14 新增）**：
  1. **切换后立即设为只读**：Phase 2 切换至 KingbaseES 且冒烟验证通过后，将 SQLite 数据库文件（`database.sqlite`）及其 WAL/SHM 文件的操作系统权限设为只读（如 `chmod 444 database.sqlite`），防止任何进程意外写入。此操作防止以下风险：(a) 运维人员误用 SQLite 路径的旧脚本修改了数据；(b) 应用若因配置错误以 `DB_TYPE=sqlite` 启动，不会污染已迁移的 SQLite 数据
  2. **回退前恢复可写**：若需回退到 SQLite，执行回退前先将文件权限恢复为可写（`chmod 644 database.sqlite`）
  3. **回退验证最小范围**：回退到 SQLite 后，至少验证以下 6 个核心端点：`POST /api/auth/login`、`GET /api/user/profile`、`GET /api/risk/history`、`GET /api/plan/current`、`GET /api/punch`、`GET /api/health`。验证通过即确认 SQLite 路径功能正常
- **回滚步骤（v2 明确）**：
  1. 恢复 SQLite 文件为可写（若已设为只读）
  2. 将 `DB_TYPE` 改回 `sqlite`
  3. 重启应用
  4. 验证功能正常（执行上述 6 个核心端点的回退验证）
  5. 注意：KingbaseES 期间产生的新数据不会自动同步回 SQLite。如需数据回退，需反向迁移脚本（KingbaseES → SQLite）
- **验收标准**：
...
- **SQLite 文件归档策略（v14 新增）**：Phase 3 移除 SQLite 支持时，SQLite 数据库文件不直接删除，应按以下策略归档保留：
  1. 将 `database.sqlite`（含 WAL/SHM 文件）压缩打包（如 `database.sqlite.2026-XX-XX.tar.gz`），归档到安全存储位置（如对象存储、备份服务器）
  2. 归档文件保留期 >= 6 个月（覆盖至少一个完整的合规审计周期）
  3. 归档目的：(a) 满足数据合规审计要求——迁移前的原始 SQLite 数据可能作为审计证据；(b) 灾难恢复的最后备份——若 KingbaseES 出现严重数据损坏且备份不可用，归档的 SQLite 数据可作为最后的数据恢复源
  4. 归档完成后，从生产服务器删除 SQLite 原文件以释放磁盘空间
...
#### 7.2.3 连接池大小确定方法（v11 新增）

**推荐简化公式**：

```
pool_max = (并发请求数 × 平均数据库查询耗时) / 1000 + 缓冲

其中：
- 并发请求数：预期的峰值并发 HTTP 请求数（中小型应用建议取值 20-50）
- 平均数据库查询耗时：单次查询的 P95 响应时间（ms），Phase 1 性能基准对比中实测
- 缓冲：2-5 个额外连接，用于处理瞬时突发流量和事务内连接（transaction 会持有连接更长时间）
```

**示例计算**（中小型应用）：
```
pool_max = (30 × 50ms) / 1000 + 3 = 1.5 + 3 ≈ 5 → 保守取 10
```

**不确定时的安全默认值**：在缺乏实际并发数据的情况下，`max=10` 是中小型 Node.js 应用的合理默认值。此值来源于：
1. Node.js 单线程事件循环模型——同一时刻真正在执行数据库查询的请求数受事件循环串行化限制，实际并发远小于 HTTP 并发数
2. `pg.Pool` 的连接复用机制——查询完成后连接立即归还池中供后续请求使用
3. 行业经验——中等规模的 Express + PostgreSQL 应用通常在 `max=5-20` 范围内运行良好

**精确确定方法**（Phase 1 性能基准对比中执行）：

1. 在 KingbaseES 测试实例上，使用压测工具（如 autocannon、k6、Artillery）模拟预期峰值负载
2. 在压测过程中监控 `pool.totalCount`、`pool.idleCount`、`pool.waitingCount` 三个指标
3. 若 `waitingCount` 持续 > 0（请求在等待连接），说明 `max` 不足，需增加
4. 若 `idleCount` 持续接近 `max`（大量空闲连接），说明 `max` 过大，可降低以节省数据库资源
5. 记录压测数据写入 Phase 1 验收报告中，作为生产环境 `DB_POOL_MAX` 的定价依据

**连接池指标监控集成**：在 `KingbaseAdapter.healthCheck()` 或独立监控端点中暴露连接池指标（`pool.totalCount`、`pool.idleCount`、`pool.waitingCount`），供运维监控系统采集。此集成在 Phase 1 中实现。

**事务内连接的特殊考虑**：`adapter.transaction()` 在 KingbaseES 下从池中获取专属 client 并持有至事务结束（含 Dify 网络调用的整个周期）。对于 `plan.js` 的方案生成事务（耗时可能数秒，Dify API 调用在事务内），该连接在事务期间不被其他请求共享。因此 `max` 需额外考虑同时进行中的长事务数量。若长事务并发量高且 `max` 不足，事务请求将在 `pool.connect()` 处排队等待可用连接。当前项目方案生成有内存幂等锁限制（同一用户 30 秒内只能一次），长事务并发量低，`max=10` 仍足够。
...
**init.sql 和 init_kingbase.sql 同步变更（v10 修正）**：在第 10.2 节的 DDL 对齐中，`life_plans` 表需新增 `UNIQUE(user_id, plan_id)` 约束。`init.sql` 第 145 行已有普通索引 `CREATE INDEX IF NOT EXISTS idx_plans_user_plan ON life_plans(user_id, plan_id)`，需改为 `CREATE UNIQUE INDEX IF NOT EXISTS idx_plans_user_plan ON life_plans(user_id, plan_id)`。此变更在第 10.1 节差异分析表中标注。

**注意（v10 新增）**：前序版本将索引名称误写为 `idx_life_plans_user_plan_id`、行号误写为第 138 行（实际为 `article_collections` 表的 `idx_collections_user_article` 索引）。正确索引名为 `idx_plans_user_plan`，位于 `init.sql` 第 145 行。由于原始索引是普通索引而非 UNIQUE，此变更是"将普通索引改为 UNIQUE 索引"（原地修改 `CREATE INDEX` → `CREATE UNIQUE INDEX`），而非"升格已有 UNIQUE 索引"。
...
**Dify 端同步变更（v4 新增，v6 补充操作步骤，v10 修正）**：禁用 `sql` 模式是服务端的单向决策——若 Dify AI 工作流的 system prompt 未同步更新，LLM 可能在 KingbaseES 环境下仍选择 `sql` 模式，导致用户看到"暂不支持"错误而非有意义的操作建议。需在 Dify 工作流侧同步变更：

**1. `db_type` 变量传递路径分析（v10 修正）**：

项目中有两条独立的 Dify 调用路径，需区分对待：

| 路径 | 代码位置 | 函数 | 是否需要 `db_type` | 理由 |
|------|---------|------|-------------------|------|
| **方案生成/风险评估** | `difyService.js` 第 84 行 | `callWorkflowBlocking(apiKey, inputs, workflowType)` | **否** | 该函数通过 `httpRequest()`（项目自定义，`difyService.js` 第 37 行）发送请求，`inputs` 参数（由调用方 plan.js/risk.js 传入）直接透传到 Dify 工作流 API body（第 103 行 `inputs: inputs`）。方案生成和风险评估工作流不涉及数据库动态 SQL 查询，无需 `db_type` 变量。**原方案（v6 补充）中修改 `difyService.js` 增加 `inputs.db_type` 的建议属于过度设计，已撤回。** |
| **admin chat（SSE 流式）** | `sseProxy.js` 第 4 行 | `proxyDifySSE({ apiKey, query, conversationId, userId, res, req })` | **是** | admin chat 工作流需要根据数据库类型切换 `tool_name`/`sql` 模式。`db_type` 变量通过 `sseProxy.js` 第 26 行 `inputs: { db_type: process.env.DB_TYPE || 'sqlite' }` 注入（见上方第 6 点改造方案），函数签名和所有调用方均无需修改。 |

**原方案 v6 伪代码的错误**（v10 新增）：前序版本给出的 `difyService.js` 改造伪代码使用 `axios.post()`，但项目实际代码使用自定义 `httpRequest()` 函数（`difyService.js` 第 37-81 行）；实际函数签名为 `callWorkflowBlocking(apiKey, inputs, workflowType)`（第 84 行），`inputs` 直接透传、`user` 硬编码为 `'api-user'`（第 106 行）。若实现者按文档伪代码实施，代码无法运行。
...
**3. 变更范围**：仅限 **admin chat 工作流**（通过 `sseProxy.js` 的 SSE 代理调用）。其他工作流（方案生成、风险评估等，通过 `difyService.js` 的 `callWorkflowBlocking` 调用）不需要此变更，因为它们不涉及数据库动态 SQL 查询。`difyService.js` 本身无需修改。
...
**5. 第 16 节文件变更清单更新（v10 修正）**：`server/services/sseProxy.js` 需改造（函数体内部 `inputs: {}` → `inputs: { db_type: process.env.DB_TYPE || 'sqlite' }`，不修改函数签名）。`server/services/difyService.js` 无需改造（`callWorkflowBlocking` 路径不需要 `db_type`，v6 的过度设计建议已撤回）。
...
经核实项目实际代码（`server/services/sseProxy.js` 第 4 行），`proxyDifySSE` 的实际函数签名为：

```javascript
// sseProxy.js 实际代码（第 4 行）—— 非 async，所有参数容纳在单个解构对象内
function proxyDifySSE({ apiKey, query, conversationId, userId, res, req }) {
```

函数**非 async**，参数列表中不包含 `baseUrl`/`route`/`user`/`inputs`。前序版本（v9）给出的函数签名改造模板与实际代码存在三处事实错误——(1) 误标为 `async`；(2) 参数解构和额外参数均与实际不匹配；(3) 调用方示例使用 `await` 和不存在的 `difyConfig` 变量。

**正确的最小化方案：不修改函数签名，仅在函数体内部将硬编码 `inputs: {}` 改为动态值**：

```javascript
// sseProxy.js 改造后（仅修改第 26 行，函数签名不变）
const body = {
  query,
  user: `user-${userId}`,
  inputs: { db_type: process.env.DB_TYPE || 'sqlite' },  // ← 唯一改动
  response_mode: 'streaming'
};
```

**此方案的关键优势**：无需改动任何调用方代码。`admin.js`（第 125-131 行）、`assistant.js`（第 20-26 行）、`chat.js`（第 27-33 行）三个调用方均以单对象参数形式调用 `proxyDifySSE({ apiKey, query, conversationId, userId, res, req })`，函数签名不变则所有调用方零改动。

**(b) 其他 `proxyDifySSE` 调用处排查结论（v10 新增，闭环）**：经实际代码检查确认——
- **`assistant.js`（第 20 行）**：调用 `proxyDifySSE` 连接 `DIFY_ASSISTANT_APP_KEY` 工作流（智能助手对话），不涉及数据库动态 SQL 查询操作，**不需要 `db_type` 参数**。`inputs: { db_type: ... }` 的传入对助手工作流无副作用（Jinja2 模板中若未引用 `{{db_type}}` 则静默忽略）。
- **`chat.js`（第 27 行）**：调用 `proxyDifySSE` 连接医生专属 Dify 工作流（医生对话），不涉及数据库动态 SQL 查询操作，**不需要 `db_type` 参数**。同上，统一传入无副作用。
- **结论**：三个调用方均零改动，`db_type` 变量统一由 `sseProxy.js` 内部注入（第 26 行），所有 SSE 流式工作流均可感知数据库类型。原方案（v9）将"排查结论"作为开放任务委托给实现者的做法已撤销。

**(c) 第 16 节文件变更清单更新（v10 修正）**：`server/services/sseProxy.js` 条目修正为"函数体内部 `inputs: {}` → `inputs: { db_type: process.env.DB_TYPE || 'sqlite' }`（不修改函数签名）"；`admin.js` 条目的 `/chat` 路由 `inputs` 传递改造说明已撤回（调用方零改动）；`server/routes/assistant.js` 和 `server/routes/chat.js` 条目无需补充（调用方零改动，已在正文中给出明确排除结论）。

**实现**：在 `admin.js` 的 `/execute` handler 中，`sql` 模式分支前增加判断：

> **说明（v15 新增）**：以下代码片段中的 `error` 函数导入自 `server/utils/response.js`。
...
**`query_table` 安全维度说明（v11 新增，v16 增强）**：`query_table` 操作将 Dify AI 生成的 WHERE/ORDER BY 子句通过字符串插值直接拼接到 SQL 语句中（`WHERE ${params.where}`），虽然 `params.where` 由 Dify AI 生成（非外部用户直接输入），但此模式仍构成 SQL 注入的间接攻击面——若 Dify AI 被恶意 prompt 操纵生成含注入代码的 WHERE 子句，注入代码将绕过参数化查询保护直接在数据库中执行。

**Phase 1 基础防护措施（v16 新增）**：仅依赖日志记录作为缓解不足。Phase 1 应实现以下基础防护：
1. **应用层 WHERE 子句安全校验**：在 `dispatchParameterizedQuery()` 的 `query_table` 分支中，使用项目已依赖的 `node-sql-parser`（v5.4.0）解析 WHERE 子句 AST，检查是否仅包含合法的条件表达式（`column OP value` 组合，允许 AND/OR 逻辑连接）。拒绝含子查询（`SELECT` 嵌套）、DML 语句（`INSERT`/`UPDATE`/`DELETE`/`DROP`）、函数调用（除 `date()`/`NOW()` 等已知安全函数外）的 WHERE 子句——返回 400 "WHERE 子句包含不允许的操作"
2. **Dify system prompt 安全约束**：在 Dify admin chat 工作流的 system prompt 中增加明确约束——"生成 WHERE 子句时仅使用 `column = 'value'`、`column > value`、`AND`/`OR` 连接的条件表达式，禁止使用子查询、函数调用或任何 DML/DDL 语句"
3. **Phase 1 验收安全测试用例（v16 新增）**：在手工回归测试清单 T15（`/api/admin/execute`）中增加安全测试——传入 `{ tool_name: "query_table", params: { table: "users", where: "1=1; DROP TABLE users;--", page: 1, page_size: 5 } }`，验证服务端返回 400 而非执行恶意 SQL

Phase 2+ 进一步增强：(1) 引入 WHERE 子句白名单机制（仅允许特定列和运算符）；(2) 考虑彻底移除 `query_table` 的 WHERE 子句字符串插值，改为结构化查询参数（`{ column, operator, value }` 的数组）。

---

## 10. init_kingbase.sql 与 init.sql 对齐方案

> **旧文件处理（v15 新增）**：自 v14 起，原 `server/db/init_kingbase.sql` 已拆分为 `init_kingbase_ddl.sql` 和 `init_kingbase_seed.sql` 两个独立文件。原 `init_kingbase.sql` 文件应在实现时删除或归档，不再使用。删除条目见第 16 节文件变更清单。
...
| life_plans | 缺少 UNIQUE 约束（**v9 新增，v10 修正**） | 仅有普通索引 `idx_plans_user_plan`（`init.sql` 第 145 行），无 `UNIQUE(user_id, plan_id)` 约束 | 无 | 在两个文件中将普通索引改为 UNIQUE 索引：`CREATE UNIQUE INDEX IF NOT EXISTS idx_plans_user_plan ON life_plans(user_id, plan_id)`（第 145 行原为 `CREATE INDEX IF NOT EXISTS`，改为 `CREATE UNIQUE INDEX IF NOT EXISTS`）。此为 `FOR UPDATE` 方案的配套防线——当首次方案生成场景下 FOR UPDATE 空结果集不获取行级锁时，UNIQUE 约束在数据库层阻止重复 plan_id 的 INSERT（见 8.5 节 v9 补充）。**v10 修正**：前序版本将索引名称误写为 `idx_life_plans_user_plan_id`、行号误写为第 138 行（该行实际为 `article_collections` 表的 `idx_collections_user_article` UNIQUE 索引）。正确索引名为 `idx_plans_user_plan`，位于第 145 行。 |
| 全部表 | 索引缺失（v10 修正） | `init.sql` 定义 18 个索引（2 个 UNIQUE INDEX + 16 个普通 INDEX，行 134-151）。**v9/v10 更新**：`life_plans` 表的 `idx_plans_user_plan`（第 145 行，原为普通索引）改为 UNIQUE 索引，UNIQUE INDEX 计数从 2 个增至 3 个、普通 INDEX 从 16 个减至 15 个。**v10 修正**：前序版本将索引名称误写为 `idx_life_plans_user_plan_id`，正确名称为 `idx_plans_user_plan` | `init_kingbase.sql` 无任何索引定义 | 重写时补充全部 18 个索引（PostgreSQL/KingbaseES 兼容语法），其中 `life_plans` 表的对应索引为 UNIQUE 索引 |
| 全部表 | JSON 列类型未决策（**v3 新增，v16 修正**） | `articles.tags`、`user_risk_info.result`、`user_risk_info.raw_input`、`life_advice.tags` 共 4 个 JSON 文本列，在 SQLite 中使用 TEXT 存储 | KingbaseES 中可用 TEXT 或 JSONB。使用 TEXT 需运行时 `::jsonb` 转换；使用 JSONB 查询更快、支持 GIN 索引，但需在迁移中验证 JSON 合法性 | **决策：生产环境使用 JSONB**（见 10.2 节）。**v16 修正**：`admin_logs.operation_content` 和 `admin_logs.operation_result` 经代码核实（admin.js insertAdminLog 函数第 147-156 行调用方传入值）确认存储纯文本（SQL 文本、操作描述如"成功"/"权限不足"），非 JSON 数据。此二列从 JSONB 推荐列表移除，保持 TEXT/VARCHAR |
...
**JSON 列类型决策（v3 新增，v16 修正）**：经代码核实，实际存储 JSON 数据的列仅 4 个——`articles.tags`（articles.js 使用 `JSON.parse` 解析）、`user_risk_info.result`（risk.js 存储 `risk_score`/`risk_level` 等评估结果 JSON）、`user_risk_info.raw_input`（风险评估原始输入 JSON）、`life_advice.tags`（assistant.js 使用 `JSON.parse` 解析）。这 4 个 JSON 文本列在 KingbaseES 中使用 **JSONB 类型**而非 TEXT：
...
**v16 重要修正**：`admin_logs.operation_content` 和 `admin_logs.operation_result` 经代码核实（admin.js 的 `insertAdminLog` 函数第 147-156 行，调用方传入值）确认——`operation_content` 存储纯文本操作描述（如 SQL 语句文本 `"SELECT * FROM users..."`、工具名称描述），`operation_result` 存储纯文本结果（如 `"成功"`、`"权限不足"`）。此二列不存储 JSON 数据，**保持 TEXT/VARCHAR 类型，不使用 JSONB**。若错误改为 JSONB，INSERT 传入纯文本（非合法 JSON）将抛出 `invalid input syntax for type json` 错误（SQLSTATE 22P02），导致所有管理员操作日志写入失败。

**推荐 JSONB**。具体：
- `init_kingbase_ddl.sql` 中将这 4 列的列类型从 TEXT 改为 JSONB
- 迁移脚本中增加 JSON 合法性校验步骤（见 12 节第 8 条）
- 对高频 JSON 查询列（`user_risk_info.result`）建立 GIN 索引以加速查询（`admin_logs.operation_content` 不创建 GIN 索引——该列不存储 JSON 数据，无需 JSON 索引）

**翻译规则**：

| SQLite | KingbaseES |
|--------|-----------|
| `INTEGER PRIMARY KEY AUTOINCREMENT` | `SERIAL PRIMARY KEY`（v11 补充：语义差异分析见 4.1 节"v11 补充"，正常应用使用场景下两者行为等价。若将来新增表对 no-reuse 语义有硬性要求，需使用 `IDENTITY` 列 + `ALWAYS` 生成策略） |
...
| `TEXT`（存储 JSON 字符串） | `JSONB`（4 个 JSON 文本列：`articles.tags`、`user_risk_info.result`、`user_risk_info.raw_input`、`life_advice.tags`）。DDL 中 `DEFAULT NULL` 或 `DEFAULT ''` 保持为 `DEFAULT NULL`（JSONB 不接受空字符串作为默认值，原 `DEFAULT ''` 需改为 `DEFAULT NULL`）。**v16 修正**：`admin_logs.operation_content` 和 `admin_logs.operation_result` 不在此列——此二列存储纯文本，保持 TEXT/VARCHAR |
...
**GIN 索引 DDL 示例（v7 新增，v16 修正）**：对高频 JSON 查询列建立 GIN 索引以加速 `->>` / `@>` 查询。以下为 `user_risk_info.result` 的 GIN 索引 DDL：
...
-- 可选：其余 3 个 JSONB 列的 GIN 索引（根据实际查询频率决定是否创建）
-- articles.tags、user_risk_info.raw_input、life_advice.tags
```

**v16 修正**：删除了 `admin_logs.operation_content` 的 GIN 索引建议——此列经代码核实不存储 JSON 数据（见 10.2 节"JSON 列类型决策"），对该列创建 GIN 索引无效且浪费存储和写入性能。

GIN 索引适用于 `@>`（包含）、`?`（键存在）、`?|`（任意键存在）等 JSONB 运算符。对于本项目最常见的 `->>` 文本提取查询，GIN 索引通过 `USING GIN (col jsonb_path_ops)` 可进一步优化（仅支持 `@>` 但索引更小更快）。

**GIN 索引写入性能说明（v16 新增）**：GIN 索引会显著增加 JSONB 列上 INSERT/UPDATE 的写入开销——每次写入时需更新 GIN 索引的倒排列表。在写入密集型场景下（如每插入一条 `user_risk_info` 记录需同时更新 `result` 列的 GIN 索引），写入延迟可能增加 20-50%。**推荐策略**：Phase 1 暂不创建 GIN 索引，在 Phase 1 性能基准对比中有/无 GIN 索引分别测试写入和查询性能差异，根据实测数据决定是否创建。若查询频率远高于写入频率（本项目的 `user_risk_info.result` 查询场景），创建 GIN 索引的收益通常大于写入开销。

**关键表完整 DDL 参考示例（v14 新增）**：

以下提供 4 张关键表（users、life_plans、punch_in、user_risk_info）的完整 KingbaseES DDL 示例，作为 `init_kingbase.sql` 重写的参考基准。实现者可将以下 DDL 直接用作 `init_kingbase_ddl.sql` 的对应部分（其他 6 张表参照相同的翻译规则从 `init.sql` 翻译）：

```sql
-- ========================================
-- users 表（KingbaseES DDL）
-- ========================================
...
-- ========================================
-- life_plans 表（KingbaseES DDL）
-- ========================================
CREATE TABLE IF NOT EXISTS life_plans (
  id SERIAL PRIMARY KEY,
  user_id INTEGER NOT NULL,
  plan_id INTEGER NOT NULL,
  plan_type VARCHAR(20) NOT NULL CHECK (plan_type IN ('diet', 'exercise', 'other')),
...
-- ========================================
-- punch_in 表（KingbaseES DDL）
-- ========================================
CREATE TABLE IF NOT EXISTS punch_in (
  id SERIAL PRIMARY KEY,
  user_id INTEGER NOT NULL,
  plan_item_id INTEGER DEFAULT NULL,
  punch_type VARCHAR(20) NOT NULL CHECK (punch_type IN ('diet', 'exercise')),
  punch_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  completion_status VARCHAR(20) NOT NULL DEFAULT 'uncompleted' CHECK (completion_status IN ('completed', 'uncompleted')),
...
-- ========================================
-- user_risk_info 表（KingbaseES DDL）
-- ========================================
CREATE TABLE IF NOT EXISTS user_risk_info (
  ...
  diabetes_history VARCHAR(50),
  diabetes_type VARCHAR(50),
  result JSONB DEFAULT NULL,
  raw_input JSONB DEFAULT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
```

**DDL 实现清单（可逐项勾选，v14 新增）**：以下将 10.1 节差异分析表中的 22 项差异转化为可逐项勾选的实现清单，实现者对照此清单重写 `init_kingbase.sql` 时可逐项确认：

| # | 表 | 差异项 | 翻译操作 | 勾选 |
|---|-----|--------|---------|------|
| 1 | users | 补充 `password_changed` 列 | 新增 `password_changed INTEGER NOT NULL DEFAULT 0` | [ ] |
| 2 | users | 补充 `created_at` 列 | 新增 `created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP` | [ ] |
| 3 | users | 补充 `updated_at` 列 | 新增 `updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP` | [ ] |
| 4 | articles | 补充 `user_id` 列 | 新增 `user_id INTEGER NOT NULL` + FK | [ ] |
| 5 | articles | 补充 `tags` 列（JSONB） | 新增 `tags JSONB DEFAULT NULL` | [ ] |
| 6 | articles | 补充 `summary` 列 | 新增 `summary TEXT DEFAULT NULL` | [ ] |
| 7 | articles | 列名 `publish_time` → `created_at` | 统一为 `created_at` | [ ] |
| 8 | articles | 列名 `view_count` → `views` | 统一为 `views INTEGER NOT NULL DEFAULT 0` | [ ] |
| 9 | diabetes_types | 列名 `pathogenesis` → `etiology` | 统一为 `etiology` | [ ] |
| 10 | diabetes_types | 列名 `manifestation` → `symptoms` | 统一为 `symptoms` | [ ] |
| 11 | article_collections | 补充 `created_at` 列 | 新增 `created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP` | [ ] |
| 12 | user_risk_info | 补充 `diabetes_history` 列 | 新增 `diabetes_history VARCHAR(50)` | [ ] |
| 13 | user_risk_info | 补充 `diabetes_type` 列 | 新增 `diabetes_type VARCHAR(50)` | [ ] |
| 14 | user_risk_info | 补充 `result` 列（JSONB，**CRITICAL**） | 新增 `result JSONB DEFAULT NULL` | [ ] |
| 15 | user_risk_info | `gender` CHECK 约束 | 新增 `CHECK (gender IN ('male', 'female'))` | [ ] |
| 16 | life_plans | 列名对齐（`type`→`plan_type`, `sort_order`→`order_num`, `time`→`time_desc`） | 统一为 SQLite 命名 | [ ] |
| 17 | life_plans | `plan_type` 枚举英文值 | 使用 `CHECK(plan_type IN ('diet','exercise','other'))` | [ ] |
| 18 | life_advice | 补充 `created_at` 列 | 新增 `created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP` | [ ] |
| 19 | punch_in | 列名 `plan_id` → `plan_item_id` | 统一为 `plan_item_id` | [ ] |
| 20 | punch_in | 枚举值英文 (`'completed'/'uncompleted'`, `'diet'/'exercise'`) | 使用英文 CHECK 值 | [ ] |
| 21 | punch_in | FK `ON DELETE SET NULL` 补充 | 新增 `ON DELETE SET NULL` 到 FK 约束 | [ ] |
| 22 | admin_logs | 列名 `admin_user_id` → `operator_id` | 统一为 `operator_id` | [ ] |
| 23 | doctor_information | `chat_token` 非空约束 | `chat_token TEXT NOT NULL` | [ ] |
| 24 | doctor_information | `description` 默认值 | `description TEXT DEFAULT ''` | [ ] |
| 25 | 全部表 | 补充全部 18 个索引（含 `idx_plans_user_plan` UNIQUE） | 参照 `init.sql` 第 134-151 行翻译为 PostgreSQL 语法 | [ ] |
| 26 | 全部表 | DDL 幂等性 | 全部使用 `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS` | [ ] |
| 27 | 全部表（4 个 JSON 列） | TEXT → JSONB | `articles.tags`, `user_risk_info.result`, `user_risk_info.raw_input`, `life_advice.tags` 类型改为 JSONB（v16 修正：`admin_logs.operation_content` 和 `admin_logs.operation_result` 保持 TEXT/VARCHAR——此二列存储纯文本非 JSON） | [ ] |
...
应用启动
  → initDatabase()
    ...
    → adapter.init()
      ...
      → KingbaseES:
        → 读取 init_kingbase_ddl.sql（仅含 CREATE TABLE IF NOT EXISTS，幂等安全）
        → 按 ; 分割 DDL → 事务外逐条 pool.query()（DDL 不包裹事务，幂等由 IF NOT EXISTS 保证）
        → 检查 users 表是否有数据
        → 无数据时：读取 init_kingbase_seed.sql → 用 bcryptjs 生成 admin123 哈希 → 替换 __BCRYPT_HASH_PLACEHOLDER__ 占位符 → 按 ; 分割 → BEGIN 事务 → 逐条 client.query() → COMMIT → client.release()
        → 执行启动时区验证：SELECT current_setting('timezone')，非 UTC 时输出 console.error 警告（v13 新增）
    → 导出 adapter 实例供路由使用
```
...
8. **JSON 列合法性校验（v3 新增，v16 修正）**：`articles.tags`、`user_risk_info.result`、`user_risk_info.raw_input`、`life_advice.tags` 共 4 个 JSON 文本列，在迁移写入 KingbaseES 前必须验证每条记录的 JSON 合法性（`JSON.parse` 不抛异常即为合法）。不合法的记录记录到迁移日志中，不阻塞整体迁移，供人工处理。**v16 修正**：`admin_logs.operation_content` 和 `admin_logs.operation_result` 不在 JSON 校验范围内——此二列存储纯文本，无需 JSON 合法性验证。
...
| JSON 字段有效性 | 对 4 个 JSON 文本列逐列 `JSON.parse` 尝试解析（v16 修正：`admin_logs.operation_content`/`operation_result` 不在此列——存储纯文本） | 全部有效（或无非法记录已记录到日志） |
...
3. **并行迁移讨论（远期优化，v10 补充架构前提）**：对于超大规模数据，可考虑按表并行迁移（需处理 FK 依赖顺序）、或采用在线迁移策略（应用双写 SQLite + KingbaseES 一段时间后切换读取源）。**需注意**：当前适配层架构（`database.js` 导出单一 `getAdapter()`、`sql.js` 使用模块级单例方言变量）从根本上不支持同时连接两个数据库。实现"双写"需要架构层面从单例模式变更为多实例模式（如 `getSqliteAdapter()` + `getKingbaseAdapter()` 双导出），改动量远超当前改造方案范围，属于独立的需求迭代。当前项目规模预计无需此复杂度
...
**逆向迁移脚本框架**：Phase 2 生产切换后如需回退到 SQLite，需将 KingbaseES 期间产生的新数据迁回 SQLite。应并行准备 `scripts/migrate-reverse-to-sqlite.js` 脚本框架，在 Phase 2 切换前完成脚本编写和 dry-run 验证。

**逆向迁移核心实现策略（v14 新增）**：

1. **时区反向转换**：KingbaseES 中存储的 UTC datetime 字段（如 `"2025-06-28 06:30:00"`）在写入 SQLite 前需加 8 小时恢复为本地时间（`"2025-06-28 14:30:00"`）。转换方式：
   ```javascript
   // 逆向时区转换：UTC → 本地时间（UTC+8）
   const utcTime = row.created_at; // "2025-06-28 06:30:00"
   const localDate = new Date(utcTime.replace(' ', 'T') + 'Z');
   localDate.setHours(localDate.getHours() + 8);
   // 格式化为 SQLite 兼容的 "YYYY-MM-DD HH:MM:SS" 字符串
   const localTime = localDate.toISOString().replace('T', ' ').substring(0, 19);
   ```
   使用 `+08:00` 固定偏移（而非本地时区），确保逆向转换与正向迁移的 `-8h` 精确对称。**注意**：此逻辑仅对 KingbaseES 中 UTC 存储的数据执行。若 KingbaseES 中部分数据已在正向迁移前通过 `phase0_utc_convert.sql` 转为 UTC，则逆向时仍需 +8h——正向-8h 与逆向+8h 的数学对称性不受 SQLite 中间状态影响。

2. **SERIAL 序列 ID 冲突处理**：KingbaseES 的 SERIAL 列 ID 在逆向写入 SQLite AUTOINCREMENT 时，需处理 ID 冲突风险：
   - **场景**：SQLite 中保留的旧数据（迁移前）的 ID 范围可能为 1-1000，KingbaseES 期间新增数据的 ID 为 1001-1100。逆向写入时需确保 KingbaseES 新增数据的 ID 不与 SQLite 现有 ID 重叠
   - **方案 A（推荐，全量覆盖）**：逆向迁移时清空 SQLite 对应表（`DELETE FROM table`），再将 KingbaseES 全量数据（含迁移前的旧数据 + KingbaseES 期间的新增数据）整体写入 SQLite。此方式无 ID 冲突风险，但需 KingbaseES 中保留完整的迁移前数据
   - **方案 B（增量合并，需 ID 偏移）**：若仅写入 KingbaseES 期间的新增数据（增量），需将所有新记录的 ID 加上一个偏移量（如 `newId = originalId + SQLITE_MAX_ID`）以避免冲突。此方式使 SQLite 中的 ID 不连续，可能影响按 ID 排序的部分查询逻辑
   - **推荐方案 A**：全量覆盖更安全、实现更简单。即使 KingbaseES 期间数据量较大，逆向迁移是一次性运维操作，停机时间可接受

3. **JSONB → TEXT 逆向转换**：KingbaseES 中 JSONB 类型列在迁移至 KingbaseES 时由 SQLite TEXT 转换而来。逆向迁移时需将 JSONB 值序列化为 TEXT 字符串写入 SQLite：
   ```javascript
   // KingbaseES → SQLite JSONB 逆向转换
   const jsonbValue = row.tags; // KingbaseES pg 驱动返回的 JSONB 值（可能已是 JS 对象）
   const textValue = typeof jsonbValue === 'object' ? JSON.stringify(jsonbValue) : String(jsonbValue);
   // 写入 SQLite TEXT 列
   ```
   `pg` 驱动默认将 JSONB 列自动解析为 JavaScript 对象（非字符串），若 3.4.8 节的 timestamp 类型解析器未覆盖 JSONB 的 OID（OID 3802），逆向迁移脚本中需在读取 KingbaseES 数据后显式 `JSON.stringify()` 序列化。若已配置 `pg.types.setTypeParser(3802, val => String(val))` 拦截 JSONB 自动解析（推荐，确保格式一致性），则逆向迁移无需额外序列化。

4. **增量 vs 全量回退策略**：
   | 策略 | 做法 | 适用场景 | 停机时间 |
   |------|------|---------|---------|
   | **全量回退（推荐）** | SQLite 清空目标表 → KingbaseES 全量数据写入 SQLite（含迁移前旧数据 + 新增数据） | KingbaseES 运行时间短、数据增量小（< 数万条） | 与正向迁移类似（< 30 分钟，见 12.2 节） |
   | 增量回退 | SQLite 保留旧数据 → 仅写入 KingbaseES 期间新增的数据（需 ID 偏移） | KingbaseES 运行时间长、数据增量大且 SQLite 旧数据量也大 | 仅写入增量部分，耗时较短但实现复杂 |
   
   推荐全量回退——实现简单、ID 无冲突风险、验证容易（全量行数对比）。仅在数据量极大且 SQLite 旧数据极多时考虑增量回退。

5. **逆向迁移验证策略**：参照正向迁移的 7 维度验证清单（12.1 节），逆向迁移后执行：
   - 行数对比（KingbaseES vs 逆向后的 SQLite）
   - 抽样逐列对比（时区已反向转换，列值应与原始 UTC 值 +8h 一致）
   - FK 有效性检查、非空约束检查、JSON 字段有效性
   - 功能冒烟测试（登录、查看方案、查看打卡记录）

6. **逆向迁移脚本执行时机**：
   - 正向迁移 dry-run 通过后、Phase 2 生产切换前：编写完成并 dry-run 验证通过
   - Phase 2 生产切换后决定回退时：作为回退操作的一部分执行
...
1. **双写机制讨论（v10 补充架构前提）**：若业务要求零数据丢失回退，需在 Phase 2 期间实现 SQLite+KingbaseES 双写。**需注意**：当前适配层架构（单例 `getAdapter()`）不支持同时连接两个数据库，实现双写需要架构层面从单例模式变更为多实例模式（`getSqliteAdapter()` + `getKingbaseAdapter()` 双导出），改动量远超当前改造方案范围，为独立的需求迭代。当前项目规模不推荐
...
| 双 DDL 同步 | 见 10.3 节 |
| 优雅关闭 | server.js 注册 SIGTERM/SIGINT 信号处理，依次调用 `adapter.close()` 释放数据库连接池后再退出进程（见 3.5.1 节"优雅关闭"段落，v11 新增） |

---

## 14. 前端确认

**确认**：前端 API 调用代码（请求路径、请求方法、请求体结构、响应体解析）无需修改。前端页面需在时间显示层增加时区转换逻辑（数据库存储为 UTC，展示时转换为用户本地时区）。
...
**需修改（v14 修正）**：
- **时间显示层时区转换**：Phase 0 切换 `CURRENT_TIMESTAMP` 后，数据库存储的时间为 UTC（如 `"2025-06-28 06:30:00"`），前端在渲染时间字符串时需将其从 UTC 转换为用户本地时区（如北京时间显示为 `"2025-06-28 14:30:00"`）。建议在 `src/utils/timezone.js` 中新建一个统一的时区转换工具函数，供所有展示时间的 Vue 组件调用。具体改造涉及：所有展示 `created_at`、`updated_at`、`punch_time`、`operation_time` 等 datetime 字段的组件，均需将原始 UTC 字符串通过 `convertUTCToLocal(utcStr)` 转换后再渲染。
- **改造模式**：
  ```javascript
  // src/utils/timezone.js（新建）
  // 将数据库返回的 UTC 时间字符串（"YYYY-MM-DD HH:MM:SS"）转换为用户本地时区显示
  export function convertUTCToLocal(utcStr) {
    if (!utcStr) return '';
    // 将 UTC 字符串解析为 UTC Date 对象，再格式化为本地时间字符串
    const [datePart, timePart] = utcStr.split(' ');
    const utcDate = new Date(datePart + 'T' + timePart + 'Z');
    // 使用 toLocaleString 或手写格式化逻辑输出本地时间
    return utcDate.toLocaleString('zh-CN', { hour12: false });
  }
  ```
  实现者可根据项目实际使用的日期库（如 dayjs/moment.js）选择等价的转换方式。
- **影响范围**：此变更不影响 API 调用代码——API 请求和响应格式不变。仅影响前端渲染层的时间字符串展示。若前端在某处展示数据库返回的时间字符串且期望展示为本地时间，则该处需引入 `convertUTCToLocal` 转换。
...
**v11 补充 —— timestamp 字段 JSON 响应格式一致性**：KingbaseAdapter 中通过 `pg.types.setTypeParser` 将 timestamp/timestamptz 类型的解析器设为原样返回字符串（见 3.4.8 节），确保 KingbaseES 后端下 datetime 字段的 JSON 响应格式（`"YYYY-MM-DD HH:MM:SS"`）与 SQLite 后端一致。此措施保障了前端解析 datetime 字段的代码无需修改——API 返回的 datetime 字段格式在两个后端下完全一致。

**v14 修正**：原方案（v1-v13）第 14 节声明"前端代码无需任何修改"，与第 6 节 Phase 0 验收标准第 6 条"前端页面中显示的时间正确（数据库存储为 UTC，前端展示应转换为本地时间）"存在逻辑矛盾——若前端代码零改动，时间显示将全部偏差 8 小时，与 Phase 0 验收标准直接冲突。v14 修正为：前端 API 调用代码无需修改，但时间显示层需增加 UTC→本地时区的转换逻辑。详见前端文件变更（第 16 节新增 `src/utils/timezone.js` 新建及引用此函数的 Vue 组件改造）。
...
| **（v13 新增）KingbaseES 服务器时区非 UTC 导致 `CURRENT_TIMESTAMP` 输出本地时间** | `sql.now()` 输出 `CURRENT_TIMESTAMP` 的决策假设 KingbaseES 也返回 UTC，但实际返回的是服务器时区时间。若 KingbaseES 服务器时区为 `Asia/Shanghai`（UTC+8，国产数据库在中国部署场景常见默认值），新写入的 timestamp 为本地时间而非 UTC——(1) Phase 1 新数据非 UTC，与"统一 UTC 存储"矛盾；(2) Phase 2 迁移后数据库内同时存在 UTC（迁移旧数据）和本地时间（`CURRENT_TIMESTAMP` 新数据），重新引入混合时间戳问题；(3) `sql.formatDateParam()` 输出的 UTC 参数字符串与数据库列中存储的本地时间字符串比较时产生 8 小时偏差 | (1) KingbaseES 服务器 `timezone` 强制设为 `UTC`（配置文件或 Docker `TZ=UTC` 环境变量，见 4.2 节"KingbaseES 服务器时区配置要求"）；(2) KingbaseAdapter 启动时执行 `SELECT current_setting('timezone')` 验证，非 UTC 时输出 `console.error` 警告（见 3.4.10 节）；(3) CI/Docker 部署配置中明确 `TZ=UTC`（见 5.1 节）；(4) Phase 1 验收时确认 KingbaseES 实例时区为 UTC |
...
| **（v9 新增，v10 修正方案）`proxyDifySSE` 硬编码 `inputs: {}` 阻断 admin chat 的 `db_type` 变量传递** | `sseProxy.js` 第 26 行硬编码 `inputs: {}`，Dify admin chat 工作流永远接收不到 `db_type` 变量，Jinja2 条件判断永不生效——方案设计的 Dify 端同步变更策略对 admin chat 路径完全无效 | **v10 修正**：最小化方案——不修改 `proxyDifySSE` 函数签名（实际为 `function proxyDifySSE({ apiKey, query, conversationId, userId, res, req })`，非 async），仅在函数体内部将 `inputs: {}` 改为 `inputs: { db_type: process.env.DB_TYPE \|\| 'sqlite' }`。所有调用方（admin.js/assistant.js/chat.js）零改动（见 9.2 节 v10 修正） |
| **（v9 新增，v10 修正索引名）`FOR UPDATE` 行级锁对"首次方案生成"场景失效** | 用户首次生成方案时 `life_plans` 表无该用户的任何行：事务内 `UPDATE ... SET is_active = 0` 影响零行（不获取行级锁），`SELECT MAX(plan_id) ... FOR UPDATE` 的 WHERE 条件匹配零行（空结果集的 FOR UPDATE 在 PostgreSQL/KingbaseES READ COMMITTED 下不获取任何行级锁）。两个并发请求均可成功 INSERT 相同 plan_id，因 life_plans 表无 UNIQUE(user_id, plan_id) 约束 | 在 `life_plans` 表上增加 `UNIQUE(user_id, plan_id)` 约束（数据库层最后防线）；同步更新 `init.sql`（第 145 行 `idx_plans_user_plan` 从普通索引改为 UNIQUE 索引）和 `init_kingbase.sql` 的 DDL；/adjust 操作不触发此问题（调整前提是已存在方案）（见 8.5 节 v9 新增，v10 修正索引名） |
| **（v11 新增，v16 扩展）pg 驱动 timestamp/JSONB/JSON 类型自动解析导致 API 响应 JSON 格式不一致** | `pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列解析为 JS Date 对象，将 JSONB（OID 3802）/JSON（OID 114）列解析为 JS 对象。JSON 序列化后 timestamp 变为 ISO 8601 格式（`"2025-06-28T06:30:00.000Z"`），JSON 变为原生对象而非 TEXT 字符串。SQLite 路径下两者均以纯文本返回。JSONB 自动解析尤为危险——`articles.tags` 等字段在代码中执行 `JSON.parse()` 二次解析，`JSON.parse(object)` 抛 TypeError 被 catch 静默返回空数组，造成**标签数据静默丢失** | 在 KingbaseAdapter 构造函数中通过 `pg.types.setTypeParser` 注册 4 种 OID 的解析器（1114=timestamp、1184=timestamptz、3802=jsonb、114=json），均设为 `val => String(val)` 原样返回字符串，与 SQLite 行为保持一致。启动验证中打印实际激活的 OID 值（见 3.4.8 节 v16 扩展） |
| **（v11 新增）KingbaseAdapter close() 方法缺失且应用优雅关闭机制遗漏** | 适配层接口定义了 `async close()` 方法，但 KingbaseAdapter 的 `close()` 实现（需调用 `pool.end()`）在方案中完全缺失；应用也未设计 SIGTERM/SIGINT 信号处理，导致连接池暴力断开、进行中事务中断 | 在 KingbaseAdapter 中实现 `close()` 方法（调用 `await this.pool.end()`）；在 server.js 中增加 SIGTERM/SIGINT 信号处理，调用 `adapter.close()` 后退出（见 3.4.9 节和 3.5.1 节"优雅关闭"段落） |
| **（v11 新增，v16 修正）`dateRange.js` 工具模块含 T 分隔符的日期格式在 KingbaseES 中的隐式类型转换行为未经确认** | `punch.js` 依赖 `parseDateRange()` 输出 `YYYY-MM-DDTHH:MM:SS` 格式的 endDate 参数（含 T 分隔符），在 KingbaseES 中需隐式将 TEXT 转换为 TIMESTAMP 才能与 `punch_time` 列比较。前序版本以"已执行验证"口吻描述但实际为理论推导（基于"KingbaseES V8R6 = PostgreSQL 12"的未验证假设） | v16 将验证 SQL 从"已执行"改为"待执行验证清单"口吻，加入 Phase 1 前置验证脚本（`scripts/verify-pg-kingbase.js`）作为额外验证项；Phase 1 双库对比测试验证时间范围筛选的一致性（见 4.2.1 节 v16 修订） |
| **（v11 新增）SQLite AUTOINCREMENT 与 PostgreSQL SERIAL 的语义差异未讨论** | SQLite `AUTOINCREMENT` 保证 ID 不重复使用，PostgreSQL `SERIAL` 基于 SEQUENCE 不具备 no-reuse 保证。若应用代码对 ID 语义有隐式依赖（如按 ID 排序推断时间、假定无间隙的分页逻辑），迁移后行为可能不一致 | 已分析：本项目所有 ID 均由数据库自动生成（应用不显式指定），正常使用场景下两者行为等价；ID 排序推断时间在两个数据库下均成立；无间隙假设在两个数据库中均不成立（与迁移无关）。Phase 1 双库对比测试验证 ID 递增行为（见 4.1 节"v11 补充"和 10.2 节翻译规则注释） |
| **（v11 新增）连接池大小确定方法缺失** | 方案推荐 `max=10` 但未提供定量方法确定此数值，也未给出负载测试方法建议，实现者可能凭感觉设定导致生产环境连接池配置不当 | 新增连接池大小确定方法子节：给出简化公式、安全默认值依据、精确确定方法（Phase 1 压测 + pool 指标监控）（见 7.2.3 节） |
| **（v11 新增，v16 增强）`query_table` WHERE 子句 SQL 注入路径安全防护不足** | `admin.js` 的 `query_table` tool_name 将 Dify AI 生成的 WHERE/ORDER BY 子句直接字符串插值到 SQL 中（`WHERE ${params.where}`），虽参数源于 Dify AI 而非外部用户，仍构成间接 SQL 注入攻击面。v11 仅建议日志记录作为缓解（事后审计），防御纵深不足 | Phase 1 实现三层基础防护：(1) 应用层 WHERE 子句安全校验——使用 `node-sql-parser` 解析 AST 检查是否仅含合法条件表达式，拒绝含子查询/DML/非安全函数的 WHERE 子句（返回 400）；(2) Dify system prompt 安全约束——禁止生成含子查询/DML 的 WHERE 子句；(3) Phase 1 验收增加安全测试用例（注入尝试应返回 400 而非执行）。Phase 2+ 引入结构化查询参数替代字符串插值（见 9.2 节"query_table 安全维度说明" v16 增强） |
| **（v16 新增）`admin_logs.operation_content` 和 `admin_logs.operation_result` 被错误归类为 JSONB——若按方案创建 JSONB 列将导致所有管理员操作日志 INSERT 失败** | 方案第 10.2 节将两列与其他 4 个 JSON 文本列一并推荐使用 JSONB。经代码核实（admin.js `insertAdminLog` 函数第 147-156 行调用方传入值）：`operation_content` 存储纯文本操作描述（如 SQL 文本 `"SELECT * FROM users..."`）；`operation_result` 存储纯文本结果（如 `"成功"`、`"权限不足"`）。若按方案改为 JSONB，INSERT 传入纯文本（非合法 JSON）将抛出 `invalid input syntax for type json` 错误（SQLSTATE 22P02），导致所有管理员操作日志写入失败 | 两列从 JSONB 推荐列表移除，保持 TEXT/VARCHAR；在 10.1 节差异分析表、10.2 节 JSON 列决策、翻译规则表、DDL 实现清单、GIN 索引建议、12 节迁移 JSON 列列表中全部修正（从前序 6 个 JSON 列更正为 4 个）；在第 15 节风险表中新增此独立风险项（见 10.1/10.2 节 v16 修正） |
| **（v16 新增）DDL 执行"失败后继续"策略未考虑 FK 依赖的级联失败** | 方案第 3.4.5 节原步骤 5 规定"某条 DDL 失败则继续执行后续 DDL"，但忽略了 FK 依赖的级联失败场景——若父表（如 `users`）创建失败，所有依赖子表（如 `life_plans`、`punch_in`、`user_risk_info` 等 8 张表）因 FK 引用不存在的父表而级联失败，错误列表含大量噪音导致运维人员难以定位根因 | (1) DDL 文件按 FK 依赖拓扑顺序排列（父表在前、子表在后）；(2) 在日志中区分"根因失败"和"级联失败"（级联失败的子表 DDL 标注 `[CASCADED]` 前缀）；(3) 推荐采用"首条 DDL 失败即终止"的保守策略（依赖 `IF NOT EXISTS` 幂等性安全重跑，见 3.4.5 节 v16 修订） |
| **（v16 新增）OID 1182 timestamptz 解析器注册缺乏来源依据** | 方案第 3.4.8 节（v11-v15）额外注册了 OID 1182 作为 timestamptz 的"KingbaseES 可能的备用 OID"。PostgreSQL 12 中 OID 1182 实际对应 `_date`（date 的数组类型），与 timestamptz 无关。将 timestamptz 的解析器注册到 `_date` 数组类型上虽不产生运行时副作用（该类型不在本项目中使用），但缺乏来源依据且可能误导后续维护者 | 已从代码示例中删除 OID 1182 注册行，仅保留 1114（timestamp）和 1184（timestamptz）的标准 OID。在启动验证中增加 `SELECT oid, typname FROM pg_type WHERE typname IN ('timestamp','timestamptz','jsonb','json')` 日志输出，确认实际 OID（见 3.4.8 节 v16 修正） |
| **（v14 新增）`pg` 驱动版本与 KingbaseES V8R6 的兼容性缺乏验证声明** | 方案推荐 pg v8.12 但未在实际 KingbaseES V8R6 实例上验证过连接、查询、事务、连接池、数据类型映射等核心功能。多处技术判断建立在"KingbaseES V8R6 行为等于 PostgreSQL 12"假设上，但国产化分支可能有特定修改（如协议层变更、OID 映射差异、时区处理差异） | Phase 1 实现前在目标 KingbaseES V8R6 实例上执行 `scripts/verify-pg-kingbase.js` 验证脚本（6 个核心验证用例，见 6 节 Phase 1 前置条件）。任何验证用例失败则阻塞 KingbaseAdapter 实现，先排查 KingbaseES 配置或 pg 版本问题。验证通过后记录 KingbaseES 精确版本号、pg 版本号和验证结果到 Phase 1 验收文档 |
| **（v14 新增）前端时区转换需求的工程范围低估** | 原方案声明"前端代码无需任何修改"，但 Phase 0 切换 UTC 存储后前端时间显示将全部偏差 8 小时。新建 `src/utils/timezone.js` 及改造时间显示组件是新增工程工作量，需纳入 Phase 0 改造计划 | 在 Phase 0 验收标准第 6 条中明确了前端时区转换的具体验证要点（`timezone.js` 已新建、Vue 组件已引入 `convertUTCToLocal`）；在第 16 节文件变更清单中新增前端文件条目；在第 14 节前端确认中明确改造范围（API 调用代码不变，仅时间显示层改造） |

---

## 16. 文件变更清单

> **前提（v15 新增）**：项目当前不存在 `scripts/` 目录，实现前需先创建此目录（`mkdir -p scripts/`）。
...
| 新建 | `server/db/adapter/KingbaseAdapter.js` | KingbaseAdapter 实现（含 init 拆分 DDL/种子文件执行（v14 修订）、`?`→`$1` 占位符转换含回退日志（v14 新增）、RETURNING id 自动追加、tableInfo、healthCheck、close（v11 新增）方法、pg.types.setTypeParser timestamp/JSONB/JSON 类型解析器配置（v11 新增，v16 扩展含 OID 1114/1184/3802/114 四种类型）、SSL/TLS 配置、pool.on('error') 事件处理） |
...
| 改造 | `server.js` | 启动流程改造：`initDatabase()` 改为 await（IIFE + async/await），确保数据库初始化完成后才启动 HTTP 服务（v3 纳入变更范围）；增加 SIGTERM/SIGINT 优雅关闭处理（调用 `adapter.close()` 后退出，v11 新增） |
...
| 改造 | `server/db/init.sql` | 将第 145 行 `idx_plans_user_plan` 从普通索引改为 UNIQUE 索引（`CREATE INDEX IF NOT EXISTS` → `CREATE UNIQUE INDEX IF NOT EXISTS idx_plans_user_plan ON life_plans(user_id, plan_id)`），配合 `FOR UPDATE` 方案的首次生成场景保护（v9 新增，v10 修正索引名，见 8.5 节） |
| 重写 | `server/db/init_kingbase_ddl.sql` | **v14 新增拆分**：从 `init_kingbase.sql` 中分离出的纯 DDL 文件（仅含 `CREATE TABLE IF NOT EXISTS` 和 `CREATE INDEX IF NOT EXISTS` 语句，不含种子数据）。对齐 init.sql schema（使用 `CREATE TABLE IF NOT EXISTS`，补充 `result`/`diabetes_history`/`diabetes_type` 字段、全部 18 个索引含 `idx_plans_user_plan` UNIQUE 索引、doctor_information 和 punch_in 约束修复、列名/枚举值统一）。DDL 在 `init()` 中于事务外逐条执行，幂等性由 `CREATE TABLE IF NOT EXISTS` 保证 |
| 重写 | `server/db/init_kingbase_seed.sql` | **v14 新增拆分**：从 `init_kingbase.sql` 中分离出的纯种子数据文件（仅含 INSERT 语句：管理员用户、医生信息、文章等）。种子数据使用 `__BCRYPT_HASH_PLACEHOLDER__` 占位符，内容对齐 seed.sql。种子数据在 `init()` 中于事务内执行，保证多表种子数据插入的原子性 |
| 删除 | `server/db/init_kingbase.sql` | **v15 新增**：旧文件已拆分为 `init_kingbase_ddl.sql` 和 `init_kingbase_seed.sql`，原文件归档或删除，不再使用（见第 10 节开头说明） |
| 新建 | `scripts/migrate-to-kingbase.js` | 一次性数据迁移脚本（含时区转换 + SERIAL 序列重置 + 迁移前后备份提示 + 多维度验证 + JSON 合法性校验） |
| 新建 | `scripts/migrate-reverse-to-sqlite.js` | 逆向迁移脚本（KingbaseES → SQLite 数据回退，含 UTC→本地时间 +8h 时区反向转换、SERIAL ID 冲突处理策略、JSONB→TEXT 逆向转换。v14 补充完整实现策略，见第 12 节"逆向迁移核心实现策略"） |
| 新建 | `scripts/verify-pg-kingbase.js` | pg v8.12 + KingbaseES V8R6 兼容性验证脚本（v14 新增，v16 扩展）。逐项验证：TCP 连接认证、参数化查询、事务、连接池、数据类型映射（含 JSONB/JSON）、timestamp/JSONB/JSON 解析器拦截、T 分隔符日期字符串隐式转换（v16 新增）。全部通过后输出 `[PASS]`，任何失败输出 `[FAIL]` |
| 新建 | `scripts/perf-benchmark.sh` | Phase 1 性能基准测试脚本（v14 新增）。自动化压测 6 个关键端点（10 并发/30s）并生成 JSON 格式性能对比报告 |
...
| 新建 | `src/utils/timezone.js` | 前端时区转换工具模块，导出 `convertUTCToLocal(utcStr)` 函数——将数据库返回的 UTC 时间字符串（`"YYYY-MM-DD HH:MM:SS"`）转换为用户本地时区字符串。供所有展示 datetime 字段的 Vue 组件引用（v14 新增） |
| 改造 | `src/`（前端 Vue 组件） | 所有展示 datetime 字段（`created_at`、`updated_at`、`punch_time`、`operation_time` 等）的 Vue 组件需引入 `convertUTCToLocal` 并调用转换后再渲染。具体涉及的组件由前端开发者根据实际页面中的时间展示位置自行识别改造（v14 新增） |
| 不变 | `src/`（前端，除上述时间显示组件外） | API 调用代码、路由路径、请求/响应格式、字段名——全部零改动 |
| 不变 | `server/middleware/` | 零改动 |
| 不变 | `server/services/difyService.js` | **v10 撤回**：原方案（v6）建议在 `callWorkflowBlocking` 中增加 `inputs.db_type` 参数传递，经核实——(1) `callWorkflowBlocking` 用于方案生成和风险评估工作流，不涉及数据库动态 SQL 查询，无需 `db_type`；(2) 原方案伪代码使用 `axios.post()` 但实际代码使用 `httpRequest()` 自定义函数，代码结构完全不匹配。该改造建议属于过度设计，已撤回 |
| 改造 | `server/services/sseProxy.js` | 函数体内部 `inputs: {}`（第 26 行）→ `inputs: { db_type: process.env.DB_TYPE \|\| 'sqlite' }`。**不修改函数签名**——实际签名为 `function proxyDifySSE({ apiKey, query, conversationId, userId, res, req })`（非 async，单解构对象），所有调用方（admin.js/assistant.js/chat.js）零改动（v9 新增改造，v10 修正为最小化方案，见 9.2 节） |
...
本轮修订基于第 7 轮组件 B 诊断报告（`b_v7_diag_v2.md`）的审查反馈及质询确认，共解决全部 10 个问题（2 个严重、6 个一般、2 个轻微）。其中问题 1-8 为第 7 轮持续至第 8 轮的问题，问题 9-10 为本轮新识别问题。本轮重点：修正文档中与实际项目代码不匹配的技术方案描述，消除方案的可直接实施性障碍。

### 一、严重问题修复（2 个）

**修订 R1：`proxyDifySSE` 函数签名描述与项目实际代码完全不匹配（问题 1，持续性问题）**

在第 9.2 节第 6 点"`proxyDifySSE` SSE 代理的 `inputs` 参数传递"中彻底重写改造方案：

- **修正前（v9）**：函数签名描述为 `async function proxyDifySSE({ apiKey, baseUrl, route }, query, user, inputs = {}, req, res)`，admin.js 调用方描述为 `await proxyDifySSE(difyConfig, query, user, inputs, req, res)`，三处与实际代码不符（非 async、参数结构完全不同、调用方使用了不存在的变量）
- **修正后（v10）**：以项目实际代码 `sseProxy.js` 第 4 行 `function proxyDifySSE({ apiKey, query, conversationId, userId, res, req })` 为准，给出最小化方案——不修改函数签名，仅在函数体内部将第 26 行 `inputs: {}` 改为 `inputs: { db_type: process.env.DB_TYPE || 'sqlite' }`。所有调用方（admin.js/assistant.js/chat.js）零改动
- 新增三处事实错误的明确说明，避免实现者按旧模板实施导致代码无法运行

**修订 R2：`difyService.js` 改造方案伪代码与项目实际代码结构完全不符（问题 2，持续性问题）**

在第 9.2 节"Dify 端同步变更"中重写 `db_type` 变量传递路径分析：

- **修正前（v6-v9）**：伪代码使用 `axios.post()`，建议修改 `callWorkflowBlocking` 增加 `inputs.db_type`
- **修正后（v10）**：新增两条独立 Dify 调用路径的对比表，明确区分——(1) `callWorkflowBlocking`（方案生成/风险评估，`difyService.js` 第 84 行）不需要 `db_type`，撤回 v6 的过度设计建议；(2) `proxyDifySSE`（admin chat SSE 流式，`sseProxy.js` 第 4 行）需要 `db_type`，通过函数体内部修改注入
- 新增原方案 v6 伪代码的三处事实错误说明：使用 `axios.post()`（实际为 `httpRequest()`）、函数签名不匹配（实际为 `callWorkflowBlocking(apiKey, inputs, workflowType)`）、`user` 硬编码（实际为 `'api-user'`）
- 更新"变更范围"（第 3 点）从"`difyService.js` 中调用"修正为"`sseProxy.js` 的 SSE 代理调用"

### 二、一般问题修复（6 个）

**修订 R3：输出文档对 `init.sql` 中 `life_plans` 表索引名称和行号的多处事实错误修正（问题 3 + 问题 10，持续性问题）**

修正了文档中 5 处 `init.sql` 索引引用的事实错误：

- **索引名称**：`idx_life_plans_user_plan_id` → `idx_plans_user_plan`（3 处：第 8.5 节 v9 同步变更段落、第 10.1 节差异分析表 `life_plans` 行、第 16 节文件变更清单 `init.sql` 条目）
- **行号**：第 138 行 → 第 145 行（2 处：第 8.5 节、第 10.1 节。第 138 行实际是 `article_collections` 表的 `idx_collections_user_article` UNIQUE 索引，与 `life_plans` 表完全无关）
- **语义修正**：将"升格"描述改为"将普通索引改为 UNIQUE 索引"，因原索引名 `idx_plans_user_plan` 是普通索引而非 UNIQUE 索引，不存在"升格已有 UNIQUE 索引"的语义
- 在差异分析表和文件变更清单中标注 v10 修正标记

**修订 R4：`proxyDifySSE` 其他调用方排查结论从"开放任务"变更为"明确排除结论"（问题 4，持续性问题）**

在第 9.2 节第 6 点(b)中，将原"推荐统一传入"的模糊指导替换为经实际代码检查确认的明确结论：

- `assistant.js`（第 20 行）：连接 `DIFY_ASSISTANT_APP_KEY` 智能助手工作流，不涉及数据库动态 SQL，不需要 `db_type`
- `chat.js`（第 27 行）：连接医生专属 Dify 工作流，不涉及数据库动态 SQL，不需要 `db_type`
- 结论：三个调用方均零改动，`db_type` 由 `sseProxy.js` 内部统一注入
- 将原方案将排查结论作为"开放任务"的做法标注为已撤销

**修订 R5：手工回归测试清单 T15、T17 验证标准补充具体内容（问题 5，持续性问题）**

在第 5.1.1 节测试清单中：

- **T15**：从"各 tool_name 返回正确结果"扩展为明确列出 6 个核心 tool_name（`query_user_profile`/`query_risk_history`/`query_punch_records`/`query_life_plans`/`query_table`/`get_table_schema`），每个标注传入参数和预期返回字段
- **T17**：从单一"GET/POST"拆分为 T17（POST `/api/assistant/chat` SSE 流式对话）和 T17a（GET `/api/assistant/conversations` 历史），各自标注具体验证点（HTTP 状态码、Content-Type、SSE 事件格式、返回字段）

**修订 R6：在线迁移（双写）讨论增加适配层架构前提说明（问题 6，持续性问题）**

在第 12.2 节第 3 点"并行迁移讨论（远期优化）"和第 12.4 节"双写机制讨论"两处：

- 新增架构前提说明：当前适配层（单例 `getAdapter()`、模块级单例方言变量）从根本上不支持同时连接两个数据库，实现"双写"需要架构层面从单例模式变更为多实例模式（`getSqliteAdapter()` + `getKingbaseAdapter()` 双导出），属于独立的需求迭代
- 避免将"双写"描述为轻量优化而产生误导

**修订 R7：`callWorkflowBlocking` 路径是否需要 `db_type` 的明确判断（问题 7，持续性问题）**

此修订与 R2 合并处理——在第 9.2 节新增的两条 Dify 调用路径对比表中，明确标注 `callWorkflowBlocking` 路径"不需要 `db_type`"及其理由（方案生成和风险评估工作流不涉及数据库动态 SQL 查询），撤回 v6 对 `difyService.js` 的过度设计建议。

**修订 R8：`adapter.db` 过渡属性在 SqliteAdapter 实现要点中显式声明（问题 8，持续性问题）**

在第 3.3 节 SqliteAdapter 实现要点清单末尾新增 `this.db` 属性暴露条目：

- 说明构造函数中 `this.db = new Database(dbPath)` 需通过 `adapter.db` 属性对外暴露
- 标注此属性供 Phase 0 双导出过渡策略使用（`database.js` 步骤 2 中 `module.exports = { db: adapter.db, ... }`）
- 警告：实现者仅阅读本节可能遗漏此暴露需求，务必在实现中显式声明

### 三、轻微问题修复（2 个）

**修订 R9：`server.js` 改造方案补充 `db` 导出消费变化说明（问题 9，本轮新识别）**

在第 3.5.1 节新增"`db` 导出消费变化确认"段落：

- 确认当前 `server.js`（第 2 行）解构导入 `{ initDatabase, db }`，但 `db` 未被 `server.js` 自身使用（数据库访问全部在路由文件中）
- 改造后解构 `{ initDatabase, getAdapter }` 删除 `db` 不会产生未使用变量错误，改造安全

**修订 R10：输出文档 `init.sql` 索引描述行号 138 属于不同表的事实错误修正（问题 10，本轮新识别）**

此修订与 R3 合并处理——在修正行号（138 → 145）的同时，在 8.5 节新增注释说明：第 138 行实际是 `article_collections` 表的 `idx_collections_user_article` UNIQUE 索引，与 `life_plans` 表完全无关。此问题与问题 3 同源（均源于对 `init.sql` 的系统性索引描述偏差），但聚焦于"行号引用指向不同表"这一独立事实维度——即使修正了索引名称，行号错误仍会导致实现者定位到错误代码。

### 四、配套更新

- 第 3.3 节 SqliteAdapter 实现要点：新增 `this.db` 属性暴露条目
- 第 3.5.1 节 server.js 改造：新增 `db` 导出消费变化确认段落
- 第 5.1.1 节测试清单：T15、T17 验证标准具体化，T17 拆分为 T17/T17a
- 第 8.5 节 init.sql 同步变更：修正索引名称、行号，补充事实错误说明
- 第 9.2 节 Dify 端同步变更：重写 `db_type` 传递路径分析（两条路径对比表）；重写 proxyDifySSE 改造方案（最小化方案，不修改函数签名）；撤回 difyService.js 改造建议（过度设计）
- 第 10.1 节差异分析表：修正 `life_plans` 行和"全部表"行的索引名称和行号
- 第 12.2/12.4 节：双写讨论增加适配层架构前提说明
- 第 16 节文件变更清单：`difyService.js` 从"改造"改为"不变"；`sseProxy.js` 修正为最小化方案描述；`admin.js` 撤回 `/chat` inputs 传递；`init.sql` 修正索引名和操作描述

### 五、版本标记更新

- 文档标题从"（v9）"更新为"（v10）"
- 所有新增和修正内容标注"v10 新增"或"v10 修正/撤回"标记，便于审阅者定位本轮变更

---

## 修订说明（v10 → v11）

本轮修订基于第 8 轮组件 B 诊断报告（`b_v8_diag_v1.md`）经质询确认（LOCATED）的审查反馈，共解决全部 7 个问题（1 个严重、3 个一般、3 个轻微）。这 7 个问题均为本轮新发现的维度，不存在与历史迭代反馈的重复。重点覆盖：跨后端 API 响应格式一致性、运行时生命周期管理、工具链完整性、运维完备性和版本标记/文档一致性。

### 一、严重问题修复（1 个）

**修订 R1：pg 驱动 timestamp 类型自动解析导致 API 响应 JSON 格式不一致（问题 1）**

在 3.4.8 节新增"pg 驱动 timestamp 类型自动解析控制"子节，包含：
- 问题完整描述：`pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列解析为 JavaScript Date 对象，JSON 序列化后变为 ISO 8601 格式（如 `"2025-06-28T06:30:00.000Z"`），而 SQLite 路径下 timestamp 以纯文本形式返回（`"YYYY-MM-DD HH:MM:SS"`），直接影响 9 张表的 11 个 datetime 字段，违背"前端代码零变动"核心承诺
- 解决方案：通过 `pg.types.setTypeParser(1114, val => String(val))` 和 `pg.types.setTypeParser(1184, val => String(val))` 将 timestamp/timestamptz 类型解析器设为原样返回字符串
- 不保留 Date 对象行为的三点理由（SQLite 格式一致性、前端零变动承诺、ISO 8601 格式排序兼容性）
- 注意事项：`setTypeParser` 的模块级全局副作用影响说明、OID 值版本稳定性说明

在第 14 节前端确认中新增"v11 补充"段落，明确声明 timestamp 格式一致性保障措施。

在第 15 节风险表中新增"pg 驱动 timestamp 类型自动解析导致 API 响应 JSON 格式不一致"风险项。

### 二、一般问题修复（3 个）

**修订 R2：KingbaseAdapter `close()` 方法实现缺失，应用优雅关闭机制完全遗漏（问题 2）**

在 3.4.9 节新增"`close()` 方法实现"子节，包含：
- `close()` 方法的完整实现（调用 `await this.pool.end()`）
- `pg.Pool.end()` 的行为说明（等待进行中查询 → 断开连接 → 拒绝新查询）

在 3.5.1 节新增"优雅关闭"段落，包含：
- SIGTERM/SIGINT 信号处理函数完整代码轮廓
- 关闭流程时序：`SIGTERM` → `adapter.close()` → `pool.end()` → `process.exit(0)`
- SqliteAdapter 的 `close()` 等价行为说明

在 13.3 节运维维度表中新增"优雅关闭"条目，指向 3.5.1 节。

在第 15 节风险表中新增"KingbaseAdapter close() 方法缺失且应用优雅关闭机制遗漏"风险项。

在第 16 节文件变更清单中更新 KingbaseAdapter.js 条目（补充 close 方法）和 server.js 条目（补充优雅关闭）。

**修订 R3：`server/utils/dateRange.js` 工具模块与 KingbaseES 日期参数的兼容性评估（问题 3）**

在 4.2.1 节新增"`server/utils/dateRange.js` 工具模块兼容性评估"子节，包含：
- 问题描述：`parseDateRange()` 输出 `YYYY-MM-DDTHH:MM:SS` 格式（含 T 分隔符），在 KingbaseES 中需隐式 TEXT→TIMESTAMP 转换
- 结论：无需改造——ISO 8601 格式（含 T 分隔符）是 PostgreSQL TIMESTAMP 解析的标准输入格式，SQLite 同样能正确处理
- 验证方法：在 KingbaseES V8R6 上的测试 SQL 语句
- Phase 1 双库对比测试验证建议

在第 15 节风险表中新增"`dateRange.js` 工具模块含 T 分隔符的日期格式在 KingbaseES 中的隐式类型转换行为未经确认"风险项。

**修订 R4：SQLite AUTOINCREMENT 与 PostgreSQL SERIAL 语义差异分析（问题 4）**

在 4.1 节差异清单中新增"自增主键语义差异"行（标注 v11 新增），并在节末新增"v11 补充"详细说明段落，包含：
- SQLite AUTOINCREMENT 与 PostgreSQL SERIAL 的三维度语义对比表（ID 重用策略、严格单调递增、回滚行为）
- 关键相似点分析：本项目所有 ID 均由数据库自动生成，正常使用场景下两者行为等价
- 三点结论：ID 排序推断时间（两个数据库均成立）、无间隙假设（两者均不成立，与迁移无关）、业务语义安全声明
- Phase 1 验证建议（序列递增行为测试 SQL）

在 10.2 节翻译规则表中为 `AUTOINCREMENT → SERIAL` 行增加语义差异注释（标注 v11 补充），指向 4.1 节详细分析。

在第 15 节风险表中新增"SQLite AUTOINCREMENT 与 PostgreSQL SERIAL 的语义差异未讨论"风险项。

### 三、轻微问题修复（3 个）

**修订 R5：连接池大小确定方法（问题 5）**

在 7.2.3 节新增"连接池大小确定方法"子节，包含：
- 推荐简化公式（含各参数定义）
- 中小型应用示例计算（`max=10` 的推导过程）
- 安全默认值的三点依据（单线程事件循环、连接复用、行业经验）
- 精确确定方法（Phase 1 压测 + pool 指标监控，含 5 步操作流程）
- 连接池指标监控集成建议
- 事务内连接的特殊考虑（长事务对 pool max 的影响分析）

在第 15 节风险表中新增"连接池大小确定方法缺失"风险项。

**修订 R6：文件名与内容版本号矛盾（问题 6）**

此问题在内部审议报告中已标识为已知问题。本轮修订将文档标题从"（v10）"更新为"（v11）"。文件名命名约定（`a_v[N]_copy_from_v[N-1].md`）维持现有模式——文件名中的序号对应"第几轮迭代的产出"，而非内容版本号。内容版本号（v11）反映的是该产出经历的修订轮次。此约定已在前期迭代中形成惯例，各参与方已知晓。

**修订 R7：`query_table` WHERE 子句注入路径安全评估（问题 7）**

在 9.2 节 `query_table` 风险说明中新增"`query_table` 安全维度说明"段落（标注 v11 新增），包含：
- 间接 SQL 注入攻击面分析：Dify AI 若被恶意 prompt 操纵，生成的 WHERE 子句可能包含注入代码，绕过参数化查询保护
- 当前缓解措施局限性：仅做日志记录（事后审计），缺乏事前防护
- Phase 2+ 防护建议：(1) Dify system prompt 安全约束；(2) 应用层 WHERE 子句安全校验（`node-sql-parser` 解析 + 条件表达式白名单检测）

在第 15 节风险表中新增"`query_table` WHERE 子句 SQL 注入路径未列入风险表"风险项。

### 四、配套更新

- 第 3.4.8 节：新增 timestamp 类型解析器配置子节
- 第 3.4.9 节：新增 `close()` 方法实现子节
- 第 3.5.1 节：新增优雅关闭处理段落
- 第 4.1 节差异清单：新增自增主键语义差异行和 v11 详细说明段落
- 第 4.2.1 节：新增 `dateRange.js` 兼容性评估子节
- 第 7.2.3 节：新增连接池大小确定方法子节
- 第 9.2 节：新增 `query_table` 安全维度说明段落
- 第 10.2 节翻译规则：`AUTOINCREMENT → SERIAL` 行增加语义差异注释
- 第 13.3 节运维维度表：新增优雅关闭条目
- 第 14 节前端确认：新增 timestamp 格式一致性保障说明
- 第 15 节风险表：新增 6 个风险项（timestamp 解析、close/优雅关闭、dateRange 兼容性、AUTOINCREMENT/SERIAL 语义、连接池大小、query_table 注入路径）
- 第 16 节文件变更清单：更新 KingbaseAdapter.js 和 server.js 条目

### 五、版本标记更新

- 文档标题从"（v10）"更新为"（v11）"
- 所有新增和修正内容标注"v11 新增"或"v11 补充"标记，便于审阅者定位本轮变更

### 六、v12 修订（本轮）

本轮修订基于技术方案审查报告（`a_v9_review_v1.md`，REJECTED）的审查反馈，解决 1 个一般性问题。

**修订 R12：3.4.8 节 `pg.types.setTypeParser` 中 timestamptz 的 OID 值错误（审查结论：REJECTED → 修改后 RE-REVIEW）**

审查发现：方案 3.4.8 节代码示例使用 OID `1182` 注册 timestamptz 类型的解析器，但标准 PostgreSQL 12（KingbaseES V8R6 内核基础）中 timestamptz 的 OID 为 **1184**。若实现者按代码示例使用 1182，`setTypeParser` 将注册到一个不正确或不存在的类型上，timestamptz 列的格式一致性修复失效，违背"前端代码零变动"核心承诺。

修订内容：
- 代码示例中 `pg.types.setTypeParser(1182, ...)` 改为 `pg.types.setTypeParser(1184, ...)`，使用 PostgreSQL 12 标准 OID
- 同时注册 OID 1182 作为 KingbaseES 备用兼容 OID（`setTypeParser` 对无效 OID 注册不产生副作用，多次注册以最后一次为准）
- 删除原注释"实际 KingbaseES 通常使用 1184"（既然已使用 1184，此注释冗余）
- 更新注意事项：增加 OID 值说明（1184 为标准 OID、1182 为兼容备用），增加启动验证推荐（打印实际激活的 OID 值）
- 第 15 节风险表和修订说明中同步修正 1182 → 1184

**影响范围**：仅 3.4.8 节代码示例、注意事项、风险表缓解措施描述和修订说明。方案其余部分无变更。

**配套更新**：
- 第 15 节风险表：timestamp 类型自动解析风险项的缓解措施列同步更新 OID 值
- 第 17 节修订说明 R1：同步更新 OID 值
- 新增第 17 节"六、v12 修订（本轮）"段落
...
本轮修订基于技术方案审查报告（`a_v9_review_v2.md`，**REJECTED**）的审查反馈，解决 1 个一般性问题：KingbaseES `CURRENT_TIMESTAMP` 时区依赖未讨论，统一 UTC 存储承诺无法保证。

### 一、问题修复（1 个）

**修订 R1：KingbaseES `CURRENT_TIMESTAMP` 在非 UTC 时区下返回本地时间，与方案"统一 UTC 存储"核心承诺矛盾**

审查发现：方案将 `sql.now()` 统一输出 `CURRENT_TIMESTAMP` 作为关键简化决策（第 4.2 节），并以此为基石推导出整个 UTC 存储策略。但方案全文始终隐含假设 `CURRENT_TIMESTAMP` 在 KingbaseES 中返回 UTC——而事实上，PostgreSQL/KingbaseES 的 `CURRENT_TIMESTAMP` 返回的是**服务器时区（`timezone` 参数）下的当前事务开始时间**。若 KingbaseES 服务器时区配置为 `Asia/Shanghai`（UTC+8，国产数据库在中国部署场景下的常见默认值），`CURRENT_TIMESTAMP` 将返回 UTC+8 时间，与 SQLite 的 `CURRENT_TIMESTAMP`（始终返回 UTC）相差 8 小时。

此遗漏直接影响：(1) Phase 1 下 KingbaseES 新写入数据可能为本地时间而非 UTC；(2) Phase 2 迁移后数据库内同时存在 UTC（迁移来的旧数据）和本地时间（`CURRENT_TIMESTAMP` 新数据），重新引入"混合时间戳"问题；(3) `sql.formatDateParam()` 输出的 UTC 参数字符串与数据库列中存储的本地时间字符串在比较时产生 8 小时偏差。实现者若不加验证地按方案实施，部署到默认时区为 Asia/Shanghai 的 KingbaseES 实例时，时间数据将全部偏移 8 小时。

**修订内容**：

1. **第 4.2 节新增"KingbaseES 服务器时区配置要求"子段**（标注 v13 新增）：
   - 明确说明 `CURRENT_TIMESTAMP` 在 KingbaseES 中返回服务器时区时间的机制
   - 分析三条核心路径的直接影响
   - 强制要求 KingbaseES 服务器 `timezone = 'UTC'`
   - 给出三种配置方式：配置文件（`kingbase.conf`）、Docker 容器（`TZ=UTC`）、连接字符串（辅助手段）
   - 给出验证方法（`SHOW timezone;` / `SELECT current_setting('timezone');`）
   - 指向 3.4.10 节的启动时区验证

2. **第 3.4.10 节新增"KingbaseES 服务器时区验证"子节**（标注 v13 新增）：
   - 验证时机：`init()` 方法事务提交后、返回前
   - 完整验证实现伪代码（含 `current_setting('timezone')` 查询 + UTC 检查 + 日志输出）
   - 设计要点：不阻塞启动（仅输出警告）、UTC 等价表示处理（`Etc/UTC`）、验证失败的降级策略

3. **第 5.1 节 CI/Docker 部署配置**（标注 v13 新增）：
   - CI 中的 KingbaseES 服务容器 YAML 配置新增 `TZ: UTC` 环境变量
   - Phase 1 部署步骤中的 Docker run 命令新增 `-e TZ=UTC` 参数，并添加注释指向 4.2 节说明

4. **第 15 节风险表新增风险项**（标注 v13 新增）：
   - 风险描述：KingbaseES 服务器时区非 UTC 导致 `CURRENT_TIMESTAMP` 输出本地时间
   - 影响三个维度：(1) Phase 1 新数据非 UTC；(2) Phase 2 迁移后混合时间戳；(3) `sql.formatDateParam()` 比较偏差
   - 缓解措施四条：(1) 服务器 `timezone=UTC`；(2) 启动时区验证；(3) CI/Docker `TZ=UTC`；(4) Phase 1 验收确认时区

**影响范围**：第 4.2 节（新增"KingbaseES 服务器时区配置要求"子段）、第 3.4.10 节（新增"KingbaseES 服务器时区验证"子节）、第 5.1 节（Docker 配置新增 `TZ=UTC`）、第 15 节（风险表新增 1 行）。方案其余部分无变更。

### 二、版本标记更新

- 文档标题从"（v12）"更新为"（v13）"
- 所有新增内容标注"v13 新增"标记，便于审阅者定位本轮变更

---

## 修订说明（v13 → v14）

本轮修订基于第 9 轮诊断报告（`b_v9_diag_v1.md`）经质询确认（LOCATED）的审查反馈，共解决全部 9 个问题（2 个严重、4 个一般、3 个轻微）。其中问题 1-6 为横跨多轮的持续性问题（自 v9/v10 起反复出现），问题 7-9 为本轮新识别问题。重点覆盖：前端时区转换矛盾消除、DDL 事务执行策略内部一致性、DDL 翻译产出物可实施性、逆向迁移完备性、驱动兼容性验证、性能验收量化和运维生命周期管理。

### 一、严重问题修复（2 个）

**修订 R1：前端时区转换逻辑矛盾——"前端需负责时区转换显示"与"前端代码无需任何修改"不可同时成立（问题 1，持续性问题：第 9 轮问题 1）**

此问题自 v9 首次识别后在 v10-v13 修订中仍未消除，是本轮最关键的修复项。核心矛盾：第 6 节 Phase 0 声明"前端需负责时区转换显示"，第 14 节断言"前端代码无需任何修改"——若前端代码零改动，时间显示将全部偏差 8 小时，与 Phase 0 验收标准第 6 条直接冲突。

修改内容：
1. **第 14 节"前端确认"重写**：将"前端代码无需任何修改"修正为"前端 API 调用代码（请求路径、请求方法、请求体结构、响应体解析）无需修改"。新增"需修改"子节——时间显示层时区转换：新建 `src/utils/timezone.js`（导出 `convertUTCToLocal(utcStr)` 函数），所有展示 datetime 字段的 Vue 组件引入并调用该函数将 UTC 转换为用户本地时区。给出 `convertUTCToLocal` 的实现轮廓（解析 UTC 字符串 → Date 对象 → 本地时间格式化）。明确标注 API 调用代码不受影响。
2. **第 6 节 Phase 0 关键行为变更**：将"前端需负责时区转换显示"扩展为包含具体实现指引的完整描述（新建 `timezone.js` + Vue 组件改造）。
3. **第 6 节 Phase 0 验收标准第 6 条**：从"前端展示应转换为本地时间"扩展为三个具体验证要点：(a) `timezone.js` 已新建且导出函数；(b) 所有展示 datetime 的 Vue 组件已引入；(c) 页面渲染的时间字符串为用户本地时区。
4. **第 16 节文件变更清单**：新增 `src/utils/timezone.js`（新建）和 `src/`（前端 Vue 组件改造）两个条目；将原"前端零改动"修正为"API 调用代码零改动，时间显示组件需改造"。
5. **第 15 节风险表**：新增"前端时区转换需求的工程范围低估"风险项。

**修订 R2：`init()` 方法的 DDL 执行策略在两个段落中相互矛盾（问题 2，持续性问题：第 9 轮问题 2）**

此问题自 v9 首次识别后仍未解决——步骤 6 要求"在一个事务内顺序执行所有语句"，同时"事务内 DDL 兼容性验证"要求"DDL 不在事务中执行"。方案将拆分方案提升为首推但步骤 1-7 描述未同步更新。

修改内容：
1. **第 3.4.5 节完全重写**：以拆分方案（DDL + 种子数据两文件）为唯一实施路径，彻底消除内部矛盾：
   - DDL 文件（`init_kingbase_ddl.sql`）：事务外逐条执行，幂等性由 `CREATE TABLE IF NOT EXISTS` 保证。若某条 DDL 失败，记录日志并继续执行后续 DDL
   - 种子数据文件（`init_kingbase_seed.sql`）：事务内执行（`BEGIN → 逐条 INSERT → COMMIT`），保证多表种子数据原子性。事务开始前先检查 `users` 表是否为空
   - 原单文件事务内执行方案标注为"DEPRECATED, v14 废弃"，不再作为可选实施路径
   - 新增"设计理由"子节（4 点解释）
2. **第 16 节文件变更清单**：`init_kingbase.sql` 条目拆分为 `init_kingbase_ddl.sql` 和 `init_kingbase_seed.sql` 两个条目。
3. **第 11.4 节数据库初始化流程**：KingbaseES 分支更新为拆分两文件执行流程。

### 二、一般问题修复（4 个）

**修订 R3：`init_kingbase.sql` 重写方案缺少具体产出物（问题 3，持续性问题：第 9 轮问题 3）**

此问题连续两轮未解决——第 10 节列出了 22 项 DDL 差异和翻译规则，但未提供重写后的实际内容或关键表的完整 DDL 示例。手工翻译 10 张表的 22 项差异出错概率高。

修改内容：
1. **第 10.2 节新增"关键表完整 DDL 参考示例"**：提供 4 张关键表（users、life_plans、punch_in、user_risk_info）的完整 KingbaseES DDL 示例（含完整的 CREATE TABLE、约束、索引、COMMENT），作为 `init_kingbase_ddl.sql` 重写的参考基准。实现者可直接复制使用。
2. **第 10.2 节新增"DDL 实现清单"**：将 10.1 节差异分析表中的翻译规则转化为 27 项可逐项勾选的实现清单（每项含编号、表名、差异项、翻译操作、勾选框），实现者对照此清单逐项翻译并勾选确认。

**修订 R4：Phase 2 数据回退的逆向迁移路径不完整（问题 4，持续性问题：第 9 轮问题 4）**

此问题连续两轮未解决——第 12 节提到 `scripts/migrate-reverse-to-sqlite.js` 但仅有框架性描述。逆向时区转换 SQL 未给出、SERIAL 序列 ID 冲突处理未讨论、JSONB→TEXT 逆向转换方案缺失、增量 vs 全量回退策略未决策。

修改内容：
1. **第 12 节新增"逆向迁移核心实现策略"子节**（标注 v14 新增），包含 6 个维度：
   - **时区反向转换**：UTC → 本地时间 +8h，给出完整转换代码示例。明确使用固定 `+08:00` 偏移确保与正向迁移数学对称
   - **SERIAL 序列 ID 冲突处理**：分析两种场景（全量覆盖 vs 增量合并），推荐方案 A（全量覆盖——清空 SQLite 表后全量写入，无 ID 冲突风险）
   - **JSONB → TEXT 逆向转换**：给出 `JSON.stringify()` 序列化方案，讨论 pg 驱动对 JSONB 的自动解析行为及 `setTypeParser(3802)` 的兼容注册建议
   - **增量 vs 全量回退策略**：两种策略的对比表（适用场景、停机时间），推荐全量回退
   - **逆向迁移验证策略**：参照正向迁移的 7 维度验证清单
   - **脚本执行时机**：dry-run 预演 → 正式回退执行
2. **第 16 节文件变更清单**：更新 `scripts/migrate-reverse-to-sqlite.js` 条目描述（从"框架"改为含完整实现策略的脚本）。

**修订 R5：`pg` 驱动版本与 KingbaseES V8R6 的兼容性缺乏验证声明（问题 5，持续性问题：第 9 轮问题 5）**

此问题连续两轮未解决——方案推荐 pg v8.12 但未声明是否已在实际 KingbaseES V8R6 实例上验证过核心功能。多处技术判断建立在"PostgreSQL 12 行为"假设上，但 KingbaseES V8R6 的国产化分支可能有特定修改。

修改内容：
1. **第 6 节 Phase 1 新增"前置条件——pg 驱动兼容性验证"**：Phase 1 实现开始前必须验证 6 个核心功能（TCP 连接认证、参数化查询、事务、连接池、数据类型映射、timestamp 解析器拦截）。编写 `scripts/verify-pg-kingbase.js` 验证脚本，全部通过后输出 `[PASS]`。任何失败则阻塞 KingbaseAdapter 实现。
2. **第 16 节文件变更清单**：新增 `scripts/verify-pg-kingbase.js` 条目。
3. **第 15 节风险表**：新增"pg 驱动版本与 KingbaseES V8R6 的兼容性缺乏验证声明"风险项。

**修订 R6：Phase 1 性能基准对比缺少负载测试方法论，验收标准不可量化（问题 6，持续性问题：第 9 轮问题 6）**

此问题连续两轮未解决——并发级别、测试数据量、绝对时间上限均未定义。Phase 1 验收标准仅给出 SQLite 相对值（2x），未给出绝对时间上限。

修改内容：
1. **第 6 节 Phase 1 验收标准第 5 条完整重写**：
   - **测试数据量**：SQLite 中预填充至少 1,000 条 users + 10,000 条 punch_in + 5,000 条 life_plans 记录（合计约 16,000 行）
   - **并发模型**：10 并发连接，持续 30 秒压测每个关键端点
   - **绝对时间上限表**：6 个关键端点的 SQLite P95 基线、KingbaseES P95 上限（`/api/plan/generate` < 2000ms、`/api/risk/history` < 200ms、`/api/health` < 50ms 等）
   - **绝对上限优先于相对基准**：任何端点 P95 不超过 2000ms，若 SQLite P95 已接近上限则 KingbaseES 不超过同值即可
   - **性能测试脚本**：新建 `scripts/perf-benchmark.sh` 自动化执行并生成 JSON 报告
2. **第 16 节文件变更清单**：新增 `scripts/perf-benchmark.sh` 条目。

### 三、轻微问题修复（3 个）

**修订 R7：`node-sql-parser` 解析失败回退到正则匹配时缺乏可观测性（问题 7，本轮新识别）**

问题：第 3.4.4 节 execute() 的 INSERT ID 获取策略在 node-sql-parser 解析失败时回退到正则匹配，但未要求输出日志。运维人员无法知晓生产环境中是否存在因解析失败而走了回退路径的 SQL。

修改内容：
1. **第 3.4.4 节 `execute()` INSERT ID 获取策略**：新增"回退日志"段落——当回退到正则匹配时，使用 `console.warn('[KingbaseAdapter] node-sql-parser 解析 INSERT 失败，回退到正则匹配。SQL: ' + sql.substring(0, 200))` 输出警告日志。说明日志的运维价值（监控回退频率、调查未覆盖的 SQL 模式）。

**修订 R8：文档版本号体系造成理解障碍（问题 8，本轮新识别）**

问题：文件名 `a_v10_copy_from_v9.md`（第 10 轮迭代的技术文档第 1 版）vs 文档标题"技术方案（v13）"（累计 13 次修订），使用两种不同的版本计数体系，对首次接触文档体系的实现者造成困惑。

修改内容：
1. **文档标题下方新增"版本声明"引用块**（标注 v14 新增）：说明文件命名约定（文件名序号对应迭代轮次，标题版本号对应修订次数），明确告诉实现者以标题中的版本号（v14）为准。

**修订 R9：Phase 2 切换后 SQLite 数据库文件的生命周期管理缺失（问题 9，本轮新识别）**

问题：Phase 2 切换至 KingbaseES 后，SQLite 文件是否应设为只读、回退验证的最小范围、Phase 3 移除 SQLite 支持时的归档策略均未定义。

修改内容：
1. **第 6 节 Phase 2 新增"SQLite 数据库文件生命周期管理"子节**：包含切换后立即设为只读（`chmod 444`）及其防止误写入/误启动的风险说明、回退前恢复可写、回退验证最小范围（6 个核心端点）。
2. **第 6 节 Phase 2 回滚步骤**：扩展为 5 步（含恢复可写和回退验证）。
3. **第 6 节 Phase 3 新增"SQLite 文件归档策略"子节**：压缩打包归档、保留期 >= 6 个月、归档目的（合规审计 + 灾难恢复最后备份）、归档后删除原文件。

### 四、配套更新

- 第 3.4.4 节：新增 node-sql-parser 回退日志策略
- 第 3.4.5 节：init() 方法完全重写为拆分 DDL/种子文件方案
- 第 4.2 节：无变更（涉及 UTC/时区讨论已在 v13 修订中完善）
- 第 6 节 Phase 0：更新前端时区转换验收标准
- 第 6 节 Phase 1：新增 pg 兼容性验证前置条件；重写性能基准验收标准
- 第 6 节 Phase 2：新增 SQLite 文件生命周期管理；扩展回滚步骤
- 第 6 节 Phase 3：新增 SQLite 文件归档策略
- 第 10.2 节：新增 4 张关键表完整 DDL 示例 + 27 项可勾选实现清单
- 第 11.4 节：KingbaseES 初始化流程更新为拆分两文件执行
- 第 12 节：新增逆向迁移核心实现策略（6 个维度）
- 第 14 节：前端确认重写（区分 API 调用代码不变 / 时间显示层需改造）
- 第 15 节风险表：新增 2 个风险项（pg 驱动兼容性、前端时区转换工程范围）
- 第 16 节文件变更清单：新增 4 个条目（`src/utils/timezone.js`、`src/` Vue 组件改造、`scripts/verify-pg-kingbase.js`、`scripts/perf-benchmark.sh`）；拆分 `init_kingbase.sql` 为两个文件条目；更新 KingbaseAdapter.js、`migrate-reverse-to-sqlite.js` 条目描述

### 五、版本标记更新

- 文档标题从"（v13）"更新为"（v14）"
- 所有新增和修正内容标注"v14 新增"、"v14 修正"或"v14 修订"标记

---

## 修订说明（v14 → v15）

本轮修订基于第 10 轮组件 B 诊断报告（`b_v10_diag_v1.md`）经质询确认（LOCATED）的审查反馈，共解决全部 8 个问题（2 个严重、4 个一般、2 个轻微）及质询报告提出的附加建议。其中问题 2-8 为横跨两轮的持续性问题（自 v10 起反复出现），问题 1 为本轮新识别问题。

### 一、严重问题修复（2 个）

**修订 R1：适配层改造路径文档完整性缺失——路由文件间无数据库访问耦合声明（问题 1）**

问题：第 3.5.2 节 Phase 0 过渡策略步骤 4 描述"逐文件改造路由层后立即重启应用并自测"，但未讨论路由文件改造后 require 路径变更问题。经分析确认各路由文件之间无数据库访问的模块间依赖，此为文档完整性缺失而非实际工程障碍。

修改内容：
1. 第 3.5.2 节步骤 4 表格单元格描述中增加"各路由文件之间无数据库访问的模块间依赖，逐文件改造不会产生跨文件适配不一致"的明确声明（标注 v15 新增）。

**修订 R2：事实错误——引用不存在的文件 `schema.adapter.js`（问题 2，持续性问题：第 2 次出现）**

问题：第 3.6 节 INSERT 操作返回值段落引用了项目中不存在的文件名 `schema.adapter.js`。

修改内容：
1. 第 3.6 节（原第 583 行）：将 `schema.adapter.js` 修正为 `sql.js`（`server/db/sql.js`），并标注该文件为本方案规划新建的方言辅助模块文件，其定义见第 4 节。

### 二、一般问题修复（4 个）

**修订 R3：事实错误——`server.js` 行号引用错误（问题 3，持续性问题：第 2 次出现）**

问题：第 3.5.1 节末尾和修订说明中称导入语句在"第 375 行"，但 `server.js` 实际仅 17 行，导入语句位于第 2 行。

修改内容：
1. 第 3.5.1 节"`db` 导出消费变化确认"段落：将"第 375 行"修正为"第 2 行"。
2. 修订说明（v10 → v11）中对应描述：将"第 375 行"修正为"第 2 行"。

**修订 R4：逻辑矛盾——database.js 改造后导出轮廓与过渡期实际导出不一致（问题 4，持续性问题：第 2 次出现）**

问题：第 3.5 节给出了最终状态导出轮廓（仅 `getAdapter`），但第 3.5.2 节的 Phase 0 双导出过渡方案要求同时导出 `db` 和 `getAdapter`。第 3.5 节未标注"此为 Phase 0 完成后的最终状态"。

修改内容：
1. 第 3.5 节代码轮廓上方增加引用块标注（标注 v15 新增）：明确说明以下为 Phase 0 完成后的最终导出状态，Phase 0 过渡期间的导出状态见 3.5.2 节。

**修订 R5：旧 `init_kingbase.sql` 文件处理策略遗漏（问题 5，持续性问题：第 2 次出现）**

问题：方案将 KingbaseES 初始化拆分为两个新文件（`init_kingbase_ddl.sql` 和 `init_kingbase_seed.sql`），但未说明旧 `init_kingbase.sql` 文件是删除、归档还是保留。

修改内容：
1. 第 10 节开头新增引用块（标注 v15 新增）：说明自 v14 起原文件已拆分，实现时应删除或归档，指向第 16 节。
2. 第 16 节文件变更清单：在 `init_kingbase_seed.sql` 条目后新增"删除 `server/db/init_kingbase.sql`"条目。

**修订 R6：`scripts/` 目录创建前提未声明（问题 6，持续性问题：第 2 次出现）**

问题：方案第 16 节列出了约 10 个新建脚本文件，但项目当前不存在 `scripts/` 目录，方案未说明需先创建此目录。

修改内容：
1. 第 16 节文件变更清单表格上方新增引用块（标注 v15 新增）：说明项目当前不存在 `scripts/` 目录，实现前需先创建（`mkdir -p scripts/`）。

### 三、轻微问题修复（2 个）

**修订 R7：admin `/execute` 禁用 `sql` 模式的代码片段中 `error` 函数导入来源未说明（问题 7，持续性问题：第 2 次出现）**

问题：第 9.2 节代码片段使用了 `error(res, ...)` 但未说明 `error` 函数的导入来源。

修改内容：
1. 第 9.2 节代码片段上方新增引用块（标注 v15 新增）：说明 `error` 函数导入自 `server/utils/response.js`。

**修订 R8：文档多处近似数量表述与第 16 节精确清单不一致（问题 8，持续性问题：第 2 次出现）**

问题：第 3.6 节使用"约 13 个文件"的近似表述，与第 16 节精确文件变更清单不一致。

修改内容：
1. 第 3.6 节开头：将"约 13 个文件，其中 11 个涉及数据库访问"修正为"13 个文件：12 个需改造 + 1 个不变，其中 11 个涉及数据库访问"。

### 四、质询报告附加建议处理

1. **问题 1 严重程度定级**：诊断报告自身分析结论为"文档完整性缺失而非实际工程障碍"，本轮按"一般"问题处理，修订中保留原分析结论但补充了明确声明。

2. **问题 2 改进建议采纳**：修正 `sql.js` 引用时同时标注该文件为方案规划新建的文件（`server/db/sql.js`），其定义见第 4 节，避免实现者因找不到文件而困惑。

3. **第 16 节文件变更清单系统性交叉验证**：逐条核对方案各章节（第 3.4 节、第 3.5 节、第 3.6 节、第 9.2 节、第 10 节）提及的改造/新建/删除文件是否均在清单中出现。验证结论：本轮修订前的清单已覆盖所有提及文件，新增的 `init_kingbase.sql` 删除条目填补了唯一缺口。无遗漏、无冗余。

### 五、配套更新

- 第 3.5 节：新增 Phase 0 最终状态标注
- 第 3.5.1 节：修正 server.js 行号引用
- 第 3.5.2 节：步骤 4 增加路由文件间无耦合声明
- 第 3.6 节：修正文件引用路径（`schema.adapter.js` → `sql.js`）；修正近似数量表述
- 第 9.2 节：新增 `error` 函数导入来源说明
- 第 10 节：新增旧 `init_kingbase.sql` 文件处理说明
- 第 16 节：新增 `scripts/` 目录创建前提说明；新增旧 `init_kingbase.sql` 删除条目

### 六、版本标记更新

- 文档标题从"（v14）"更新为"（v15）"
- 所有新增和修正内容标注"v15 新增"或"v15 修正"标记

---

## 修订说明（v15 → v16）

本轮修订基于第 11 轮组件 B 诊断报告（`b_v11_diag_v2.md`）经质询确认（LOCATED）的审查反馈，共解决全部 10 个编号问题（2 个严重、6 个一般、2 个轻微）及横切关注点的 BOOLEAN 类型建议。

### 一、严重问题修复（2 个）

**修订 R1：JSONB 列的类型解析器未配置，导致 API 响应格式不一致（问题 1）**

方案 v11-v15 精细配置了 timestamp 类型的解析器（`pg.types.setTypeParser`）但遗漏了 JSONB（OID 3802）和 JSON（OID 114）类型。pg 驱动默认将 JSONB 列自动解析为 JS 对象，而 SQLite 后端以 TEXT 字符串形式返回。经代码路径核实：`articles.tags`（articles.js 使用 `JSON.parse` 解析）和 `life_advice.tags`（assistant.js 使用 `JSON.parse` 解析）将发生静默数据丢失——`JSON.parse` 接收对象参数时抛 TypeError 被 catch 返回空数组。此问题与已修复的 timestamp 格式差异同源——都是 pg 驱动默认类型解析导致，属于不完整的防御性设计。

修改内容：
1. **第 3.4.8 节扩展**：标题从"pg 驱动 timestamp 类型自动解析控制"扩展为"pg 驱动类型自动解析控制（timestamp + JSONB/JSON）"。在代码示例中新增 `pg.types.setTypeParser(3802, val => String(val))`（jsonb）和 `pg.types.setTypeParser(114, val => String(val))`（json）。补充 JSONB 自动解析导致静默数据丢失的完整影响分析。新增横切关注点——BOOLEAN 类型（OID 16）的 pg 驱动解析差异说明（DDL 保持 INTEGER/SMALLINT 类型，无需额外配置解析器）。
2. **第 14 节前端确认**：timestamp 格式一致性说明扩展至覆盖 JSONB/JSON 类型。

**修订 R2：`admin_logs.operation_content` 和 `admin_logs.operation_result` 被错误归类为 JSON 文本列（问题 2）**

方案第 10.2 节将两列与其他 4 个 JSON 文本列一并推荐使用 JSONB。经代码核实（admin.js `insertAdminLog` 函数第 147-156 行调用方传入值）：`operation_content` 存储纯文本操作描述（SQL 文本如 `"SELECT * FROM users..."`、工具名称描述）；`operation_result` 存储纯文本结果（如 `"成功"`、`"权限不足"`）。若按方案改为 JSONB，INSERT 传入纯文本（非合法 JSON）将抛出 `invalid input syntax for type json` 错误（SQLSTATE 22P02），导致所有管理员操作日志写入失败。

修改内容：
1. **第 10.1 节差异分析表**：JSON 列类型行从 6 列更正为 4 列，新增 v16 修正说明。
2. **第 10.2 节 JSON 列类型决策**：新增经代码核实的明确列名清单（4 列），将 `admin_logs.operation_content` 和 `admin_logs.operation_result` 排除在 JSONB 外，保持 TEXT/VARCHAR。
3. **第 10.2 节翻译规则表**：JSONB 行从 6 列更正为 4 列。
4. **第 10.2 节 GIN 索引 DDL 示例**：删除 `admin_logs.operation_content` 的 GIN 索引建议；新增 GIN 索引写入性能说明（标记为 Phase 1 性能基准对比后决定）；新增"推荐 Phase 1 暂不创建 GIN 索引"的策略。
5. **第 10.2 节 DDL 实现清单**：第 27 项从"全部表（6 个 JSON 列）"更正为"全部表（4 个 JSON 列）"。
6. **第 12 节数据迁移**：JSON 列合法性校验范围从 6 列更正为 4 列。
7. **第 12.1 节迁移验证**：JSON 字段有效性验证范围同步更正。
8. **第 15 节风险表**：新增独立风险项。

### 二、一般问题修复（6 个）

**修订 R3：版本声明元数据与文件名不一致（问题 3）**

修改内容：文档第 3 行版本声明中的文件引用名从 `a_v10_copy_from_v9.md`（v10 产出）修正为 `a_v12_copy_from_v11.md`（v12 产出）；文档标题从"（v15）"更新为"（v16）"；版本声明同步更新。

**修订 R4：OID 1182 timestamptz 解析器注册缺乏来源依据（问题 4）**

修改内容：在第 3.4.8 节删除 OID 1182 注册行（`pg.types.setTypeParser(1182, ...)`）。经核实——PostgreSQL 12 中 OID 1182 实际对应 `_date`（date 的数组类型），与 timestamptz 无关。新增"OID 1182 说明"段落，解释删除理由。代码示例仅保留 1114（timestamp）、1184（timestamptz）、3802（jsonb）、114（json）四种标准 OID。在第 15 节风险表中新增独立风险项。

**修订 R5：dateRange.js 兼容性结论以"已执行验证"口吻呈现但实际为理论推导（问题 5）**

修改内容：在第 4.2.1 节，将验证 SQL 从"已执行"口吻（"应成功返回"）修改为"待执行验证清单"口吻（"预期验证应通过"）。将验证 SQL 加入 Phase 1 前置验证脚本的额外验证项。在第 15 节风险表中更新对应风险项描述。

**修订 R6：DDL 执行"失败后继续"策略未考虑 FK 依赖的级联失败（问题 6）**

修改内容：在第 3.4.5 节 DDL 执行流程中新增步骤 4（DDL 文件按 FK 依赖拓扑顺序排列）；将原步骤 5 扩展为步骤 6，新增"根因 vs 级联失败区分"策略（`[CASCADED]` 日志前缀标注）和"首条 DDL 失败即终止"的保守策略推荐。在第 15 节风险表中新增独立风险项。

**修订 R7：`query_table` WHERE 子句安全风险缓解措施不足（问题 7）**

修改内容：在第 9.2 节 `query_table` 安全维度说明中，从"Phase 1 保持现状但记录到风险表"增强为"Phase 1 实现三层基础防护"：(1) 应用层 WHERE 子句安全校验（`node-sql-parser` AST 检查）；(2) Dify system prompt 安全约束；(3) Phase 1 验收安全测试用例。在第 15 节风险表中更新对应风险项缓解措施。

**修订 R8：GIN 索引写入性能开销未评估，且 operation_content 的 GIN 索引建议本身错误（问题 8）**

修改内容：在第 10.2 节 GIN 索引 DDL 示例中删除 `admin_logs.operation_content` 的 GIN 索引（联动问题 2）；新增"GIN 索引写入性能说明"段落（INSERT/UPDATE 写入延迟可能增加 20-50%）；将创建策略从"推荐先创建默认 GIN 索引"修改为"Phase 1 暂不创建 GIN 索引，性能基准对比中分别测试有/无 GIN 索引的写入和查询性能差异，根据实测数据决定"。

### 三、轻微问题修复（2 个）

**修订 R9：`sql.formatDateParam` 缺少完整代码示例（问题 9）**

修改内容：在第 4.2 节方言统一策略中，将"约 5 行"实现逻辑描述替换为约 8 行的完整实现轮廓代码（含 `getUTC*` 方法和 `padStart` 补零）。

**修订 R10：`server/routes/index.js` 在"不变的文件"段落中表述存在轻微歧义（问题 10）**

修改内容：在第 3.6 节，将 `index.js` 的说明从"不变的文件"段落中移出，改为独立的引用块。明确标注 `index.js` 不在"不变的文件"之列——其 `/health` 端点需改造。

### 四、横切关注点处理

**BOOLEAN 类型差异**：诊断报告第四节发现 pg 驱动对 BOOLEAN（OID 16）的默认解析差异（返回 JS boolean vs SQLite INTEGER 返回 JS number）。按诊断报告推荐——DDL 保持使用 INTEGER/SMALLINT 类型（避免翻译为 BOOLEAN），无需额外配置 OID 16 的解析器。在第 3.4.8 节新增"BOOLEAN 类型说明"段落。

### 五、配套更新

- 第 3.4.5 节：DDL 执行流程新增 FK 依赖拓扑顺序和级联失败处理策略
- 第 3.4.8 节：扩展为 timestamp + JSONB/JSON 类型解析控制（含 OID 1182 删除、BOOLEAN 类型说明）
- 第 3.6 节：index.js 从"不变的文件"段落中移出
- 第 4.2 节：sql.formatDateParam 增加完整代码轮廓
- 第 4.2.1 节：dateRange.js 验证 SQL 从"已执行"改为"待执行"口吻
- 第 9.2 节：query_table 安全防护从日志记录升级为三层防护（含安全测试用例）
- 第 10.1 节：差异分析表 JSON 列类型行修正（6→4 列）
- 第 10.2 节：JSON 列类型决策修正（含代码核实的列名清单）；GIN 索引 DDL 修正（删除 operation_content，新增写入性能说明，推荐 Phase 1 暂不创建）；翻译规则表 JSONB 行修正；DDL 实现清单第 27 项修正
- 第 12 节：数据迁移 JSON 列清单修正（6→4 列）
- 第 12.1 节：迁移验证 JSON 字段有效性范围修正
- 第 14 节：前端确认补充 JSONB/JSON 格式一致性保障
- 第 15 节风险表：更新 3 个已有风险项（timestamp/JSONB、dateRange.js、query_table）；新增 3 个风险项（JSONB 列类型误判、DDL FK 级联失败、OID 1182 缺乏来源）
- 第 16 节文件变更清单：KingbaseAdapter.js 条目扩展为含 JSONB/JSON 类型解析器

### 六、版本标记更新

- 文档标题从"（v15）"更新为"（v16）"
- 所有新增和修正内容标注"v16 新增"、"v16 扩展"、"v16 修正"或"v16 修订"标记
