...
**解决方案**：在 KingbaseAdapter 构造函数中，向 `pg.Pool` 注册自定义类型解析器，将 timestamp（OID 1114）和 timestamptz（OID 1182）类型的解析器设为原样返回字符串（`val => String(val)`），与 SQLite 行为保持一致：

```javascript
// KingbaseAdapter 构造函数内，pg.Pool 实例化之前
pg.types.setTypeParser(1114, val => String(val));  // timestamp → 保持字符串
pg.types.setTypeParser(1182, val => String(val));  // timestamptz → 保持字符串（实际 KingbaseES 通常使用 1184）
```
...
- KingbaseES V8R6 基于 PostgreSQL 12 内核，OID 定义与 PostgreSQL 一致，上述 OID 值可安全使用。若将来升级至不同大版本，建议在启动验证中执行 `SELECT oid, typname FROM pg_type WHERE typname IN ('timestamp','timestamptz')` 确认 OID 值
...
| **（v11 新增）pg 驱动 timestamp 类型自动解析导致 API 响应 JSON 格式不一致** | `pg` 驱动默认将 TIMESTAMP/TIMESTAMPTZ 列自动解析为 JavaScript Date 对象，JSON 序列化后变为 ISO 8601 字符串（如 `"2025-06-28T06:30:00.000Z"`），而 SQLite 路径下 timestamp 以纯文本形式返回（如 `"2025-06-28 06:30:00"`），同一 API 端点在两个后端下返回不同 JSON 格式，违背"前端代码零变动"的核心承诺 | 在 KingbaseAdapter 构造函数中通过 `pg.types.setTypeParser(1114, val => String(val))` 和 `pg.types.setTypeParser(1182, val => String(val))` 将 timestamp/timestamptz 类型的解析器设为原样返回字符串，与 SQLite 行为保持一致（见 3.4.8 节） |
...
- 解决方案：通过 `pg.types.setTypeParser(1114, val => String(val))` 和 `pg.types.setTypeParser(1182, val => String(val))` 将 timestamp/timestamptz 类型解析器设为原样返回字符串
