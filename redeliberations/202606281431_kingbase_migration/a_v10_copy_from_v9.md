...
**`db` 导出消费变化确认（v10 新增）**：当前 `server.js`（第 375 行）解构导入了 `{ initDatabase, db }`，但 `db` 未被 `server.js` 自身使用（`server.js` 不执行数据库查询，数据库访问全部在路由文件中）。改造后的解构 `{ initDatabase, getAdapter }` 删除 `db` 不会产生未使用变量错误，改造安全。
...
- 确认当前 `server.js`（第 375 行）解构导入 `{ initDatabase, db }`，但 `db` 未被 `server.js` 自身使用（数据库访问全部在路由文件中）
