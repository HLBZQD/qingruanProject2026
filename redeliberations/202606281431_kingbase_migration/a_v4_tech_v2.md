...
- **前置步骤**（v3 新增）：在验收测试开始前，运行一次性 SQL 脚本 `scripts/phase0_utc_convert.sql` 将现有 SQLite 数据中所有 datetime 字段从本地时间转换为 UTC（详见 4.2 节）。执行前必须备份 `database.sqlite`。
- **验收标准**：
  1. UTC 转换脚本执行成功（所有 datetime 字段已转换为 UTC，详见 4.2 节脚本）
  2. 所有现有 API 端点返回的 HTTP 状态码和响应结构与改造前一致
  3. 用户注册/登录流程正常
  4. 风险预测 → 方案生成 → 打卡记录完整流程正常
  5. 管理员日志记录和查询正常
  6. 科普文章 CRUD 正常
  7. 前端页面中显示的时间正确（数据库存储为 UTC，前端展示应转换为本地时间）
  8. 无功能回归
...
**Dify 端同步变更（v4 新增）**：禁用 `sql` 模式是服务端的单向决策——若 Dify AI 工作流的 system prompt 未同步更新，LLM 可能在 KingbaseES 环境下仍选择 `sql` 模式，导致用户看到"暂不支持"错误而非有意义的操作建议。需在 Dify 工作流侧同步变更：

1. **system prompt 注入 `db_type` 变量**：在 Dify 管理后台的工作流 system prompt 中注入 `db_type` 上下文变量（通过 API 调用时传入或工作流环境变量配置）。当 `db_type` 为 `kingbase` 时，prompt 应引导 LLM 优先使用 `tool_name` 模式，避免触发 `sql` 模式。示例 prompt 片段：
   ```
   当前数据库类型：{{db_type}}。
   {% if db_type == 'kingbase' %}
   注意：当前使用 KingbaseES 数据库，仅支持 tool_name 参数调用，不支持直接 SQL 执行。
   请优先使用 tool_name 参数，包括：query_user_profile, query_risk_history, ...
   {% endif %}
   ```
2. **端到端验收**：在 Phase 1 验收标准中增加 Dify admin 对话的端到端测试条目——覆盖"AI 在 KingbaseES 下不会尝试 sql 模式"的场景（见第 6 节 Phase 1 验收标准补充）
...
| 改造 | `server/routes/admin.js` | 同上 + `PRAGMA table_info` → `adapter.tableInfo()` + `db.transaction` → `adapter.transaction`；`/logs` handler 标记 async；`sql` 模式在 KingbaseES 下禁用；`insertAdminLog` 增加 `adapter` 参数（事务内传入 txAdapter）；`dispatchParameterizedQuery` 整体 async 改造（签名、内部调用、返回值） |
...
| 新建 | `scripts/phase0_utc_convert.sql` | Phase 0 一次性 UTC 转换脚本（现有 SQLite 数据 datetime 字段从本地时间转 UTC，v3 新增） |
