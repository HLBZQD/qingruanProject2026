...
- ❌ **步骤 5**（免责声明拒绝行为偏离）：设计定义"拒绝 → `next(false)` 返回上一页，无上一页则回 `/home`"；实际实现始终重定向至 `/home`（第 137 行 `next('/home')`）。差异影响：设计允许用户在拒绝免责声明后保留在原页面，实际强制回首页可能破坏用户导航意图（例如从 /profile/risk 进入被拦截，拒绝后本应回到 /profile，实际却到了 /home）。
...
| 免责声明拒绝行为偏离 | 低 | router/index.ts:137 | 设计定义 next(false) 保留来源页，实际重定向 /home |
...
- ❌ **注册表单完全缺失**：设计定义的注册表单（用户名 + 密码 + 确认密码 + 验证 + 提交注册）未实现。当前"立即注册"链接指向 `/login`（自环）。
  ...
  - 后端 `POST /api/auth/register` 已实现（`server/routes/auth.js:11-47`），注册成功返回 `{token, role, user}` 可直接自动登录（authStore.login 兼容该响应结构）。
...
- 📝 **Markdown 渲染统一化说明**：LifePlan.vue 是项目中唯一已全面使用 `useMarkdown.ts` composable 的页面组件（`renderMarkdown()` 含 marked 链接安全渲染器 + DOMPurify 白名单加固）。其他 5 个需要 Markdown 渲染的页面组件（DoctorChatView、ArticleDetailView、HealthAdvice、Admin、Risk）仍各自内联 `marked.parse() + DOMPurify.sanitize()`，未复用统一管道。此差异源于开发时序：LifePlan.vue 开发较晚，开发时 useMarkdown composable 已存在并被采用；其余页面在 useMarkdown 创建之前已完成开发，未做后续统一重构。功能层面等价，但存在代码重复和链接安全渲染器缺失（内联方案未注入 `rel="noopener noreferrer"`）。
...
| 待实现功能 | 前置依赖 | 说明 |
|-----------|---------|------|
| 用户注册前端 | useApi.ts（已存在）, Login.vue 现有结构 | 需在 Login.vue 添加注册表单视图（含视图切换逻辑），调用 POST /api/auth/register。后端返回 `{token, role, user}` 与 authStore.login() 的响应处理完全兼容，注册成功直接调用 authStore.setAuth 自动登录。**不需要** helpers.ts（A2）的日期格式化或防抖截流——注册表单无此类需要。**不需要** useAuth.ts（A4）——JWT 解析已由 authStore 完成。 |
| 文章收藏 | useArticleApi.ts（新增 collect/uncollect/collection list 函数）, ArticleDetailView.vue, NewsView.vue（可选） | 后端三个收藏端点均已实现（`server/routes/articles.js:31-38, 171-185`）。需在 useArticleApi.ts 中新增 3 个 fetch 函数并替换 ArticleDetailView.vue 的 toggleCollect 占位。 |
...
| JWT 过期检测（路由守卫） | useAuth composable（A4） | 路由守卫需引入 useAuth 进行 token exp 检查，当前仅检查 token 存在性（`router/index.ts:118`）。 |
...
| Admin.vue SSE 统一 | chatStore.sendAdminMessage（已存在） | Admin.vue 当前内联实现 SSE 解析逻辑（`Admin.vue:36-78`），与 chatStore 重复。chatStore 已暴露 sendAdminMessage 方法（含消息管理、isStreaming、error 处理、断线重连），Admin.vue 仅需切换调用入口并丢弃内联实现。**此任务独立于 A6（useSSE 抽取）**——chatStore.sendAdminMessage 已封装完整的 SSE 收发循环，Admin.vue 无需感知 useSSE composable 的存在。 |
| Markdown 渲染统一化 | useMarkdown.ts 已存在 | 将 DoctorChatView/ArticleDetailView/HealthAdvice/Admin/Risk 的内联 marked+DOMPurify 替换为 renderMarkdown() 调用，消除链接安全渲染器缺失（`rel="noopener noreferrer"`）。 |
| 路由守卫免责声明拒绝行为修复 | **无需前置依赖**（最简方案） | 最简方案（仅修改 `router/index.ts:137` 的 `next('/home')` 为 `next(false)` 语义，即：无来源页时回 `/home`，有来源页时停留在来源页）完全独立，无需等待 A5。最佳实践（将 disclaimer 函数移入 useUI）依赖 A5 完成后方可进行。 |

### 11.3 搜索功能设计分析

搜索功能当前状态：
- 入口：Home.vue 搜索栏触发 `onSearch()`（`Home.vue:82-93`），当前为占位 Toast
...
1. **推荐方案**：
   ...
   - API 策略：前端在搜索时全量拉取 `GET /api/articles`（取 pageSize=100），在本地进行标题/标签关键词匹配
   - 工作量：0.8-1.0d（含 NewsView.vue 搜索模式切换 + 本地过滤逻辑 + 关键词高亮）
2. **选择依据**：
   - 当前文章量级 ≤100 条，全量拉取网络开销 <50KB，本地过滤耗时 <5ms，无性能瓶颈
   - 无需新建后端端点，开发周期短，可快速上线验证搜索需求
   - 升级路径清晰：当文章量级 >200 条或用户反馈搜索结果不满足时，升级为后端全文搜索方案
3. **适用条件**：**仅适用于文章总量 < 200 条的阶段**。超过此量级，全量拉取的网络开销和本地过滤的 CPU 开销将显著增加，需升级为后端搜索方案。
4. **前置决策任务（纳入 Phase 1）**：搜索交互设计决策（0.2d）——产出搜索交互原型（NewsView.vue 搜索模式下的 UI 草稿 + 搜索状态转换图），明确搜索触发逻辑（回车/防抖/清除）和空结果/错误处理。此决策任务确保开发者在启动 C2 编码前对齐搜索交互预期，避免返工。
...
| A4 | `src/composables/useAuth.ts` | 0.3d | 从 authStore 抽取 JWT 工具函数（含过期检测） |
...
| A7 | Markdown 渲染统一化 | 0.3d | 将 DoctorChatView/ArticleDetailView/HealthAdvice/Admin/Risk 的内联 marked+DOMPurify 替换为 renderMarkdown() 调用。**低风险提示**：`renderMarkdown()` 额外注入 `rel="noopener noreferrer"` 到外部链接（`useMarkdown.ts:35-40`），此为安全改进，不改变内容结构，但建议在 5 个页面各做 1 次快速冒烟测试确认链接样式无异样。 |
| A8 | 路由守卫免责声明拒绝行为修复 | 0.1d | router/index.ts:137 修改为 next(false) 语义（最简方案独立于 A5）；最佳实践需 A5 完成后同步将 disclaimer 函数移入 useUI |

| A10 | 搜索功能设计决策 | 0.2d | 见 §11.3 推荐方案的「前置决策任务」：产出搜索交互原型（NewsView.vue 搜索模式下的 UI 草稿 + 搜索状态转换图），明确搜索触发逻辑和空结果/错误处理，确保 C2 编码前对齐交互预期 |

**依赖链**：A 组内无相互依赖，可全并行。A8 最简方案独立于 A5，最佳实践 A8→(A5)。A10 为 C2 前置，应在 Phase 1 内完成。**A 组总工期：0.5d**（取最长路径 A6）。

### Group B：认证与用户功能增强（依赖 A4，A5 非严格）

| 编号 | 模块 | 工作量 | 说明 |
|------|------|-------|------|
| B1 | Login.vue 注册表单 | 1.0d | 添加注册视图（Views 切换）+ 表单校验（用户名/密码/确认密码）+ 调用 POST /api/auth/register + 注册成功自动登录（authStore.setAuth）。后端已实现，响应与 authStore.login 兼容。**不依赖 A2/A4/A5**。 |
...
| 编号 | 模块 | 工作量 | 说明 |
|------|------|-------|------|
| C1 | 文章收藏功能 | 1.5d | 扩展 useArticleApi.ts 新增 collectArticle/uncollectArticle/getCollections 三个函数 + ArticleDetailView.vue toggleCollect 替换占位为真实交互 + NewsView.vue 列表收藏状态展示（可选）+ 收藏列表页（可选）。后端三个端点已全部实现。 |
| C2 | 搜索功能 | 1.0d | **推荐方案**：前端本地过滤（见 §11.3）。NewsView.vue 搜索模式（URL `?keyword=xxx` 驱动）+ 全量拉取文章（pageSize=100）后在本地进行标题/标签关键词匹配 + 关键词高亮。**适用条件**：文章总量 < 200 条阶段；超过此量级需升级为后端全文搜索方案。若采用备选后端搜索方案（新增 Express 路由 + 后端 SQLite LIKE），工作量约 1.5-2.0d。**前置依赖**：A10（搜索设计决策 0.2d，Phase 1 内完成）。 |

**依赖链**：C1 无前置依赖（useArticleApi.ts 的扩展不阻塞其他模块）；C2 前置依赖 A10（搜索设计决策，Phase 1 内完成）。C1/C2 可并行。**C 组总工期：1.5d**（取 C1 1.5d 与 C2 1.0d 中的最长路径 C1）。
...
| D3 | Admin.vue SSE 逻辑统一 | 0.2d | Admin.vue 废弃内联的 parseSSEBuffer/readSSEStream/dispatchSSEEvent（`Admin.vue:36-78`），改调 chatStore.sendAdminMessage（已完整实现 SSE 收发循环 + 消息管理）。**此任务独立于 A6**——chatStore.sendAdminMessage 是完成的功能接口，Admin.vue 只需切换调用入口。A6 完成后 chatStore 内部可改为使用 useSSE composable，但 Admin.vue 不感知此变化。**文件冲突提示**：D1（扩展 chatStore.ts）与 D3（修改 Admin.vue 调用 chatStore 的路径）均涉及 `chatStore.ts`。建议 D3 在 D1 的 chatStore 改动合入后再启动，或在人力分配中让同一人承担 D1 和 D3 以避免合并冲突。 |
| D4 | sendAdminChatMessage 重复定义去重 | 0.1d | `useAdminApi.ts:28-48` 与 `useChatApi.ts:97-118` 中的 `sendAdminChatMessage` 实现完全一致（函数签名和 fetch 逻辑均相同）。此任务严格依赖 D3 完成——D3 将 Admin.vue 迁移至 chatStore.sendAdminMessage 后，`useAdminApi.ts` 中的 `sendAdminChatMessage` 即成为死代码，方可移除。**不可在 Phase 1 独立执行**：`Admin.vue:8` 当前从 `@/composables/useAdminApi` 导入 `sendAdminChatMessage` 并在第 155 行实际调用，在 D3 迁移 Admin.vue 调用入口之前删除该函数将导致构建失败。执行方式：D3 完成后，移除 `useAdminApi.ts` 中的 `sendAdminChatMessage`，Admin.vue 改从 `chatStore` 导入。 |

**依赖链**：D1 无前置依赖（extension of useChatApi.ts is self-contained）；D2 在 chatStore 内闭环，无外部依赖；D3 建议在 D1 合入后启动（共享 chatStore.ts 修改，避免合并冲突）——或由同人承担 D1+D3 以消除冲突；D4 严格依赖 D3 完成（D3 将 Admin.vue 迁移到 chatStore.sendAdminMessage 后方可移除 useAdminApi.ts 中的重复定义，否则 Admin.vue:8 的导入将导致构建失败）。D1/D2 可并行，D3 与 D1 半并行（同人或先后执行）。**D 组总工期：1.0d**（取最长 D1）。

### Group E：pinia-plugin-persistedstate 迁移（决策前置任务）

| 编号 | 模块 | 工作量 | 说明 |
|------|------|-------|------|
| E1 | 持久化方案决策（已完成） | 0.1d | 本报告 §12 E1 决策分析框架已完整完成 sessionStorage+BroadcastChannel vs localStorage+pinia-plugin-persistedstate 的 5 维度对比分析，并给出明确推荐（保留当前方案）。此任务的增量工作仅为将已有分析格式化输出为独立决策文档（Markdown 格式）以供团队归档，工作量 0.1d。决策本身无需重新执行。 |
...
Phase 2 ──┐  B1 注册表单             (1.0d)
           ...
           ├  D1 会话历史             (1.0d)
           ├  D2 AI 导航              (0.3d)
            ├  D3 Admin SSE统一        (0.2d)  ← 建议D1合入后启动; D4严格依赖D3
            └  D4 sendAdminChatMessage去重(0.1d)  ← 严格依赖D3
               (可全并行，D3建议D1合入后)
Phase 3 ───  E1 持久化决策文档归档    (0.1d)  ← 格式化已有分析，非重新决策
               (独立任务，1人)
```
...
|------|---------|------|
| 人1 | C1 (1.5d) | 1.5d |
| 人2 | B1 (1.0d) + D2 (0.3d) + D3 (0.2d) | 1.5d |
| 人3 | D1 (1.0d) + B2 (0.5d) + B3 (0.2d) | 1.7d |

C2（1.0d）无法在 3 人内消化。若 C2 由人1 承担（C1 1.5d + C2 1.0d = 2.5d），人1 路径延长至 2.5d；或增派第 4 人。

**人力与工期**：
- **3 人方案**：Phase 1 (0.5d) + Phase 2 (≥2.5d) + Phase 3 (0.1d) ≈ **3.1d**（C2 单列排队）
- **4 人方案**：Phase 1 (0.5d) + Phase 2 (1.7d) + Phase 3 (0.1d) ≈ **2.3d**（C2 由人4并行承担 1.0d）
- **推荐 4 人方案**：4 人可使 Phase 2 工期控制在 1.7d 以内，总工期 ~2.3d

> **注意**：以上为纯前端开发工时，不含联调、测试时间。C2（搜索功能）工作量基于最简前端方案（1.0d），前置决策任务 A10（0.2d）已纳入 Phase 1。若采用备选后端搜索方案，C2 工作量增至 1.5-2.0d，Phase 2 工期需相应延长。D3 建议在 D1 合入后启动以避免 chatStore.ts 合并冲突。
...
| 路由守卫免责声明拒绝行为 | next(false) 保留来源页 | 始终重定向 /home（`router/index.ts:137`） | router/index.ts:137 | 修复为 next(false) 语义（第12节 A8） |
...
- **P1（影响安全性或高级体验）**：安全性加固项或高级功能增强，当前有替代机制兜底（如 401 拦截兜底 JWT 过期检测缺失、用户可通过手动清除 session 绕过部分问题）。
- **P2（架构合规/体验优化）**：功能可用但存在代码重复、架构偏离或用户体验不完整，不影响核心使用流程。
- **P3（清洁工/文档对齐）**：次要修复或文档级对齐，不影响功能或用户体验。

| 优先级 | 功能 | 根因位置 | 影响页面 | 后端状态 |
|--------|------|---------|---------|:--------:|
| P0 | 用户注册 | `Login.vue:76` 自环 `<router-link to="/login">` | Login | ✅ 已实现 |
| P0 | **Login.vue 死链接「立即注册」** | `Login.vue:76` — 用户点击后停留在登录页面且无任何反馈，这是独立于注册功能实现的用户可感知缺陷。建议在注册功能实现前将链接替换为 Toast 提示或改为非链接样式 | Login | — |
...
| P1 | 会话历史加载 | useChatApi 缺少 conversations 端点函数 | DoctorChatView | ✅ 已实现 |
| P1 | AI 助手导航（navigate） | `chatStore.ts:716-718` 空函数体 | AiChatDialog | — |
| P2 | 编辑资料 | `Profile.vue:108-109` 占位 Toast | Profile | ✅ 已实现 |
| P2 | 搜索功能 | `Home.vue:82-93` 占位 Toast + 后端无搜索端点 + 设计文档未定义 | Home | ❌ 无端点 |
...
| P2 | sendAdminChatMessage 重复定义 | `useAdminApi.ts:28-48` 与 `useChatApi.ts:97-118` 实现完全一致（函数签名和 fetch 逻辑均相同）。D3 将 Admin.vue 迁移后，useAdminApi.ts 版本成为死代码，应一并清理 | useAdminApi.ts, useChatApi.ts | — |
| P3 | 免责声明拒绝行为修复 | `router/index.ts:137` next('/home') → 应为 next(false) 语义 | 全局 | — |
...
| 质询意见 | 回应 |
|---------|------|
| **问题1**：§11.2 与 §12 依赖声明不一致——B1 注册表单在 §11.2 标记依赖 useApi.ts + Login.vue 现有结构，但在 §12 Group B 声明 B1→(A2, A4)，即额外依赖 helpers.ts 和 useAuth.ts。B2 编辑资料在 §11.2 标记依赖 useUserApi.ts + Profile.vue，但 §12 声明 B2→(A5)。注册表单仅需 POST /api/auth/register 后由 authStore 处理 token，无需 A2/A4；编辑资料可直接沿用 Profile.vue 已有的 SweetAlert2 交互模式，无需 A5。 | **已修正**。统一两处依赖声明。(1) §11.2 中 B1 依赖保持"useApi.ts（已存在）, Login.vue 现有结构"，明确标注"不需要 helpers.ts 的日期格式化或防抖截流——注册表单无此类需要；不需要 useAuth.ts——JWT 解析已由 authStore 完成"。(2) §11.2 中 B2 依赖改为"useUserApi.ts（扩展 updateProfile）, Profile.vue"，标注 A5 为"非严格依赖——Profile.vue 已直接使用 SweetAlert2，编辑表单可直接沿用"。(3) §12 Group B 的 B1→(A2,A4) 改为 B1→()；B2→(A5) 改为 B2→(A5, 非严格)。 |
| **问题2**：Phase 2 并行人力估算与 3 人约束矛盾——Phase 2 有 8 条并行轨道，3 人无法在 1.5d 内完成，实际总工期 ≥2.2d。C2 搜索功能 0.5d 假设无 API 依赖，可能严重低估。 | **已修正**。(1) 重新计算 Phase 2 人力分配并逐人列出任务负载：人1 C1(1.5d)、人2 B1(1.0d)+D2(0.3d)+D3(0.2d)=1.5d、人3 D1(1.0d)+B2(0.5d)+B3(0.2d)=1.7d，C2 无法在 3 人内消化。(2) 新增 4 人方案（推荐）：Phase 2 1.7d + Phase 1 0.5d + Phase 3 0.5d ≈ 2.7d。(3) C2 工作量从 0.5d 修正为 1.0-2.0d（依赖设计方案），并在 §11.3 新增"搜索功能设计分析"章节，明确后端无搜索端点、设计文档未定义搜索功能、需先完成设计决策。 |
| **问题3**：D3（useSSE 重构）与 A6（新建 useSSE.ts）任务边界模糊——若 A6 抽取后 chatStore 内部改用 useSSE，D3 可能与 A6 高度重叠甚至成为冗余任务。 | **已修正**。明确 A6 产出边界：A6 产出的 useSSE composable 供 chatStore 内部调用，chatStore 对外仍暴露 sendMessage/sendAdminMessage 等高层接口，下游页面无需感知 useSSE 的存在。D3 的任务从"Admin.vue 改调 chatStore.sendAdminMessage"明确为"Admin.vue 废弃内联的 parseSSEBuffer/readSSEStream/dispatchSSEEvent 三函数，改调已存在的 chatStore.sendAdminMessage"，工作量从 0.5d 降至 0.2d。D3 独立于 A6——chatStore.sendAdminMessage 已是完整的功能接口，A6 完成后 chatStore 内部改用 useSSE 但 D3 不感知此变化。 |
| **问题4**：未验证后端 API 实际实现状态——报告列出 7 个未覆盖 API 端点并据此规划前端并行开发任务，但未检查这些后端端点是否已实现。 | **已修正**。(1) 阅读验证了 `server/routes/` 下全部 12 个路由文件。(2) 在 §10.1 新增"后端 API 实现状态验证"小节，逐端点列出后端实现位置和核心逻辑（含具体行号）。(3) 7 个端点全部已实现——前端开发的阻塞因素并非后端不可用，而是前端 composable 层和页面交互层未实现。前端开发无需 mock。(4) 在 §14 优先级清单中新增"后端状态"列，逐项标注后端实现状态。 |
| **问题5**：搜索功能（C2）前置条件分析缺失——标记为"无前置依赖""独立功能"，工作量 0.5d，但完全未分析搜索功能需要什么。 | **已修正**。(1) 在 §11.3 新增"搜索功能设计分析"章节，列出搜索范围、结果展示、API 需求三个待决策维度，并标注后端当前无搜索端点的事实。(2) C2 工作量从 0.5d 修正为 1.0-2.0d，说明不同方案对应的工作量范围。(3) 在 §12 执行顺序备注中标注"建议在启动 Phase 2 前先完成搜索功能的设计决策"。 |
| **问题6**：E1 持久化方案切换缺乏决策框架——2.0d 工作量但未列出方案优劣对比、未定义"功能稳定"标准、未评估切换成本和回滚路径。 | **已修正**。(1) E1 从编码任务改为决策前置任务（0.5d），产出决策文档。(2) 新增 sessionStorage+BroadcastChannel vs localStorage+pinia-plugin-persistedstate 对比分析矩阵（安全隔离/跨标签页体验/持久化/实现复杂度/依赖）。(3) 给出明确推荐：保留当前 sessionStorage+BroadcastChannel 方案。(4) 决策前置动作：Phase 2 完成后收集多标签页问题数据作为是否迁移的补充判断依据。(5) 若后续决定迁移，编码+测试拆解为 1.0d（非当前必备项）。 |
| **问题7**：部分差距项未纳入 §14 优先级清单——路由守卫免责声明拒绝行为修复已在 §14 列为 P3（第 784 行）；Login.vue 死链接「立即注册」为用户可感知缺陷，应作为独立项纳入。 | **已修正**。(1) 确认路由守卫修复已在 §14 列为 P3，补充"后端状态"列为"—"（纯前端问题）。(2) 在 §14 新增独立条目"Login.vue 死链接「立即注册」"（P0），说明：此为独立于注册功能实现的用户可感知缺陷——用户点击后停留在登录页面且无任何反馈，建议在注册功能实现前将链接替换为 Toast 提示或改为非链接样式。(3) 在 §14 新增"Admin.vue SSE 逻辑重复"条目（P2），作为独立可行动项。 |
...
| 质询意见 | 回应 |
|---------|------|
| 第9节 API 覆盖率统计数据错误——声称26已覆盖/5未覆盖/31总计/83.9%，实际约25已覆盖/7未覆盖/32可操作端点/~78% | **已修正**。对表格逐行重新核对，修正为：25 已覆盖 / 7 未覆盖（POST /api/auth/register、PUT /api/user/profile、GET /api/chat/doctor/:id/conversations、POST /api/articles/:id/collect、DELETE /api/articles/:id/collect、GET /api/articles/collections、GET /api/assistant/conversations）/ 32 可操作端点 / 覆盖率 ≈78.1%。原统计错误系人工计数时将 register 和 2 个 conversations 端点从"未覆盖"误判为"已覆盖"或遗漏。 |
| 第7.3节引用 `Login.vue:77` 的自环链接，实际代码位于 `Login.vue:76`，行号偏差1行 | **已修正**。`Login.vue:77` → `Login.vue:76`（对应实际代码第76行 `<router-link to="/login">立即注册</router-link>`）。原行号偏差系编码时引用误差。 |
| 第7.6节指出 LifePlan.vue 已使用 renderMarkdown() 统一渲染管道，第7.5/7.8/7.12节指出其他页面内联 marked+DOMPurify 未复用 useMarkdown.ts，但报告未解释为何同一 composable 已在部分页面使用而其他页面未用 | **已补充说明**。在第 8.6 节（原 7.6 节）LifePlan.vue 分析后新增"Markdown 渲染统一化说明"段落，并在第 13 节（原 12 节）架构偏离表中补充根因解释：LifePlan.vue 开发较晚，useMarkdown composable 已存在时被采用；其余页面在 useMarkdown 创建前完成开发，未做后续统一重构。功能等价（均为 marked.parse + DOMPurify.sanitize），但内联方案缺失 useMarkdown 内置的链接安全渲染器（`rel="noopener noreferrer"`）。 |
| 第7.7节（NewsView）将"文章收藏按钮未实现：列表中不显示收藏状态"列为差距，但设计文档 4.1.5 节 NewsView 组件树中未定义列表页收藏按钮（仅在 ArticleDetailView 中定义收藏按钮），此差距描述的依据不完全来自设计文档 | **已修正**。将"文章收藏按钮未实现"改为"列表页收藏状态展示未实现"，并注明设计文档 4.1.5 节 NewsView 组件树确实未定义列表页收藏按钮，此点属于功能完整性补充建议而非设计合规差距。收藏功能的核心未实现部分仍为 ArticleDetailView 的 toggleCollect() 占位。 |
| 路由模块（router/index.ts）缺少独立的模块差距分析 | **已新增第 6 节"路由模块差距分析"**。对照设计文档 1.6 节路由表和 1.6.2 节导航守卫伪代码，逐条对比了 14 条路由的路径、meta 字段、懒加载语法和嵌套路由结构；逐一分析了 5 步导航守卫的实现状态。发现以下差距：(1) JWT 过期检测缺失（仅检查 token 存在性不检查 exp）；(2) 免责声明拒绝行为偏离（设计 next(false) 保留来源页，实际重定向 /home）；(3) 导航守卫步骤 3/4 顺序颠倒；(4) 免责声明函数未走 useUI composable；(5) 路由表缺少 requiresDisclaimer 显式声明（设计文档精度不足）。同步更新了依赖关系分析（第 11 节）、并行开发分组（第 12 节，新增 A8 路由守卫修复项和 B3 JWT 过期检测项）和关键未实现功能清单（第 14 节）。 |

## 修订说明（v3）

| 质询意见 | 回应 |
|---------|------|
| §2.1 第4项（`syncFromStorage` 调用）使用 ❌ 前缀，但内容描述为"此点符合设计"，应为 ✅，属标记格式错误 | **已修正**。将 ❌ 改为 ✅，同时追加说明：`main.ts:12` 注释"自动从 localStorage 恢复登录态"与实际存储介质 `sessionStorage`（`authStore.ts:39-41`）不一致，注释需修正为 sessionStorage。 |
| §11.2 路由守卫免责声明拒绝行为修复（A8）标注依赖 useUI composable（A5），但最简方案（仅修改 `router/index.ts:137` 的 `next('/home')` 为保留来源页语义）独立于 A5 的完成 | **已细化**。将 A8 的前置依赖从"无（严格依赖 A5）"改为非严格依赖：最简方案独立于 A5，最佳实践（将 disclaimer 函数移入 useUI）依赖 A5。同步更新了 §12 Group A 的 A8 行描述和依赖链说明。 |
| §10 API 覆盖率表格遗漏了设计文档 §3.1.11 定义的 2 个 Dify 代理端点：`POST /api/dify/workflow/:workflow_id` 和 `POST /api/dify/agent/:agent_id`。经前端代码 grep 验证，当前前端无任何文件调用 `/api/dify/*` 路径，这两个端点为 Express 后端内部调用 | **已补充**。在 API 覆盖率表格中新增 2 个 Dify 代理端点的行，标注 N/A（后端内部调用，前端不直调），并说明其由各专用端点（如 POST /api/risk/predict、POST /api/assistant/chat 等）在服务端内部转发至 Dify。覆盖率统计的总行数从 33 行更新为 35 行（含 3 个 N/A 端点），可操作端点和覆盖率保持不变（25/32 ≈ 78.1%）。此补充对覆盖率分析和差距结论无实质性影响（7 个未覆盖 API 的识别不受影响），仅提升计数精确性。
...
| 质询意见 | 回应 |
|---------|------|
| **问题1**：env.d.ts 缺失影响分析不准确——Report声称 env.d.ts 缺失导致 ".vue 文件 import 可能报类型错误"并设 A1(0.1d)，但 `tsconfig.app.json:5` 已配置 `"types": ["vite/client"]`，Vite 客户端类型库内置了 `.vue` 模块类型声明，项目实际不存在因 env.d.ts 缺失导致的编译错误。此问题在 v2/v3/v4 多轮修订中持续存在未修正。 | **已彻底修正**。(1) 经代码验证，`tsconfig.app.json:5` 确实引入 `"types": ["vite/client"]`，Vite 客户端类型库内置 `.vue` 模块声明。(2) 经全局 grep 搜索确认，项目 src/ 下无任何 `VITE_` 环境变量使用、无 `import.meta.env` 引用，Vite 环境变量类型扩展也无实际需要。(3) §2.2 重写：明确 Vite 已内置 `.vue` 类型声明，A1 中 `.vue` 模块声明部分为冗余工作；Vite 环境变量类型扩展在当前也无实际需要。(4) A1 任务工作量从 0.1d 改为 0d，标记为"取消"，并附注说明条件。 |
| **问题2**：搜索功能分析仅罗列选项，缺乏可操作的推荐方向。此问题在 v3/v4 多轮修订中持续存在——始终停留在"罗列维度 + 建议先做设计决策"的层面，从未给出明确方案推荐。 | **已突破性修正**。(1) §11.3 全面重写：基于项目当前约束（后端无搜索端点、设计文档未定义搜索、Home.vue 仅一个入口、文章量级 <100 条）给出明确推荐方向——「最简可行方案：前端本地过滤（文章标题+标签）+ NewsView.vue 内联搜索结果（URL ?keyword=xxx 驱动）」。(2) 给出四层选择依据：文章量级约束（<100条无性能瓶颈）、开发周期短、无需后端端点、升级路径清晰（>200条时升级为后端搜索）。(3) 明确适用条件：文章总量 <200 条阶段；超过此量级需升级为后端全文搜索方案。(4) 新增 A10 搜索设计决策前置任务（0.2d）纳入 Phase 1，确保 C2 编码前对齐交互预期。(5) C2 工作量明确为 1.0d（最简方案），取消 1.0-2.0d 的范围取值。(6) 备选方案说明保留以供决策参考。 |
| **问题3**：并行开发中 D1/D3 共享文件（chatStore.ts）的合并冲突风险未评估——Phase 2 将 D1（扩展 useChatApi.ts + chatStore.ts）与 D3（Admin.vue 调用 chatStore 路径变更）并行列队，但 D1 和 D3 均涉及 chatStore.ts 的修改，可能产生合并冲突。 | **已修正**。(1) D3 行新增"文件冲突提示"：标注 D1 和 D3 共享 chatStore.ts，建议 D3 在 D1 的 chatStore 改动合入后再启动，或在人力分配中让同一人承担 D1 和 D3。(2) D 依赖链更新：D3 标注"建议在 D1 合入后启动"，D1/D3 从全并行改为半并行。(3) 执行顺序注释补充。(4) 新增 D4 行（与 A9 联动，去重清理任务）。 |
| **问题4**：E1 决策任务与内嵌分析内容严重重叠——本报告 §12 第 759-787 行已完整完成了 sessionStorage+BroadcastChannel vs localStorage+pinia-plugin-persistedstate 的 5 维度对比分析和明确推荐。E1 的 0.5d 产出物与已写就的分析内容高度重叠，增量工作不明确。 | **已修正**。(1) E1 工作量从 0.5d 降为 0.1d，任务描述从"产出决策文档"改为"将已有分析格式化输出为独立决策文档以供团队归档"。(2) 明确增量工作：仅格式化整理现有分析内容，决策本身无需重新执行。(3) E1 仍保留为 Phase 3 独立任务，但工作量已反映实际的归档工作量。 |
| **问题5**：文章收藏 P0 优先级排序缺少理由，与 JWT 过期检测 P1 形成矛盾——文章收藏（单页面功能）P0 但 JWT 过期检测（影响全局认证安全）P1，排序依据缺失。 | **已修正**。(1) §14 开头新增"优先级定义"段落，明确定义 P0（阻塞核心功能：用户可感知的功能缺失，直接影响产品使用闭环）、P1（安全性/高级体验：有替代机制兜底）、P2（架构合规/体验优化）、P3（清洁工/文档对齐）。(2) JWT 过期检测条目补充兜底机制说明：useApi.ts 401 响应拦截器在 token 过期后首次 API 调用时触发重新登录，但 token 过期后到首次 API 调用之间存在短暂窗口。(3) 文章收藏保持 P0：用户点击收藏按钮无响应，属于用户可感知的核心功能缺失，且与认证/注册行成产品「阅读→收藏」的核心闭环。(4) env.d.ts 从 P3 清单中移除（已取消）。 |
| **问题6**：全量拉取搜索方案的扩展性风险未提及——C2 最简方案描述为"前端拉取文章全量后本地过滤"，但未提及文章量级增长后的性能瓶颈。 | **已修正**。(1) §11.3 推荐方案新增"适用条件"：明确仅适用于文章总量 <200 条的阶段，超过此量级需升级为后端搜索方案。(2) C2 任务行和 §11.3 搜索分析中同步标注适用条件。(3) 选择依据中纳入量级约束说明（<100条无瓶颈）。 |
| **问题7**：Markdown 统一化（A7）的链接属性变更可能影响视觉行为——`renderMarkdown()` 额外注入 `rel="noopener noreferrer"` 到所有链接，但报告将此变化仅描述为"安全改进"，未评估潜在的视觉副作用。 | **已修正**。(1) A7 任务行新增"低风险提示"：`renderMarkdown()` 通过 marked 链接渲染器（`useMarkdown.ts:35-40`）向外部链接注入 `rel="noopener noreferrer"`，此为安全改进，不改变内容结构，建议在 5 个页面各做 1 次快速冒烟测试确认链接样式无异样。 |
| **问题8**：sendAdminChatMessage 重复定义未纳入修复计划——`useAdminApi.ts:28-48` 和 `useChatApi.ts:97-118` 中的 `sendAdminChatMessage` 实现完全一致，但该重复未作为待修复差距纳入任何优先级清单或开发分组。 | **已修正**。(1) 经代码比对确认两处实现完全一致（函数签名、fetch 逻辑均相同）。(2) 新增 A9 任务（0.1d）：sendAdminChatMessage 重复定义去重，与 D3 联动——D3 完成后 useAdminApi.ts 版本成为死代码，可一并清理。(3) §14 优先级清单新增 P2 条目：sendAdminChatMessage 重复定义。(4) D 组新增 D4 行（与 A9 为同一任务的不同视角）。(5) 执行顺序和依赖链中纳入 A9/D4。 |
...
| 质询意见 | 回应 |
|---------|------|
| A9 任务调度声明与实际代码依赖矛盾：§12 Group A 声称 A9（删除 `useAdminApi.ts` 的 `sendAdminChatMessage`）可在 Phase 1 独立执行且"不影响运行行为"，但经代码验证 `Admin.vue:8` 当前从 `@/composables/useAdminApi` 导入该函数且在第 155 行实际调用——在当前代码中该函数非死代码。若 Phase 1 执行 A9 而 Phase 2 的 D3 尚未迁移 Admin.vue 至 chatStore.sendAdminMessage，Admin.vue 将因缺失导入而构建失败。此矛盾使执行者对 A9 的执行时机产生误判，直接影响并行开发分组的可操作性。 | **已修正**。(1) 经代码验证确认 `Admin.vue:8` 导入 `sendAdminChatMessage` 并实际调用，该函数非死代码，A9 不可在 D3 之前独立执行。(2) A9 从 Group A（Phase 1）完全移除。(3) D4（Group D, Phase 2）扩展为 `sendAdminChatMessage` 去重的唯一任务，明确标注严格依赖 D3 完成，并补充"不可在 Phase 1 独立执行"的警告和代码证据（`Admin.vue:8:155`）。(4) Group A 依赖链中移除 A9 相关描述。(5) Phase 1 执行顺序图中移除 A9 行；Phase 2 中 D3/ D4 注释从"与D4联动"改为"D4严格依赖D3"。 |
