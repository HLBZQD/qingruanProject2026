...
- ❌ **注册表单完全缺失**：设计定义的注册表单（用户名 + 密码 + 确认密码 + 验证 + 提交注册）未实现。当前"立即注册"链接指向 `/login`（自环）。
  - 根因：`Login.vue:76` 的 `<router-link to="/login">立即注册</router-link>` 是自环链接，未触发注册视图展示。
...
| JWT 过期检测（路由守卫） | useAuth composable (A4) | 路由守卫需引入 useAuth 进行 token exp 检查，当前仅检查 token 存在性 |
...
| Markdown 渲染统一化 | useMarkdown.ts 已存在 | 将 DoctorChatView/ArticleDetailView/HealthAdvice/Admin/Risk 的内联 marked+DOMPurify 替换为 renderMarkdown() 调用 |
| 路由守卫免责声明拒绝行为修复 | (A5，非严格) | 最简方案（仅修改 router/index.ts:137 为 next(false) 语义）独立于 A5 完成；最佳实践（将 disclaimer 函数移入 useUI）依赖 A5 |
...
| A7 | Markdown 渲染统一化 | 0.3d | 将 DoctorChatView/ArticleDetailView/HealthAdvice/Admin/Risk 的内联 marked+DOMPurify 替换为 renderMarkdown() 调用 |
| A8 | 路由守卫免责声明拒绝行为修复 | 0.1d | router/index.ts:137 修改为 next(false) 语义（最简方案独立于 A5）；最佳实践需 A5 完成后同步将 disclaimer 函数移入 useUI |

**依赖链**：A 组内无相互依赖，可全并行。A8 最简方案独立于 A5，最佳实践 A8→(A5)。**A 组总工期：0.5d**（取最长路径 A6）。
...
| B3 | 路由守卫 JWT 过期检测 | 0.2d | 引入 A4（useAuth）进行 token exp 检查 |

**依赖链**：B1→(A2,A4)；B2→(A5)；B3→(A4)。B1/B2/B3 可并行。**B 组总工期：1.0d**（取最长 B1）。
...
Phase 2 ──┐  B1 注册表单        (1.0d)
           ├  B2 编辑资料        (0.5d)
           ├  B3 路由JWT过期检测 (0.2d)
           ├  C1 文章收藏        (1.5d)  ← 可与 B 组并行
           ├  C2 搜索功能        (0.5d)  ← 可与 B/C1 并行
...
**总串行工期估算**：~4.0d（含 E1）
**总并行工期估算**：~3.5d（Phase 1 0.5d + Phase 2 1.5d + Phase 3 2.0d）
...
| 路由守卫 JWT 过期检测 | 设计步骤 2 检查"Token是否存在且未过期" | 仅检查 token 存在性，不检测 exp | router/index.ts:118 | 引入 useAuth composable（A4 项）进行 token exp 检查 |
| 路由守卫免责声明函数位置 | useUI().hasAcceptedDisclaimer() / showDisclaimer() | 函数直接定义在 router/index.ts | router/index.ts:93-109 | 完成 useUI composable（A5 项）后移入 |
| 路由守卫免责声明拒绝行为 | next(false) 保留来源页 | 始终重定向 /home | router/index.ts:137 | 修复为 next(false) 语义（A8 项） |
...
| P3 | models.ts | 文件缺失 | — |
| P3 | 路由守卫免责声明拒绝行为修复 | `router/index.ts:137` | 全局 |
