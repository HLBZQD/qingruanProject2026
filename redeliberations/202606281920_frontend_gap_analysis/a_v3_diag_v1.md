...
| A9 | sendAdminChatMessage 重复定义去重 | 0.1d | `useAdminApi.ts:28-48` 与 `useChatApi.ts:97-118` 中的 `sendAdminChatMessage` 实现完全一致（函数签名和 fetch 逻辑均相同）。此任务与 D3 联动——D3 将 Admin.vue 迁移至 chatStore.sendAdminMessage 后，`useAdminApi.ts` 中的 `sendAdminChatMessage` 即成为死代码。执行方式：D3 完成后，移除 `useAdminApi.ts` 中的 `sendAdminChatMessage`，并在 Admin.vue 中调整导入路径（从 `@/composables/useAdminApi` 改至 `chatStore` 或 `@/composables/useChatApi`） |
| A10 | 搜索功能设计决策 | 0.2d | 见 §11.3 推荐方案的「前置决策任务」：产出搜索交互原型（NewsView.vue 搜索模式下的 UI 草稿 + 搜索状态转换图），明确搜索触发逻辑和空结果/错误处理，确保 C2 编码前对齐交互预期 |

**依赖链**：A 组内无相互依赖，可全并行。A9 与 D3 联动（D3 完成后 A9 可执行），但 A9 可独立于 D3 先于 Phase 1 执行（仅影响清理 `useAdminApi.ts` 的死代码，不影响运行行为）。A8 最简方案独立于 A5，最佳实践 A8→(A5)。A10 为 C2 前置，应在 Phase 1 内完成。**A 组总工期：0.5d**（取最长路径 A6）。
...
| D4 | sendAdminChatMessage 重复定义去重 | 0.1d | 与 A9 为同一任务，仅在不同分组视角下陈述。若 A9 在 Phase 1 已执行，D4 无需重复。执行方式：D3 完成后移除 `useAdminApi.ts` 中的 `sendAdminChatMessage`，Admin.vue 改从 `chatStore` 导入。 |

**依赖链**：D1 无前置依赖（extension of useChatApi.ts is self-contained）；D2 在 chatStore 内闭环，无外部依赖；D3 建议在 D1 合入后启动（共享 chatStore.ts 修改，避免合并冲突）——或由同人承担 D1+D3 以消除冲突；D4 依赖 D3 完成（与 A9 联动）。D1/D2 可并行，D3 与 D1 半并行（同人或先后执行）。**D 组总工期：1.0d**（取最长 D1）。
...
Phase 1 ──┐  A1 ~~env.d.ts~~         (0d, 取消)
           ...
           ├  A7 Markdown统一        (0.3d)
           ├  A8 路由守卫修复         (0.1d)
           ├  A9 sendAdminChatMessage去重(0.1d, 可与D3/D4联动)
           └  A10 搜索设计决策        (0.2d)
               (可全并行，最大路径 A6: 0.5d)
Phase 2 ──┐  B1 注册表单             (1.0d)
           ...
           ├  D1 会话历史             (1.0d)
           ├  D2 AI 导航              (0.3d)
           ├  D3 Admin SSE统一        (0.2d)  ← 建议D1合入后启动; 与D4联动
           └  D4 去重联动清理          (0.1d, 与A9同一任务)
               (可全并行，D3建议D1合入后)
