# detail_v1.md — 风险预测与生活方案核心闭环 技术规格
...
`PlanItem` 类型：`{ plan_type: string, order_num: number, time_desc: string, title: string, content: string }`（与 `life_plans` 表列对应）。
...
Step 2 - Step 7: 在 db.transaction 内执行
...
Step 8: 分组构造响应
  const dietPlans = items.filter(i => i.plan_type === 'diet')
  const exercisePlans = items.filter(i => i.plan_type === 'exercise')
  const otherPlans = items.filter(i => i.plan_type === 'other')
...
Step 8: 返回（与 generate 一致结构）
  success(res, {
    plan_id: maxId,
    diet_plans: items.filter(i => i.plan_type === 'diet'),
    exercise_plans: items.filter(i => i.plan_type === 'exercise'),
    other_plans: items.filter(i => i.plan_type === 'other') || []
  }, '方案调整成功')
```
