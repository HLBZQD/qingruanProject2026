...
>
> **v2 修订**：修复 R1（BMI 空结果崩溃）、R2（黑名单误杀）、R4（时间格式不一致）、S3（multer fileFilter 确认）。
...
POST /generate
  ...
  │   ├─ bmi !== null && bmi > 24                       │
  │   │   → 饮食指导 recommended=true                    │
  │   │     reason="基于您的BMI，饮食管理是..."            │
  │   ├─ bmi !== null && bmi > 28                       │
  │   │   → 运动指南 recommended=true                    │
  │   │     reason="基于您的BMI，适量运动..."              │
  │   └─ 返回 { stage: "category_selection", categories } │
  │      (无记录时全部 recommended=false，优雅降级)       │
  │
  └─ 有 category ───────────────────────────────────────┐
...
const bmi = riskRow ? riskRow.bmi : null;
// riskRow 为 undefined 时 bmi = null，后续 bmi > 24/bmi > 28 均不触发，
// 所有分类保持 recommended=false，优雅降级
```
...
-- 插入文章
-- 使用 strftime 生成 ISO8601 T 分隔格式，与设计文档 3.2.21 响应格式一致
INSERT INTO articles (user_id, title, cover, author, content, category, tags, summary, created_at)
VALUES (?, ?, ?, 'AI健康助手', ?, ?, ?, ?, strftime('%Y-%m-%dT%H:%M:%S', 'now', 'localtime'))
```
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (allowed.includes(file.mimetype)) {
      ...
      // multer >= 1.4.0 会将 fileFilter 中 cb(new Error(...)) 的 Error 实例
      // 原样传播到 upload.single() 的回调 err 参数中。
      // AppError 继承 Error，因此 instanceOf AppError 检查有效。
...
upload.single('avatar') err
  ...
  │   (multer >= 1.4.0 保证 fileFilter 的 Error 原样传播，AppError 可被正确识别)
  │
  ...
  "data": [
    {
      ...
      "operation_content": "SELECT * FROM users",
      "operation_result": "成功, 影响10行",
...
- `operation_time` 列由 SQLite 生成（admin_logs 表 DEFAULT）

### 5.6 响应挂载 pagination 的处理

不使用 `success()` 辅助函数，直接构造完整对象后 `res.json()`：

```js
res.status(200).json({ success: true, message: '查询成功', data: rows, pagination });
```

原因：`success()` 签名 `(res, data, message, statusCode)` 直接调用 `res.status().json()`，无法在调用后追加 `pagination` 字段。与 `articles.js` 中 `res.status(200).json({ success: true, ... })` 风格一致。
...
for (const keyword of forbidden) {
  // 使用 \b 单词边界 + 'i' 大小写不敏感，只匹配完整 SQL 关键字，
  // 避免误杀列名/表名/字符串字面量中的子串。
  // 例如 SELECT last_insert_rowid() → 不匹配（insert 是子串而非独立单词）
  // 例如 SELECT update_time FROM ... → 不匹配
...
- `trimmed` 在 6.2.2 中已经 `toUpperCase()`，此处直接使用，无需再次 `.toUpperCase()`
- `'i'` 标志作为防御性编程保留，即使 `trimmed` 全大写也能正确处理
- `\b` 单词边界确保只匹配独立 SQL 关键字，不误伤含有关键词子串的合法 SQL（如 `last_insert_rowid()`、`EXTRACT()`）
...
| `forbidden` 关键词 + `\b` 边界 | 正则黑名单 | 子查询注入/EXEC 调用，仅匹配独立关键字 |
...
| created_at 格式 | 使用 `strftime('%Y-%m-%dT%H:%M:%S', 'now', 'localtime')`，输出 ISO8601 T 分隔格式，与设计文档 3.2.21 一致 |
| multer fileFilter AppError | multer >= 1.4.0 会将 fileFilter 中 `cb(new Error(...))` 的 Error 实例原样传播到 `upload.single()` 回调，`instanceof AppError` 检查有效 |
...
| 编号 | 严重度 | 问题 | 修订内容 |
|------|--------|------|---------|
| R1 | 🔴 CRITICAL | BMI 查询空结果崩溃 | §3.6 新增空结果处理：`riskRow ? riskRow.bmi : null`，无记录时所有分类 recommended=false |
| R2 | 🔴 CRITICAL | admin/execute 黑名单子串误杀 | §6.2.3 改为 `new RegExp('\\b' + keyword + '\\b', 'i').test(trimmed)` 单词边界匹配，同时移除冗余 `.toUpperCase()` 调用 |
| R4 | 🟡 HIGH | Mock 文章 created_at 格式不一致 | §3.8 INSERT 改为 `strftime('%Y-%m-%dT%H:%M:%S', 'now', 'localtime')`，确保与设计文档 3.2.21 响应格式一致 |
| S3 | 🟡 NON-BLOCKING | multer fileFilter AppError 传播确认 | §4.4/§4.6 新增 multer >= 1.4.0 版本说明，确认 `instanceof AppError` 有效 |
